...
def _expected_control_vector(control: dict[str, Any]) -> dict[str, Any]:
    return {
...
def _mutation_probe(
    ...
    return {
        ...
        "packet_digest_changed": (
            changed.get("packet_semantic_digest")
            != pinned_receipt.get("packet_semantic_digest")
        ),
        "operator_digest_changed": (
            changed.get("operator_registry_digest")
            != pinned_receipt.get("operator_registry_digest")
        ),
        "receipt_digest_changed": (
            changed.get("receipt_digest") != pinned_receipt.get("receipt_digest")
        ),
    }
...
def verify(repo_root: Path | None = None) -> dict[str, Any]:
    ...
    checks["packet_digest_pinned"] = (
        packet_validation.get("packet_semantic_digest")
        == registry["packet_semantic_digest"]
        == packet.get("packet_semantic_digest")
    )
    checks["operator_digest_pinned"] = (
        packet_validation.get("operator_registry_digest")
        == registry["operator_registry_digest"]
        == packet.get("operator_registry_digest")
    )
    ...
    for name in ("reversible_zero", "irreversible_loss"):
        ...
        checks[f"{name}_identity_pinned"] = (
            _control_vector(receipt) == _expected_control_vector(control)
        )

    zero = generated["reversible_zero"]
    checks["zero_control_zero"] = (
        zero.get("raw_result", {}).get("classification") == "ZERO_RESIDUE"
        and zero.get("raw_result", {}).get("residue") == {}
        and zero.get("raw_result", {}).get("residue_zero") is True
    )

    loss = generated["irreversible_loss"]
    expected_loss = registry["controls"]["irreversible_loss"][
        "expected_irreversible_loss"
    ]
    checks["loss_control_nonzero"] = (
        loss.get("raw_result", {}).get("classification") == "NONZERO_RESIDUE"
        and loss.get("raw_result", {}).get("residue_zero") is False
        and loss.get("raw_result", {})
        .get("residue", {})
        .get("__irreversible_loss__", {})
        .get("after")
        == expected_loss
    )
    ...
    replay = replay_evaluation_receipt(
        ...
        packet,
        state,
        registry["controls"]["irreversible_loss"]["edge_path"],
    )
    checks["stored_receipt_replays"] = replay.get("status") == "MATCH"

    mutation = _mutation_probe(packet, state, zero)
    checks["declaration_mutation_detected"] = all(
        (
            mutation["status"] == "RECORDED",
            mutation["packet_digest_changed"],
            mutation["operator_digest_changed"],
            mutation["receipt_digest_changed"],
        )
    )

    tampered = copy.deepcopy(pinned_receipt)
    tampered["raw_result"]["classification"] = "FORGED"
    checks["receipt_tamper_detected"] = (
        validate_evaluation_receipt(tampered).get("status") == "HOLD"
    )

    attestation = registry["external_attestation"]
    checks["external_attestation_unbound"] = (
        attestation.get("standing") == "NOT_SELF_BOUND"
        and all(
            attestation.get(key) is None
            for key in (
                "repository",
                "git_head",
                "workflow",
    ...
    details["packet_semantic_digest"] = packet_validation.get(
        "packet_semantic_digest"
    )
    details["operator_registry_digest"] = packet_validation.get(
        "operator_registry_digest"
    )
    details["zero_receipt_id"] = zero.get("receipt_id")
    details["loss_receipt_id"] = loss.get("receipt_id")
    details["stored_validation"] = stored_validation
    details["replay"] = replay
    details["mutation_probe"] = mutation
