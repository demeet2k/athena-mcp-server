# Manuscript Cell 0333

## Cell title

Square x Code x Runtime Body x Loop

## Zero-point theorem

`Square` geometry instructs `Code` to metabolize `Runtime Body` through `Loop` closure without losing route identity.

## Tensor address

`<Geom=Square, Op=Code, Body=Runtime Body, Closure=Loop, Rail=Su, Arc=2, Hub=AppC->AppF->AppP->AppN->AppM, Truth=AMBIG, Regime=restart, Lineage=EAAA>`
...
- `Level I`: root manuscript `0333`
- `Level II`: cluster family `0333::*`
- `Level III`: neuron family `0333::*::*`
- `Level IV`: toroidal return family `0333::*::*::*`

## Expansion law

This root manuscript expands into 256 cluster manuscripts under `0333::*`, then 256 neuron manuscripts under each cluster, then 256 toroidal return manuscripts under each neuron.
