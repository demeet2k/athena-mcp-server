# Manuscript Cell 0302

## Cell title

Square x Code x Foundation Body x Witness

## Zero-point theorem

`Square` geometry instructs `Code` to metabolize `Foundation Body` through `Witness` closure without losing route identity.

## Tensor address

`<Geom=Square, Op=Code, Body=Foundation Body, Closure=Witness, Rail=Sa, Arc=5, Hub=AppC->AppF->AppA->AppI->AppM, Truth=PROMOTABLE, Regime=promotion, Lineage=EAEF>`
...
- `Level I`: root manuscript `0302`
- `Level II`: cluster family `0302::*`
- `Level III`: neuron family `0302::*::*`
- `Level IV`: toroidal return family `0302::*::*::*`

## Expansion law

This root manuscript expands into 256 cluster manuscripts under `0302::*`, then 256 neuron manuscripts under each cluster, then 256 toroidal return manuscripts under each neuron.
