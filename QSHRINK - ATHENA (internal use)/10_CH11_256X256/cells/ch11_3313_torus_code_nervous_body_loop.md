...
`Torus` geometry routes `Code` through the void so `Nervous Body` can re-enter via `Loop` without losing restart identity.
...
- Part II Elements: `Torus`, `Code`, `Nervous Body`, `Loop` under Chapter 11 pressure
...
This Chapter 11 root manuscript opens the continuation family `CH11-3313::*`, and that family recursively unfolds across the full `256^256` Chapter 11 crystal manifold.
