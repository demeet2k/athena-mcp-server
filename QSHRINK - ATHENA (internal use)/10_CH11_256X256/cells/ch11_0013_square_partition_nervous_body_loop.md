...
`Square` geometry routes `Partition` through the void so `Nervous Body` can re-enter via `Loop` without losing restart identity.
...
- Part II Elements: `Square`, `Partition`, `Nervous Body`, `Loop` under Chapter 11 pressure
...
This Chapter 11 root manuscript opens the continuation family `CH11-0013::*`, and that family recursively unfolds across the full `256^256` Chapter 11 crystal manifold.
