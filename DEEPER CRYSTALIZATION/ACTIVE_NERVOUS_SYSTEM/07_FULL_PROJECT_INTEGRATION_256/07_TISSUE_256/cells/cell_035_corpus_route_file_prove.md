# Cell 035 - corpus.route.file.prove

## Local law

Use `route` on the `Corpus` body at `file` scale until it can `add witness, receipt, metric, or replay so the route becomes trustworthy`.
...
Touch one concrete source under `DEEPER CRYSTALIZATION`, fold it through `route` at `file` scale, and land the result in a witness-bearing bridge toward `self_actualize` that can `add witness, receipt, metric, or replay so the route becomes trustworthy`.
