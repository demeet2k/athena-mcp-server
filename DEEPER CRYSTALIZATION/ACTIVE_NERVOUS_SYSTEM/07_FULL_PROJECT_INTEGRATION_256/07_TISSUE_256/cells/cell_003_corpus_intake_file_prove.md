# Cell 003 - corpus.intake.file.prove

## Local law

Use `intake` on the `Corpus` body at `file` scale until it can `add witness, receipt, metric, or replay so the route becomes trustworthy`.
...
Touch one concrete source under `DEEPER CRYSTALIZATION`, fold it through `intake` at `file` scale, and land the result in a witness-bearing bridge toward `self_actualize` that can `add witness, receipt, metric, or replay so the route becomes trustworthy`.
