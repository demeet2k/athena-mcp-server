# Cell 052 - corpus.replay.file.publish

## Local law

Use `replay` on the `Corpus` body at `file` scale until it can `promote the route into a canonical surface other agents can safely inherit`.
...
Touch one concrete source under `DEEPER CRYSTALIZATION`, fold it through `replay` at `file` scale, and land the result in a witness-bearing bridge toward `self_actualize` that can `promote the route into a canonical surface other agents can safely inherit`.
