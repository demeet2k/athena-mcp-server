# Cell 175 - runtime.route.ecosystem.prove

## Local law

Use `route` on the `Runtime` body at `ecosystem` scale until it can `add witness, receipt, metric, or replay so the route becomes trustworthy`.
...
Touch one concrete source under `self_actualize`, fold it through `route` at `ecosystem` scale, and land the result in a witness-bearing bridge toward `ACTIVE_NERVOUS_SYSTEM` that can `add witness, receipt, metric, or replay so the route becomes trustworthy`.
