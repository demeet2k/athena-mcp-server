# Cell 014 - corpus.intake.ecosystem.link

## Local law

Use `intake` on the `Corpus` body at `ecosystem` scale until it can `connect the representation to parents, peers, and downstream destinations`.
...
Touch one concrete source under `DEEPER CRYSTALIZATION`, fold it through `intake` at `ecosystem` scale, and land the result in a witness-bearing bridge toward `self_actualize` that can `connect the representation to parents, peers, and downstream destinations`.
