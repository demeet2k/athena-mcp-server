# Cell 211 - manuscript.normalize.file.prove

## Local law

Use `normalize` on the `Manuscript` body at `file` scale until it can `add witness, receipt, metric, or replay so the route becomes trustworthy`.
...
Touch one concrete source under `ACTIVE_NERVOUS_SYSTEM`, fold it through `normalize` at `file` scale, and land the result in a witness-bearing bridge toward `DEEPER CRYSTALIZATION` that can `add witness, receipt, metric, or replay so the route becomes trustworthy`.
