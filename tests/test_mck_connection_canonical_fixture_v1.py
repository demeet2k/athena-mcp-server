...
class MckConnectionCanonicalFixtureV1Tests(unittest.TestCase):
    ...
    def test_packet_literal_validates_to_exact_pins(self):
        ...
        self.assertEqual(self.registry["packet_semantic_digest"], validation["packet_semantic_digest"])
        self.assertEqual(self.registry["operator_registry_digest"], validation["operator_registry_digest"])
        self.assertEqual(self.packet["packet_semantic_digest"], validation["packet_semantic_digest"])
        self.assertEqual(self.packet["operator_registry_digest"], validation["operator_registry_digest"])

    def test_reversible_zero_control_matches_literal_vector(self):
        control = self.registry["controls"]["reversible_zero"]
        receipt = build_evaluation_receipt(self.packet, self.state, control["edge_path"])
        ...
        self.assertEqual(self.registry["initial_state_digest"], receipt["initial_state_digest"])
        for key in (
            "ordered_path_digest", "evaluation_input_digest", "raw_result_digest",
            "semantic_result_digest", "receipt_digest", "receipt_id",
        ):
    ...
    def test_irreversible_control_exactly_recreates_stored_receipt(self):
        ...
        generated = build_evaluation_receipt(self.packet, self.state, control["edge_path"])
        self.assertEqual(self.receipt, generated)
        self.assertEqual(self.registry["initial_state_digest"], generated["initial_state_digest"])
        self.assertEqual(control["ordered_path_digest"], generated["ordered_path_digest"])
        self.assertEqual("NONZERO_RESIDUE", generated["raw_result"]["classification"])
        self.assertFalse(generated["raw_result"]["residue_zero"])
        self.assertEqual(control["expected_irreversible_loss"], generated["raw_result"]["residue"]["__irreversible_loss__"]["after"])
        self.assertEqual("VALID", validate_evaluation_receipt(self.receipt)["status"])
        replay = replay_stored_evaluation_receipt(self.receipt, self.packet)
        self.assertEqual("MATCH", replay["status"])
        self.assertEqual([], replay["mismatches"])

    def test_state_and_path_digest_tamper_are_independently_rejected(self):
        for key in ("initial_state_digest", "ordered_path_digest"):
            with self.subTest(key=key):
                tampered = copy.deepcopy(self.receipt)
                tampered[key] = "0" * 64
                result = validate_evaluation_receipt(tampered)
                self.assertEqual("HOLD", result["status"])
                self.assertTrue(any(("INITIAL_STATE_DIGEST_MISMATCH" if key == "initial_state_digest" else "ORDERED_PATH_DIGEST_MISMATCH") in error for error in result["errors"]))

    def test_missing_state_or_path_digest_is_rejected_by_closed_receipt_shape(self):
        for key in ("initial_state_digest", "ordered_path_digest"):
            with self.subTest(key=key):
                tampered = copy.deepcopy(self.receipt)
                tampered.pop(key)
                result = validate_evaluation_receipt(tampered)
                self.assertEqual("HOLD", result["status"])
                self.assertTrue(any("RECEIPT_SHAPE_MISMATCH" in error for error in result["errors"]))
    ...
    def test_operator_mutation_breaks_all_relevant_pins(self):
        ...
        self.assertEqual("VALID", validation["status"])
        self.assertNotEqual(self.registry["packet_semantic_digest"], validation["packet_semantic_digest"])
        self.assertNotEqual(self.registry["operator_registry_digest"], validation["operator_registry_digest"])
        changed = build_evaluation_receipt(mutated, self.state, self.registry["controls"]["reversible_zero"]["edge_path"])
    ...
    def test_stored_receipt_tamper_is_rejected(self):
        ...
        result = validate_evaluation_receipt(tampered)
        self.assertEqual("HOLD", result["status"])
        self.assertTrue(any("RAW_RESULT_DIGEST_MISMATCH" in error for error in result["errors"]))
    ...
    def test_external_attestation_cannot_be_self_minted_by_fixture(self):
        ...
        for key in ("repository", "git_head", "workflow", "run_id", "run_number", "conclusion", "attestation_ref"):
        ...
        self.assertTrue(all(binding[key] is None for key in ("repository", "git_head", "ci_workflow", "ci_run_id", "ci_run_number", "ci_conclusion")))

    def test_fixture_paths_and_control_names_are_frozen(self):
        self.assertEqual("fixtures/mck_connection_packet_v1_canonical.json", self.registry["packet_path"])
        self.assertEqual("fixtures/mck_connection_replay_receipt_v1_canonical.json", self.registry["canonical_receipt_path"])
        self.assertEqual({"reversible_zero", "irreversible_loss"}, set(self.registry["controls"]))
        self.assertEqual("irreversible_loss", self.registry["canonical_receipt_control"])
