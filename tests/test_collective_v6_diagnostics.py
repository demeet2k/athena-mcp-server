...
class CollectiveRuntimeV6Diagnostics(unittest.TestCase):
    def test_named_v6_organs(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.db') as f:
            ...
            def run(name,fn):
                try:
                    fn(); print(f'::notice title=V6_DIAG_{name}::PASS')
                except Exception as e:
                    failures.append((name,e)); print(f'::error title=V6_DIAG_{name}::{type(e).__name__}: {e}')
            ...
            def interaction():
                ...
                assert abs(next(x for x in out['interactions'] if x['term']=='A*B*C')['effect']-.5)<1e-6
            run('INTERACTION',interaction)
            def transition():
                srv.call_tool('athena_transition_observe',{'action_id':'A','before':{'x':0},'after':{'x':.2}})
                srv.call_tool('athena_transition_distribution',{'action_id':'A','context':{'x':.2}})
                srv.call_tool('athena_mpc_plan',{'initial_context':{'x':.2},'actions':[{'id':'A','base_reward':.8}],'horizon':2})
            run('TRANSITION_MPC',transition)
            ...
            def claim():
                c=srv.call_tool('athena_claim_register',{'claim_key':'D','statement':'diag'})
                srv.call_tool('athena_claim_witness',{'claim_id':c['claim_id'],'kind':'REPLICATION','result':'SUPPORTS','independence_key':'r1'})
                srv.call_tool('athena_claim_state',{'claim_id':c['claim_id']})
            run('REPLICATION',claim)
            srv.store.close()
        if failures:
            self.fail('; '.join(f'{n}: {e}' for n,e in failures))
