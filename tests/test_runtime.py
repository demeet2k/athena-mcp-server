...
class RuntimeTests(unittest.TestCase):
 ...
 def test_exact_budget_allocation_constrains_next(self):
  expensive={'id':'expensive',**{**BASE_METRICS,'delta_j':10},'resource_cost':10}; affordable={'id':'affordable',**BASE_METRICS,'resource_cost':2}
  out=compile_orchestration('seed',[expensive,affordable],budget={'total_cost':2,'max_branches':1,'unit':'gpu-hour'}); self.assertEqual(out['allocation_plan']['solver'],'EXACT_ENUMERATION'); self.assertEqual(out['allocation_plan']['optimality'],'PROVEN_FOR_DECLARED_UTILITY'); self.assertEqual(out['allocation_plan']['selected'],['affordable']); self.assertEqual(out['next']['id'],'affordable'); self.assertEqual([x['id'] for x in out['budgeted_successor_frontier']],['affordable']); rejected={x['candidate']:x['reasons'] for x in out['decision_explanation']['rejected']}; self.assertIn('not_budget_allocated',rejected['expensive'])
 def test_large_budget_frontier_labels_heuristic_fallback(self):
  candidates=[{'id':f'c{i:02d}',**{**BASE_METRICS,'gain':i+1},'resource_cost':1} for i in range(19)]
  out=compile_orchestration('seed',candidates,budget={'total_cost':3,'max_branches':3}); self.assertEqual(out['allocation_plan']['solver'],'GREEDY_DENSITY_FALLBACK'); self.assertEqual(out['allocation_plan']['optimality'],'HEURISTIC_NOT_PROVEN'); self.assertEqual(out['allocation_plan']['selected'],['c16','c17','c18']); self.assertEqual(out['next']['id'],'c18')
