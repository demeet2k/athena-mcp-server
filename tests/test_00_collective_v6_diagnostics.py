...
class CollectiveRuntimeV6EarlyDiagnostics(unittest.TestCase):
    def test_named_v6_organs_first(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.db') as f:
            ...
            def run(name,fn):
                try:
                    fn(); print(f'::notice title=V6_{name}::PASS')
                except Exception as e:
                    failures.append((name,e)); print(f'::error title=V6_{name}::{type(e).__name__}: {e}')
            ...
            def cl():
                c=srv.call_tool('athena_claim_register',{'claim_key':'D','statement':'diag'});srv.call_tool('athena_claim_witness',{'claim_id':c['claim_id'],'kind':'REPLICATION','result':'SUPPORTS','independence_key':'r1'});srv.call_tool('athena_claim_state',{'claim_id':c['claim_id']})
            run('REPLICATION',cl)
            srv.store.close()
