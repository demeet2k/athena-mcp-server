...
class ReleaseDistributionTests(unittest.TestCase):
    ...
    def test_release_identity_matches_current_package_and_two_plane_runtime(self):
        m=self.manifest
        self.assertEqual(m['schema'],'ATHENA.RELEASE.DISTRIBUTION.3')
        self.assertEqual(m['version'],self.project['version']);self.assertEqual(m['version'],'3.2.0');self.assertEqual(m['tag'],'v3.2.0')
        ...
        self.assertEqual(m['runtime']['manifest'],EFFECTIVE_UNIFIED_MANIFEST_VERSION)
        self.assertEqual(m['runtime']['scientific_manifest'],UNIFIED_MANIFEST_VERSION);self.assertEqual(UNIFIED_MANIFEST_VERSION,'ATHENA.RUNTIME.UNIFIED.9')
        self.assertEqual(m['runtime']['effective_manifest'],EFFECTIVE_UNIFIED_MANIFEST_VERSION);self.assertEqual(EFFECTIVE_UNIFIED_MANIFEST_VERSION,'ATHENA.RUNTIME.UNIFIED.10')
        self.assertEqual(m['runtime']['organ_inventory'],COORDINATION_INVENTORY_VERSION);self.assertEqual(COORDINATION_INVENTORY_VERSION,'ATHENA.ORGAN.INVENTORY.1.1')
        self.assertEqual(m['runtime']['architecture_drift'],'ATHENA.ARCHITECTURE.DRIFT.1')
        self.assertEqual(m['runtime']['party_reward_current'],PARTY_REWARD_VERSION);self.assertEqual(PARTY_REWARD_VERSION,'PARTY.REWARD.PROVENANCE.3.2')
        self.assertEqual(m['runtime']['collective_frontier'],'COLLECTIVE_ROBUST_V13');self.assertEqual(m['runtime']['promotion'],PROMOTION_VERSION);self.assertEqual(m['runtime']['trusted_verifier'],GITHUB_PROMOTION_VERIFIER_VERSION)
    ...
    def test_required_assets_match_current_distribution_not_historical_carrier(self):
        assets=set(self.manifest['release']['required_assets']);self.assertEqual(assets,{'athena_canonical_mcp-3.2.0-py3-none-any.whl','promotion-receipt.json','release-manifest.json','release-attestation.json','SHA256SUMS'});joined=' '.join(sorted(assets));self.assertNotIn('kc144-core-registries.tar.xz',joined);self.assertNotIn('hub_server',json.dumps(self.manifest))

    def test_v13_and_coordination_surface_are_explicit_in_distribution(self):
        ...
        for uri in ('athena://manifest','athena://runtime/unified-manifest','athena://promotion','athena://collective/v13','athena://cohesion/v1','athena://party-coordination/v1','athena://architecture/inventory','athena://architecture/drift'):self.assertIn(uri,resources)

    def test_release_workflow_is_pr_testable_but_publish_is_manual_master_only(self):
        w=self.workflow
        for fragment in ('pull_request:','workflow_dispatch:','package-readiness:','promotion-qualification:',"if: github.event_name == 'workflow_dispatch' && github.ref == 'refs/heads/master'",'RELEASE_HEAD: ${{ github.event.pull_request.head.sha || github.sha }}','git rev-parse origin/master','scripts/qualify_github_head.py','actions/download-artifact@v5','actions/upload-artifact@v4','release-candidate-v3.2.0-${{ env.RELEASE_HEAD }}','promotion-receipt-${{ env.RELEASE_HEAD }}','python -m pip wheel --no-deps . -w dist','athena_mcp.server','ATHENA.RUNTIME.UNIFIED.9','ATHENA.RUNTIME.UNIFIED.10','ATHENA.ORGAN.INVENTORY.1.1','ATHENA.ARCHITECTURE.DRIFT.1','COLLECTIVE_ROBUST_V13','athena://architecture/drift','gh release create','--verify-tag','refs/tags/$TAG','TAG_TARGET','(cd dist && sha256sum -c SHA256SUMS)','release-attestation.json','trusted_promotion'):self.assertIn(fragment,w)
        self.assertNotIn("branches:\n      - 'agent/aor-collective-unified'",w);self.assertNotRegex(w,re.compile(r'(?m)^\s*push:\s*$'));self.assertNotRegex(w,re.compile(r'continue-on-error:\s*true'));self.assertNotIn('athena_mcp.hub_server',w);self.assertNotIn('kc144-core-registries.tar.xz',w)
    ...
    def test_package_readiness_binds_trusted_receipt_and_architecture_to_same_release_head(self):
        w=self.workflow
        for fragment in ('test "$(git rev-parse HEAD)" = "$RELEASE_HEAD"',"assert promotion['git_head']==os.environ['RELEASE_HEAD']","assert promotion['promotion']['status']=='QUALIFIED'","assert promotion['replay']['match'] is True","'release_commit':os.environ['RELEASE_HEAD']","'promrun':promotion['promotion']['run_id']","'verification_ref':promotion['verification_ref']","'receipt_sha256':sha(promo)","assert att['release_commit']==os.environ['RELEASE_HEAD']",'test "$TAG_TARGET" = "$RELEASE_HEAD"',"assert base['artifact']=='ATHENA.RUNTIME.UNIFIED.9'","assert effective['artifact']=='ATHENA.RUNTIME.UNIFIED.10'","assert drift['latest']['status']=='PASS'"):self.assertIn(fragment,w)
        self.assertNotIn("promotion['git_head']==os.environ['GITHUB_SHA']",w);self.assertNotIn("'release_commit':os.environ['GITHUB_SHA']",w)

    def test_release_notes_preserve_scientific_effective_and_trust_claim_ceilings(self):
        n=self.notes
        for phrase in ('ATHENA 3.2.0','Collective V13','UNIFIED.9','UNIFIED.10','ORGAN_INVENTORY.1.1','ARCHITECTURE.DRIFT.1','Party V3.2','GITHUB_PROMOTION_VERIFIER.1','QMC_CONTINUOUS_HYPERPOSTERIOR != EXACT_CONTINUOUS_HYPERPARAMETER_BAYES','FITC_INDUCING_GP != FULL_GP_POSTERIOR_OR_VARIATIONAL_OPTIMUM','STAGE2_PSEUDO_OUTCOME_PRESERVES_OBSERVED_A1_L1_BEFORE_STAGE1_INTERVENTION','ELLIPSOIDAL_GAUSSIAN_ROBUST_PLAN != GENERAL_DISTRIBUTIONALLY_ROBUST_OPTIMIZATION','caller-bound readiness','one coherent exact-head Actions run/check-suite','This release certifies repository/package/distribution state. It is not a production deployment'):self.assertIn(phrase,n)
    ...
    def test_authority_boundaries_do_not_collapse_distribution_into_truth_admin_or_coordination_authority(self):
        ...
        for phrase in ('not deployment','not empirical truth','do not become y1 authority','coordination claims do not become y1 semantic authority','do not become assignment, execution, result-truth, or global-xp authority','not github administrative hardening','does not authorize production','not evidence for v3.2.0 bytes'):self.assertIn(phrase,boundaries)
