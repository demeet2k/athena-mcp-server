...
def _coordinates():
    ...
    proof = {
        ...
        "immutable_locator_hash": LOCATOR,
...
def test_complete_seventeen_role_path_settles_rollback_without_side_effects():
    result = _closure()
...
def test_complete_w24_return_bundle_is_required():
    gate, records = _coordinates()
    values = list(deepcopy(records))
    values[12]["observed_state"] = "UNKNOWN"
    values = _resign(
        values,
        12,
        "rollback_observer",
...
def test_persistence_requires_at_least_one_year_retention():
    gate, records = _coordinates()
    values = list(deepcopy(records))
    values[13]["retention_until"] = "2026-07-28T01:18:00Z"
    values = _resign(
        values,
        13,
        "return_persistence_writer",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("immutable_locator_hash", "sha256:" + "5" * 64),
...
def test_settlement_cannot_precede_persistence_observation():
    gate, records = _coordinates()
    values = list(deepcopy(records))
    values[15]["issued_at"] = "2026-07-27T01:18:30Z"
    values = _resign(
        values,
        15,
        "promotion_settlement_issuer",
...
@pytest.mark.parametrize(
    ("index", "field"),
    [
        (13, "w24_return_bundle_digest"),
        (14, "persistence_proof_digest"),
        (15, "persistence_observation_digest"),
        (16, "promotion_settlement_digest"),
    ],
)
...
def test_duplicate_json_members_rejected():
    ...
    serialized[13] = serialized[13].replace(
        '"occurrence_id":',
...
def test_contract_digest_phase_and_successor_are_exact():
    ...
    assert snapshot["settlement_contract"]["total_cross_wave_roles"] == 17
    assert snapshot["successor"].startswith("KC144.XNAV.W26::")
