...
class PartyCoordinationTests(unittest.TestCase):
    ...
    def test_tools_and_resource_register_without_displacing_message_board(self):
        ...
        for name in [
            'athena_message_board','athena_party_form','athena_party_join','athena_party_state',
            'athena_party_list','athena_party_observe','athena_transport_aor_to_collective'
        ]:
    ...
    def test_acknowledged_board_communication_gates_bonus_and_big3(self):
        self.seed_party()
        early=self.tool(self.a,'athena_party_observe',{
            'observation_id':'OBS.EARLY','party_id':'PARTY.TEST','observer':'meta','base_xp':100,
            'results':[
                {'goal_id':'goal.analysis','agent_id':'alpha','witness_ref':'result://analysis'},
                {'goal_id':'goal.code','agent_id':'beta','witness_ref':'result://code'},
            ],
            'witness_ref':'outcome://early',
        })
        ...
        posted=self.board_a.post(
            ...
            message='Analysis contract is stable; implementation can integrate.'
        )
        ...
        award=self.tool(self.a,'athena_party_observe',{
            ...
            'results':[
                {'goal_id':'goal.analysis','agent_id':'alpha','witness_ref':'result://analysis-v2'},
                {'goal_id':'goal.code','agent_id':'beta','witness_ref':'result://code-v2'},
            ],
            'witness_ref':'outcome://success',
        })
        ...
        duplicate=self.tool(self.a,'athena_party_observe',{
            ...
            'results':[
                {'goal_id':'goal.analysis','agent_id':'alpha','witness_ref':'result://analysis-v3'},
                {'goal_id':'goal.code','agent_id':'beta','witness_ref':'result://code-v3'},
            ],
            'witness_ref':'outcome://success',
        })
    ...
    def test_result_must_cover_assigned_goals_from_two_party_members(self):
        self.seed_party()
        posted=self.board_a.post(agent_id='alpha',recipients=['beta'],message_kind='UPDATE',message='handoff')
