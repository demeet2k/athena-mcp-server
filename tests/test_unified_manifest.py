...
class UnifiedManifestTests(unittest.TestCase):
    ...
    def test_live_manifest_reports_v12_promotion2_and_compatibility(self):
        manifest=self.tool('athena_runtime_manifest')
        self.assertEqual(manifest['artifact'],'ATHENA.RUNTIME.UNIFIED.7')
        for compat in ('ATHENA.RUNTIME.UNIFIED.1','ATHENA.RUNTIME.UNIFIED.2','ATHENA.RUNTIME.UNIFIED.3','ATHENA.RUNTIME.UNIFIED.4','ATHENA.RUNTIME.UNIFIED.5','ATHENA.RUNTIME.UNIFIED.6'):self.assertIn(compat,manifest['artifact_compat'])
        self.assertEqual(manifest['runtime_class'],'Server')
        for layer in ['COLLECTIVE_MEMORY_V2','COLLECTIVE_LEARNING_V3','COLLECTIVE_ECOLOGY_V4','COLLECTIVE_SCIENCE_V5','COLLECTIVE_DISCOVERY_V6','COLLECTIVE_DUAL_CONTROL_V7','COLLECTIVE_BELIEF_V8','COLLECTIVE_INFERENCE_V9','COLLECTIVE_PROBABILISTIC_V10','COLLECTIVE_ADAPTIVE_V11','COLLECTIVE_JOINT_V12','AOR_DECISION_CORTEX','AOR.3','AUTHORITY_Y1','RAG.1','HUG.ABI.1','GAP.1','FIELD.1','AORCOLL.TRANSPORT.1','CYCLE.1','SCHEMA.2','OMEGA.1','RECON.1','SELFTEST.1','SURFACE.2','COMPOSITION.2','PROMOTION.2']:
            self.assertIn(layer,manifest['layers'])
        joined='\n'.join(manifest['invariants'])
        for phrase in ['UNKNOWN != 0','pheromone/reuse/popularity != evidence','reachability/navigation closure != logical or causal proof','semantic VID CAS != Git HEAD CAS != topology version CAS','caller-supplied CI/smoke attestation != externally verified promotion qualification','PROMOTION.2 ATTESTED_READY != QUALIFIED','BELIEF_POSTERIOR != CANONICAL_TRUTH','GAUSSIAN_LINEAR_POSTERIOR != GENERAL_CONTINUOUS_BAYES','V10 FIXED_KERNEL_GP != GENERAL_WORLD_TRUTH','V11 marginal-likelihood optimum != true kernel','V11 supplied-DAG latent projection != data-discovered PAG','V11 stacked TMLE != super-learner theorem','V11 finite-model BAPOMDP != general Bayes-adaptive control','V12 finite-grid GP hyperposterior != continuous hyperparameter Bayes','V12 bounded PAG candidate != FCI/RFCI PAG theorem','V12 two-timepoint parametric g-formula != longitudinal TMLE or identification proof','V12 independent-Gaussian chance certificate != distribution-free resource guarantee','athena_claim_* = Y1 canonical authority']:
        ...
        self.assertIn('ATTESTED_READY',manifest['promotion']);self.assertIn('QUALIFIED',manifest['promotion'])
    ...
    def test_maxdev_law_contains_v12_and_promotion2_boundaries(self):
        law=self.tool('athena_maxdev_law')['text']
        for phrase in ['RECONSTRUCT through RECONRUN + canonical OMEGA','AOR ranks eligible comparable candidates','V11 GP hyperfit is DESIGN_ONLY unless apply=true','V12 GP hyperposterior','V12 GP BMA prediction','V12 subset-GP','V12 BMA GP EVSI','V12 PAG candidate','V12 longitudinal g-formula','V12 chance-constrained resource selection','never feed predictions/beliefs/simulations/plans/designs back as observations or Y authority','PROMOTE through PROMOTION.2','ATTESTED_READY','QUALIFIED requires a host-internal trusted verifier receipt']:
            self.assertIn(phrase,law)

    def test_manifest_resources_and_surface2_include_v12(self):
        ...
        for uri in ['athena://manifest','athena://runtime/unified-manifest','athena://runtime/maxdev','athena://collective/v5','athena://collective/v6','athena://collective/v7','athena://collective/v8','athena://collective/v9','athena://collective/v10','athena://collective/v11','athena://collective/v12']:
            self.assertIn(uri,uris)
        payload=self.read_json('athena://runtime/unified-manifest');canonical=self.read_json('athena://manifest')
        self.assertEqual(payload['artifact'],'ATHENA.RUNTIME.UNIFIED.7');self.assertEqual(canonical['artifact'],'ATHENA.RUNTIME.UNIFIED.7');self.assertEqual(canonical['layers'],payload['layers'])
        audit=self.tool('athena_surface_audit',{'run_probes':True})
        for group in ('manifest','collective_v4_v5_v6','collective_v7','collective_v8','collective_v9','collective_v10','collective_v11','collective_v12','promotion'):self.assertEqual(audit['groups'][group]['status'],'PASS')
