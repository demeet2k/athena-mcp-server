...
class UnifiedManifestTests(unittest.TestCase):
    ...
    def test_live_manifest_reports_v6_single_server_full_layers_and_compatibility(self):
        manifest=self.tool('athena_runtime_manifest')
        self.assertEqual(manifest['artifact'],'ATHENA.RUNTIME.UNIFIED.2');self.assertIn('ATHENA.RUNTIME.UNIFIED.1',manifest['artifact_compat']);self.assertEqual(manifest['runtime_class'],'Server')
        for layer in ['COLLECTIVE_MEMORY_V2','COLLECTIVE_LEARNING_V3','COLLECTIVE_ECOLOGY_V4','COLLECTIVE_SCIENCE_V5','COLLECTIVE_DISCOVERY_V6','AOR_DECISION_CORTEX','AOR.3','AUTHORITY_Y1','RAG.1','HUG.ABI.1','GAP.1','FIELD.1','AORCOLL.TRANSPORT.1','CYCLE.1','SCHEMA.2','OMEGA.1','RECON.1','SELFTEST.1','SURFACE.2','COMPOSITION.2','PROMOTION.1']:
            self.assertIn(layer,manifest['layers'])
        joined='\n'.join(manifest['invariants'])
        for phrase in ['UNKNOWN != 0','pheromone/reuse/popularity != evidence','reachability/navigation closure != logical or causal proof','semantic VID CAS != Git HEAD CAS != topology version CAS','POSTERIOR != TRUTH','EIG != EVIDENCE','athena_claim_* = Y1 canonical authority']:
            self.assertIn(phrase,joined)
        self.assertIn('consensus/pheromone/reward are never typed authority or evidence',manifest['braid_law'])
        self.assertIn('athena_discovery_claim_*',manifest['claim_namespace_law'])
        unresolved={x['id']:x for x in manifest['unresolved']}
        self.assertEqual(unresolved['QHUG_SEMANTICS']['status'],'UNRESOLVED_UNLESS_REGISTERED_AND_WITNESSED');self.assertIn('directed reachability',unresolved['STRONGER_CLOSURE']['boundary']);self.assertEqual(unresolved['MODEL_TO_AUTHORITY_BRIDGE']['status'],'EXPLICIT_WITNESS_REQUIRED')
    ...
    def test_manifest_resources_and_canonical_alias_are_part_of_surface2(self):
        ...
        for uri in ['athena://manifest','athena://runtime/unified-manifest','athena://runtime/maxdev','athena://collective/v5','athena://collective/v6']:
            self.assertIn(uri,uris)
        payload=self.read_json('athena://runtime/unified-manifest');canonical=self.read_json('athena://manifest')
        self.assertEqual(payload['artifact'],'ATHENA.RUNTIME.UNIFIED.2');self.assertEqual(canonical['artifact'],'ATHENA.RUNTIME.UNIFIED.2');self.assertEqual(canonical['layers'],payload['layers'])
        audit=self.tool('athena_surface_audit',{'run_probes':True});self.assertEqual(audit['groups']['manifest']['status'],'PASS');self.assertEqual(audit['groups']['collective_v4_v5_v6']['status'],'PASS')
