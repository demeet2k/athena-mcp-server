...
class UnifiedManifestTests(unittest.TestCase):
    ...
    def test_live_manifest_reports_v16_v15_deployment_github_verifier_and_compatibility(self):
        manifest=self.tool('athena_runtime_manifest')
        self.assertEqual(manifest['artifact'],'ATHENA.RUNTIME.UNIFIED.12')
        for compat in ('ATHENA.RUNTIME.UNIFIED.1','ATHENA.RUNTIME.UNIFIED.2','ATHENA.RUNTIME.UNIFIED.3','ATHENA.RUNTIME.UNIFIED.4','ATHENA.RUNTIME.UNIFIED.5','ATHENA.RUNTIME.UNIFIED.6','ATHENA.RUNTIME.UNIFIED.7','ATHENA.RUNTIME.UNIFIED.8','ATHENA.RUNTIME.UNIFIED.9','ATHENA.RUNTIME.UNIFIED.10','ATHENA.RUNTIME.UNIFIED.11'):self.assertIn(compat,manifest['artifact_compat'])
        self.assertEqual(manifest['runtime_class'],'Server')
        for layer in ['COLLECTIVE_MEMORY_V2','COLLECTIVE_LEARNING_V3','COLLECTIVE_ECOLOGY_V4','COLLECTIVE_SCIENCE_V5','COLLECTIVE_DISCOVERY_V6','COLLECTIVE_DUAL_CONTROL_V7','COLLECTIVE_BELIEF_V8','COLLECTIVE_INFERENCE_V9','COLLECTIVE_PROBABILISTIC_V10','COLLECTIVE_ADAPTIVE_V11','COLLECTIVE_JOINT_V12','COLLECTIVE_ROBUST_V13','COLLECTIVE_SYNTHESIS_V14','COLLECTIVE_CALIBRATED_V15','COLLECTIVE_GENERALIZED_V16','AOR_DECISION_CORTEX','AOR.3','AUTHORITY_Y1','RAG.1','HUG.ABI.1','GAP.1','FIELD.1','AORCOLL.TRANSPORT.1','CYCLE.1','SCHEMA.2','OMEGA.1','RECON.1','SELFTEST.1','SURFACE.2','COMPOSITION.2','PROMOTION.2','GITHUB_PROMOTION_VERIFIER.1']:
            self.assertIn(layer,manifest['layers'])
        self.assertLess(manifest['layers'].index('COLLECTIVE_CALIBRATED_V15'),manifest['layers'].index('COLLECTIVE_GENERALIZED_V16'))
        self.assertLess(manifest['layers'].index('COLLECTIVE_GENERALIZED_V16'),manifest['layers'].index('GITHUB_PROMOTION_VERIFIER.1'))
        ...
        self.assertIn('collective_v16',manifest['organs'])
        self.assertEqual(manifest['collective_generalized']['version'],'COLLECTIVE_RUNTIME_V16')
        self.assertEqual(manifest['collective_generalized']['coordinate'],'COLLECTIVE_GENERALIZED=<OG,MH,GM,EF,NR,L>')
        self.assertEqual(manifest['collective_generalized']['authority'],'MODEL_SCIENCE_TWIN_AND_PLAN_ONLY')
        self.assertEqual(manifest['collective_generalized']['scope']['multistage_dr_max_treatments'],6)
        self.assertEqual(manifest['collective_generalized']['scope']['coupled_policy_max_horizon'],6)
        self.assertIn('COLLECTIVE(V1-V16)',manifest['cycle'])
        self.assertIn('COLLECTIVE_GENERALIZED_V16',manifest['navigation'])
        ...
        for uid in ('GENERAL_BELIEF_CONTROL','FORMAL_CAUSAL_DISCOVERY','LONGITUDINAL_CAUSAL_POLICY','STOCHASTIC_RESOURCE_CONTROL'):
            self.assertIn('v15_boundary',unresolved[uid]);self.assertIn('v16_boundary',unresolved[uid])
    ...
    def test_maxdev_law_contains_v16_v15_and_trusted_github_promotion_boundaries(self):
        law=self.tool('athena_maxdev_law')['text']
        for phrase in ['RECONSTRUCT through RECONRUN + canonical OMEGA','V14 SYNTHESIS LAW','V15 CALIBRATION LAW','pool identical structural-support coordinates','out-of-fold reliability','stage-1 policies may use baseline only','reject unknown Gaussian/state/action coordinates','geometric nearest witness','when a declared radius contains no witness, return no local certificate','cross-fit two-timepoint sequential TMLE/AIPW','multivariate-Gaussian conditioning','transport approximation error','rectangular total-variation ambiguity','zero selected tests are not proof','caller-bound packets stop at ATTESTED_READY','prefer athena_promotion_verify_github','one coherent exact-head Actions run/check-suite','Never splice checks across runs/suites','V16 GENERALIZED LAW','ordered DAG posterior is exact only within the caller-declared topological order','multistage longitudinal DR supports at most six binary treatment stages','finite Gaussian-mixture update is exact only for the supplied mixture','CV residual quantiles are not distribution-free coverage certificates','coupled model-family robust policy evaluates supplied policies']:
            self.assertIn(phrase,law)

    def test_manifest_resources_surface2_promotion_and_deployment_include_v16(self):
        names={x['name'] for x in self.rpc('tools/list')['result']['tools']};uris={x['uri'] for x in self.rpc('resources/list')['result']['resources']}
        for name in ('athena_runtime_manifest','athena_maxdev_law','athena_promotion_evaluate','athena_promotion_verify_github','athena_structural_reliability_calibrate','athena_longitudinal_tmle_crossfit','athena_sequential_dr_policy_crossfit','athena_joint_gaussian_update','athena_joint_gaussian_control','athena_approx_error_transport','athena_multistage_tv_dro_plan','athena_ordered_dag_posterior','athena_longitudinal_dr_multistage_crossfit','athena_gaussian_mixture_update','athena_approx_error_field','athena_coupled_model_robust_policy','athena_deployment_manifest','athena_deployment_validate'):
            self.assertIn(name,names)
        for uri in ['athena://manifest','athena://runtime/unified-manifest','athena://runtime/maxdev','athena://promotion','athena://collective/v13','athena://collective/v14','athena://collective/v15','athena://collective/v16','athena://deployment','athena://deployment/evidence','athena://deployment/rollout','athena://deployment/security']:
            self.assertIn(uri,uris)
        tool_manifest=self.tool('athena_runtime_manifest')
        payload=self.read_json('athena://runtime/unified-manifest');canonical=self.read_json('athena://manifest');promotion=self.read_json('athena://promotion');v15=self.read_json('athena://collective/v15');v16=self.read_json('athena://collective/v16')
        for current in (tool_manifest,payload,canonical):
            self.assertEqual(current['artifact'],'ATHENA.RUNTIME.UNIFIED.12')
            ...
            self.assertEqual(current['collective_generalized'],tool_manifest['collective_generalized'])
        ...
        self.assertEqual(v16['runtime']['version'],'COLLECTIVE_RUNTIME_V16');self.assertEqual(v16['runtime']['coordinate'],'COLLECTIVE_GENERALIZED=<OG,MH,GM,EF,NR,L>');self.assertIn('Y1 authority',v16['boundary']);self.assertIn('trusted promotion',v16['boundary'])
        ...
        for group in ('manifest','collective_v13','collective_v14','collective_v15','collective_v16','promotion'):self.assertEqual(audit['groups'][group]['status'],'PASS')
