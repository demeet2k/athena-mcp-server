...
class UnifiedManifestTests(unittest.TestCase):
    ...
    def test_live_manifest_reports_v10_single_server_layers_and_compatibility(self):
        manifest=self.tool('athena_runtime_manifest')
        self.assertEqual(manifest['artifact'],'ATHENA.RUNTIME.UNIFIED.5')
        for compat in ('ATHENA.RUNTIME.UNIFIED.1','ATHENA.RUNTIME.UNIFIED.2','ATHENA.RUNTIME.UNIFIED.3','ATHENA.RUNTIME.UNIFIED.4'):self.assertIn(compat,manifest['artifact_compat'])
        self.assertEqual(manifest['runtime_class'],'Server')
        for layer in ['COLLECTIVE_MEMORY_V2','COLLECTIVE_LEARNING_V3','COLLECTIVE_ECOLOGY_V4','COLLECTIVE_SCIENCE_V5','COLLECTIVE_DISCOVERY_V6','COLLECTIVE_DUAL_CONTROL_V7','COLLECTIVE_BELIEF_V8','COLLECTIVE_INFERENCE_V9','COLLECTIVE_PROBABILISTIC_V10','AOR_DECISION_CORTEX','AOR.3','AUTHORITY_Y1','RAG.1','HUG.ABI.1','GAP.1','FIELD.1','AORCOLL.TRANSPORT.1','CYCLE.1','SCHEMA.2','OMEGA.1','RECON.1','SELFTEST.1','SURFACE.2','COMPOSITION.2','PROMOTION.1']:
            self.assertIn(layer,manifest['layers'])
        joined='\n'.join(manifest['invariants'])
        for phrase in ['UNKNOWN != 0','pheromone/reuse/popularity != evidence','reachability/navigation closure != logical or causal proof','semantic VID CAS != Git HEAD CAS != topology version CAS','BELIEF_POSTERIOR != CANONICAL_TRUTH','GAUSSIAN_LINEAR_POSTERIOR != GENERAL_CONTINUOUS_BAYES','V10 FIXED_KERNEL_GP != GENERAL_WORLD_TRUTH','V10 BOUNDED_PC_STABLE != FCI_OR_HIDDEN_CONFOUNDER_DISCOVERY','V10 TMLE estimate != identification proof','finite POMDP certificate != infinite-horizon','athena_claim_* = Y1 canonical authority']:
    ...
    def test_maxdev_law_contains_operational_science_and_v10_boundaries(self):
        law=self.tool('athena_maxdev_law')['text']
        for phrase in ['RECONSTRUCT through RECONRUN + canonical OMEGA','pheromone/reuse/consensus never become evidence or Y authority','FIELD assembles real residual work','AOR ranks eligible comparable candidates','V10 GP state updates only through explicit athena_gp_observe','V10 PC-stable','V10 binary TMLE','V10 finite POMDP','V10 learned evidence-dependence','never feed predictions/beliefs/simulations/plans/designs back as observations or Y authority','PROMOTE only the exact head']:
    ...
    def test_manifest_resources_and_canonical_alias_are_part_of_surface2(self):
        ...
        for uri in ['athena://manifest','athena://runtime/unified-manifest','athena://runtime/maxdev','athena://collective/v5','athena://collective/v6','athena://collective/v7','athena://collective/v8','athena://collective/v9','athena://collective/v10']:
            self.assertIn(uri,uris)
        payload=self.read_json('athena://runtime/unified-manifest');canonical=self.read_json('athena://manifest')
        self.assertEqual(payload['artifact'],'ATHENA.RUNTIME.UNIFIED.5');self.assertEqual(canonical['artifact'],'ATHENA.RUNTIME.UNIFIED.5');self.assertEqual(canonical['layers'],payload['layers'])
        audit=self.tool('athena_surface_audit',{'run_probes':True})
        for group in ('manifest','collective_v4_v5_v6','collective_v7','collective_v8','collective_v9','collective_v10'):self.assertEqual(audit['groups'][group]['status'],'PASS')
