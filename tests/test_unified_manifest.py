...
class UnifiedManifestTests(unittest.TestCase):
    ...
    def test_live_manifest_reports_v9_single_server_layers_and_compatibility(self):
        manifest=self.tool('athena_runtime_manifest')
        self.assertEqual(manifest['artifact'],'ATHENA.RUNTIME.UNIFIED.4')
        for compat in ('ATHENA.RUNTIME.UNIFIED.1','ATHENA.RUNTIME.UNIFIED.2','ATHENA.RUNTIME.UNIFIED.3'):self.assertIn(compat,manifest['artifact_compat'])
        self.assertEqual(manifest['runtime_class'],'Server')
        for layer in ['COLLECTIVE_MEMORY_V2','COLLECTIVE_LEARNING_V3','COLLECTIVE_ECOLOGY_V4','COLLECTIVE_SCIENCE_V5','COLLECTIVE_DISCOVERY_V6','COLLECTIVE_DUAL_CONTROL_V7','COLLECTIVE_BELIEF_V8','COLLECTIVE_INFERENCE_V9','AOR_DECISION_CORTEX','AOR.3','AUTHORITY_Y1','RAG.1','HUG.ABI.1','GAP.1','FIELD.1','AORCOLL.TRANSPORT.1','CYCLE.1','SCHEMA.2','OMEGA.1','RECON.1','SELFTEST.1','SURFACE.2','COMPOSITION.2','PROMOTION.1']:
            self.assertIn(layer,manifest['layers'])
        joined='\n'.join(manifest['invariants'])
        for phrase in ['UNKNOWN != 0','pheromone/reuse/popularity != evidence','reachability/navigation closure != logical or causal proof','semantic VID CAS != Git HEAD CAS != topology version CAS','POSTERIOR != TRUTH','EIG != EVIDENCE','V7 uncertainty decomposition proxies != physical decomposition','BELIEF_POSTERIOR != CANONICAL_TRUTH','GAUSSIAN_LINEAR_POSTERIOR != GENERAL_CONTINUOUS_BAYES','AIPW estimate != identification proof','heuristic partial graph != FCI/PAG/CPDAG theorem','athena_claim_* = Y1 canonical authority']:
        ...
        self.assertIn('directed reachability',unresolved['STRONGER_CLOSURE']['boundary'])
    ...
    def test_manifest_resources_and_canonical_alias_are_part_of_surface2(self):
        ...
        for uri in ['athena://manifest','athena://runtime/unified-manifest','athena://runtime/maxdev','athena://collective/v5','athena://collective/v6','athena://collective/v7','athena://collective/v8','athena://collective/v9']:
            self.assertIn(uri,uris)
        payload=self.read_json('athena://runtime/unified-manifest');canonical=self.read_json('athena://manifest')
        self.assertEqual(payload['artifact'],'ATHENA.RUNTIME.UNIFIED.4');self.assertEqual(canonical['artifact'],'ATHENA.RUNTIME.UNIFIED.4');self.assertEqual(canonical['layers'],payload['layers'])
        audit=self.tool('athena_surface_audit',{'run_probes':True});self.assertEqual(audit['groups']['manifest']['status'],'PASS');self.assertEqual(audit['groups']['collective_v4_v5_v6']['status'],'PASS');self.assertEqual(audit['groups']['collective_v7']['status'],'PASS');self.assertEqual(audit['groups']['collective_v8']['status'],'PASS');self.assertEqual(audit['groups']['collective_v9']['status'],'PASS')
