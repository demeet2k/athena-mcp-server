...
class UnifiedManifestTests(unittest.TestCase):
    ...
    def test_live_manifest_reports_v15_deployment_github_verifier_and_compatibility(self):
        ...
        for phrase in ['UNKNOWN != 0','one coherent host-bound GitHub Actions run/check-suite','checks from different suites/runs are never spliced','trusted GitHub repository/API/run context comes from host environment','BELIEF_POSTERIOR != CANONICAL_TRUTH','FINITE_FACTOR_PRODUCT_BELIEF != FULL_JOINT_POSTERIOR','OUT_OF_FOLD_ISOTONIC_RELIABILITY != CAUSAL_GRAPH_POSTERIOR','CROSS_FITTED_TWO_TIMEPOINT_TMLE != GENERAL_LONGITUDINAL_TMLE_THEOREM','CROSS_FITTED_SEQUENTIAL_DR != GENERAL_OFF_POLICY_CAUSAL_VALUE','LINEAR_GAUSSIAN_UPDATE != GENERAL_CONTINUOUS_JOINT_BAYES','GAUSSIAN_LINEAR_CONTROL != GENERAL_BELIEF_MDP','DECLARED_LIPSCHITZ_ERROR_ENVELOPE != EMPIRICAL_GLOBAL_ERROR_TRUTH','RECTANGULAR_TV_ROBUST_MDP != GENERAL_MULTISTAGE_DRO','athena_claim_* = Y1 canonical authority']:
    ...
    def test_maxdev_law_contains_v15_and_trusted_github_promotion_boundaries(self):
        ...
        for phrase in ['RECONSTRUCT through RECONRUN + canonical OMEGA','V14 SYNTHESIS LAW','V15 CALIBRATION LAW','out-of-fold reliability','cross-fit two-timepoint sequential TMLE/AIPW','multivariate-Gaussian conditioning','transport approximation error','rectangular total-variation ambiguity','caller-bound packets stop at ATTESTED_READY','prefer athena_promotion_verify_github','one coherent exact-head Actions run/check-suite','Never splice checks across runs/suites']:
    ...
    def test_manifest_resources_surface2_promotion_and_deployment_include_v15(self):
        ...
        self.assertEqual(payload['artifact'],'ATHENA.RUNTIME.UNIFIED.11');self.assertEqual(canonical['artifact'],'ATHENA.RUNTIME.UNIFIED.11');self.assertEqual(canonical['layers'],payload['layers'])
