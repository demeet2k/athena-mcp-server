...
class UnifiedManifestTests(unittest.TestCase):
    ...
    def test_live_manifest_reports_v12_promotion2_github_verifier_and_compatibility(self):
    ...
    def test_maxdev_law_contains_v12_and_trusted_github_promotion_boundaries(self):
        law=self.tool('athena_maxdev_law')['text']
        for phrase in ['RECONSTRUCT through RECONRUN + canonical OMEGA','AOR ranks eligible comparable candidates','V11 GP hyperfit is DESIGN_ONLY unless apply=true','V12 GP hyperposterior','V12 GP BMA prediction','V12 subset-GP','V12 BMA GP EVSI','V12 PAG candidate','V12 longitudinal g-formula','V12 chance-constrained resource selection','caller-bound packets stop at ATTESTED_READY','prefer athena_promotion_verify_github','one coherent exact-head Actions run/check-suite','Never splice checks across runs/suites','caller-selected trust roots']:
            self.assertIn(phrase,law)

    def test_manifest_resources_surface2_and_promotion_include_v12_and_verifier(self):
        ...
        for uri in ['athena://manifest','athena://runtime/unified-manifest','athena://runtime/maxdev','athena://promotion','athena://collective/v5','athena://collective/v6','athena://collective/v7','athena://collective/v8','athena://collective/v9','athena://collective/v10','athena://collective/v11','athena://collective/v12']:
