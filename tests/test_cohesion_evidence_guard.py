    ...
    def test_public_resource_exposes_evidence_and_duplicate_guard_membranes(self):
        with tempfile.NamedTemporaryFile(suffix=".db") as db:
            ...
            try:
                ...
                self.assertIn("athena_cohesion_duplicate_guard", payload["tools"])
                self.assertTrue(any("PARTIAL_MATCHED_SUBSET" in law for law in payload["laws"]))
                self.assertTrue(any("PARTITION_PROOF != CLEAR_EXACT_WORK_IDENTITY" in law for law in payload["laws"]))
            finally:
                server.store.close()
