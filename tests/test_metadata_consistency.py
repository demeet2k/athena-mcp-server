...
class MetadataConsistencyTests(unittest.TestCase):
    def test_package_and_server_versions_match(self):
        ...
        self.assertEqual(project['version'],'2.6.0')

    def test_v7_is_exposed_by_mcp(self):
        with tempfile.NamedTemporaryFile(suffix='.db') as f:
            srv=Server(f.name)
            init=srv.handle({'jsonrpc':'2.0','id':1,'method':'initialize','params':{'protocolVersion':'2025-11-25'}})['result']
            self.assertEqual(init['serverInfo']['version'],'2.6.0')
            ...
            for uri in ('athena://collective/v4','athena://collective/v5','athena://collective/v6','athena://collective/v7'):
                self.assertIn(uri,uris)
            for i,uri in enumerate(('athena://collective/v5','athena://collective/v6','athena://collective/v7'),start=4):
