...
class MetadataConsistencyTests(unittest.TestCase):
    ...
    def test_v11_is_exposed_by_mcp(self):
        with tempfile.NamedTemporaryFile(suffix='.db') as f:
            ...
            for name in (
                ...
                'athena_gaussian_belief_register','athena_decision_evpi','athena_decision_evsi','athena_causal_aipw',
                'athena_gp_register','athena_gp_predict','athena_pc_stable_discover','athena_causal_tmle_binary','athena_pomdp_solve',
                'athena_gp_hyperfit','athena_gp_decision_evsi','athena_latent_project_admg','athena_causal_tmle_ensemble',
                'athena_sensitivity_rr_surface','athena_bapomdp_solve','athena_evidence_dependence_interval',
            ):
                self.assertIn(name,names)
            uris={x['uri'] for x in srv.handle({'jsonrpc':'2.0','id':3,'method':'resources/list'})['result']['resources']}
            for uri in ('athena://collective/v7','athena://collective/v8','athena://collective/v9','athena://collective/v10','athena://collective/v11'):
                self.assertIn(uri,uris)
            for i,uri in enumerate(('athena://collective/v9','athena://collective/v10','athena://collective/v11'),start=4):
