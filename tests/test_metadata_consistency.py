...
class MetadataConsistencyTests(unittest.TestCase):
    def test_package_and_server_versions_match_current_v16_candidate(self):
        root=Path(__file__).resolve().parents[1]
        project=tomllib.loads((root/'pyproject.toml').read_text())['project']
        self.assertEqual(project['version'],'3.5.0')
        ...
        for phrase in ('ordered-dag posterior','multistage cross-fitted longitudinal dr','gaussian-mixture belief','approximation-error fields','coupled finite-model robust policy evaluation'):
            self.assertIn(phrase,description)

    def test_v5_v16_deployment_and_claim_namespaces_are_exposed_without_collision(self):
        with tempfile.NamedTemporaryFile(suffix='.db') as f:
            ...
            v16=srv.handle({'jsonrpc':'2.0','id':6,'method':'resources/read','params':{'uri':'athena://collective/v16'}})['result']['contents'][0]['text']
            self.assertIn('COLLECTIVE_RUNTIME_V16',v16);self.assertIn('ORDER_CONSTRAINED_DAG_POSTERIOR',v16);self.assertIn('trusted promotion',v16)
            srv.store.close()
