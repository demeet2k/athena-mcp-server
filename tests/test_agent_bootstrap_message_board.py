...
class AgentBootstrapMessageBoardTests(unittest.TestCase):
    ...
    def _boot(runtime, agent_id, task, **coordination):
        coordination.setdefault("coordination_mode", "AUTO")
    ...
    def test_auto_claims_then_reuses_same_presence_without_head_churn(self):
        ...
        first = self._boot(runtime, "agent-a", "build parser", work_key="WK:PARSER", targets=["athena_mcp/parser.py"])
        ...
        second = self._boot(runtime, "agent-a", "build parser", work_key="WK:PARSER", targets=["athena_mcp/parser.py"])
        ...
        self.assertEqual(second["coordination"]["routing_digest"], first["coordination"]["routing_digest"])

    def test_second_agent_exact_duplicate_holds_without_second_claim(self):
        _, a, b, _ = self._fixture()
        self._boot(self._runtime(a), "agent-a", "build parser", work_key="WK:PARSER")
        duplicate = self._boot(self._runtime(b), "agent-b", "build parser", work_key="WK:PARSER")
        ...
        self.assertFalse((b / "runtime/message_board/v1/agents/agent-b.json").exists())
    ...
    def test_same_agent_different_work_holds_until_release(self):
        ...
        self.assertEqual(_run(a, "rev-parse", "HEAD"), head)
        self.assertEqual(switched["coordination"]["presence"]["claim_id"], first["coordination"]["presence"]["claim_id"])

    def test_explicit_replica_can_overlap_primary(self):
        _, a, b, _ = self._fixture()
        self._boot(self._runtime(a), "agent-a", "benchmark kernel", work_key="WK:KERNEL")
        replica = self._boot(
            self._runtime(b),
            "agent-b",
            "benchmark kernel",
        ...
        self.assertTrue(any(edge.get("intentional") for edge in replica["coordination"]["board"]["exact_overlaps"]))

    def test_replica_requires_explicit_reason(self):
        _, a, _, _ = self._fixture()
        with self.assertRaisesRegex(ValueError, "REPLICA requires replication_reason"):
            self._boot(self._runtime(a), "agent-a", "benchmark kernel", coordination_claim_mode="REPLICA")

    def test_fuzzy_similarity_is_warning_not_veto(self):
        _, a, b, _ = self._fixture()
        self._boot(self._runtime(a), "agent-a", "build agent bootstrap message board gate")
        second = self._boot(self._runtime(b), "agent-b", "build agent bootstrap message board integration")
        self.assertEqual(second["coordination"]["pre_dispatch"], "ALLOW", second)
        self.assertTrue(second["coordination"]["transition"].get("potential_overlaps"), second)

    def test_read_only_and_disabled_do_not_create_presence(self):
        _, a, b, _ = self._fixture()
        read_only = self._boot(self._runtime(a), "agent-a", "inspect parser", coordination_mode="READ_ONLY")
        self.assertEqual(read_only["coordination"]["pre_dispatch"], "ADVISORY_ONLY")
        self.assertFalse((a / "runtime/message_board/v1/agents/agent-a.json").exists())
        disabled = self._boot(self._runtime(b), "agent-b", "offline inspect", coordination_mode="DISABLED")
        self.assertEqual(disabled["coordination"]["pre_dispatch"], "DISABLED")
        self.assertEqual(disabled["status"], "BOOTSTRAPPED", disabled)
        self.assertFalse((b / "runtime/message_board/v1").exists())

    def test_auto_failed_shared_freshness_fails_closed(self):
        _, a, _, _ = self._fixture()
        _run(a, "remote", "set-url", "origin", str(a.parent / "missing.git"))
    ...
    def test_tool_default_observes_without_claim_until_work_identity_exists(self):
        _, a, b, _ = self._fixture()
        observed = self._runtime(a).call_tool(
            "athena_agent_bootstrap",
            {
                "agent_id": "observer-a",
                "task": "continue cold process",
                "fetch": False,
                "shared_remote_mode": "REQUIRED",
                "continuation_shared_remote_mode": "DISABLED",
            },
        )
        self.assertEqual(observed["coordination"]["pre_dispatch"], "ADVISORY_ONLY", observed)
        self.assertEqual(observed["coordination"]["requested_mode"], "AUTO")
        self.assertEqual(observed["coordination"]["auto_resolution"], "READ_ONLY_NO_EXPLICIT_WORK_ID")
        self.assertFalse((a / "runtime/message_board/v1/agents/observer-a.json").exists())

        claimed = self._runtime(b).call_tool(
            "athena_agent_bootstrap",
            {
                "agent_id": "worker-b",
                "task": "build parser",
        ...
        self.assertEqual(claimed["coordination"]["pre_dispatch"], "ALLOW", claimed)
        self.assertEqual(claimed["coordination"]["auto_resolution"], "CLAIM_EXPLICIT_WORK_ID")

    def test_direct_python_boot_is_nonmutating_without_explicit_coordination(self):
        _, a, _, _ = self._fixture()
        before = _run(a, "rev-parse", "HEAD")
        packet = self._runtime(a).bootstrap(
            agent_id="library-a",
            task="library observation",
            fetch=False,
            shared_remote_mode="REQUIRED",
            continuation_shared_remote_mode="DISABLED",
        )
        self.assertEqual(packet["coordination"]["pre_dispatch"], "DISABLED")
        self.assertEqual(_run(a, "rev-parse", "HEAD"), before)

    def test_call_tool_schema_and_dispatch_accept_coordination_fields(self):
        _, a, _, _ = self._fixture()
        boot_schema = next(tool["inputSchema"] for tool in AGENT_BOOT_TOOLS if tool["name"] == "athena_agent_bootstrap")
        for field in ("coordination_mode", "work_key", "targets", "coordination_claim_mode", "lease_seconds", "replication_reason"):
            self.assertIn(field, boot_schema["properties"])
        protocol_boot = next(tool for tool in TOOLS if tool["name"] == "athena_agent_bootstrap")
        self.assertIn("coordination_mode", protocol_boot["inputSchema"]["properties"])
    ...
    def test_extension_spec_declares_message_board_as_sole_authority(self):
        ...
        spec = json.loads((Path(__file__).resolve().parents[1] / "spec" / "AGENT_BOOT_MESSAGE_BOARD_V1.json").read_text(encoding="utf-8"))
        self.assertEqual(spec["artifact"], "ATHENA.AGENT.BOOT.MESSAGE_BOARD.V1")
        self.assertEqual(spec["authority"]["message_board"], "sole presence/claim/message/ack authority")
        self.assertEqual(spec["inputs"]["coordination_mode"]["tool_default_request"], "AUTO")
        self.assertIn("BOOT_OBSERVATION != WORK_START", spec["laws"])
        self.assertIn("MISSING_WORK_KEY != FABRICATED_MATA_WORK_KEY", spec["laws"])
