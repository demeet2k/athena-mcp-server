...
PINNED_FROZEN_CANONICAL_SHA256="3ecc6a6db7a649d73a7d6d3b5d62382060fdfb5d25006ac9610a444e72311129"
...
class MythicHolonomyAlchemyHardeningTests(unittest.TestCase):
    ...
    def test_static_fixture_is_semantically_exact_but_not_remote_evidence(self):
        self.assertEqual(SOURCE_PACKET_CANONICAL_SHA256,PINNED_FROZEN_CANONICAL_SHA256)
        self.assertEqual(canonical_packet_sha256(),PINNED_FROZEN_CANONICAL_SHA256)
        self.assertEqual(self.result["packet_semantic_sha256"],PINNED_FROZEN_CANONICAL_SHA256)
    ...
    def test_caller_structured_loss_cannot_self_mint_typed_coverage(self):
        packet=deepcopy(PACKET)
        target=next(c for c in packet["cases"] if c["case_id"]=="HOL-H01-04")
        target["declared_loss"]=[{
            "typed_feature_covers":[
                "role_delta","decoder_delta","ontology_delta","authority_delta",
                "standing_delta","provenance_delta"
            ]
        }]
        result=MythicHolonomyRuntime().evaluate(packet)
        index={c["case_id"]:i for i,c in enumerate(packet["cases"])}
        loop=result["arms"]["A2_COMPOSED_HOLONOMY"]["results"][index["HOL-H01-04"]]
        self.assertIsNone(loop["holonomy_vector"]["unaccounted_loss"])
        self.assertEqual(loop["unaccounted_loss_standing"],"UNKNOWN_UNTYPED_LOSS_LEDGER")
        self.assertEqual(result["projection_policy"]["typed_loss_coverage"],"UNAVAILABLE_V0_NO_VERIFIED_TYPED_LOSS_SCHEMA")
        self.assertIn("CALLER_STRUCTURED_LOSS != VERIFIED_TYPED_COVERAGE",result["laws"])
