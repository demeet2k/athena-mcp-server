...
def anchor(epoch=1):
    ...
    return {
        ...
        "agent_coordinate_name": "A1",
        "reseed_epoch": epoch,
        "pulse_age_before": 1,
        "pulse_age_after": 0,
        "git": {
            "repo": "demeet2k/athena-mcp-server",
            "ref": "refs/heads/agent/campaign-v3-life-dispatch-v1-r2",
            "head_before": head,
            "head_after": head,
            "tree_after": "tree-a",
            "changed": False,
        ...
        "target_versions": [{"id": QUEST, "version": VERSION}],
...
def current_positions(head="a" * 40, tree="tree-a"):
    return [
        {
            "repo": "demeet2k/athena-mcp-server",
            "ref": "refs/heads/agent/campaign-v3-life-dispatch-v1-r2",
...
def outcome(
    ...
    *,
    ...
    return life.resolve_observed_gtc_outcome(
        ...
        current_git_positions=current_positions() if positions is None else positions,
    )
...
class CampaignV3LifeDispatchTests(unittest.TestCase):
    ...
    def test_anchor_subject_mismatch_holds_before_campaign_mutation(self):
        bad = anchor()
        bad["agent_coordinate_name"] = "OTHER"
        with patch.object(life, "bind_current_pulse_branch_to_loop") as mocked:
            receipt = life.bind_current_pulse_branch_with_life_policy(
                binding_kwargs={"campaign_id": "GAME"},
                quest_id=QUEST,
            ...
            mocked.assert_not_called()
        self.assertEqual("HOLD_INVALID_LIFE_BINDING", receipt["status"])
        self.assertIn("RESEED_ANCHOR:reseed_anchor_agent_mismatch", receipt["failures"])

    def test_reseed_anchor_replay_burns_life_but_cannot_reseed_twice(self):
        receipt = binding()
        world = outcome(
            receipt,
            receipt["initial_world"],
        ...
        self.assertEqual(1, len(world["consumed_reseed_anchor_digests"]))
        out = outcome(
            receipt,
            world,
            status="FAILED",
        ...
        self.assertEqual("LIFE_CONSUMED_ANCHOR_HOLD", out["status"])
        self.assertIn("reseed_anchor_replay", out["failures"])
        self.assertEqual(1, out["world"]["agents"]["A1"]["base_lives_remaining"])

    def test_stale_git_position_burns_life_but_blocks_reseed(self):
        receipt = binding()
        out = outcome(
            receipt,
            receipt["initial_world"],
            ...
            conditions={"tests_pass": False, "readback_pass": True},
            reseed=anchor(1),
            positions=current_positions(head="b" * 40),
        )
        self.assertEqual("LIFE_CONSUMED_ANCHOR_HOLD", out["status"])
        self.assertTrue(any(x.startswith("reseed_anchor_stale_head:") for x in out["failures"]))
        self.assertEqual(2, out["world"]["agents"]["A1"]["base_lives_remaining"])

    def test_missing_current_git_positions_burns_life_and_holds(self):
        receipt = binding()
        out = life.resolve_observed_gtc_outcome(
            dispatch_receipt=receipt,
            world=receipt["initial_world"],
            completion=completion("FAILED"),
            executed=True,
            hard_gate_status="FAIL",
            clear_condition_outcomes={"tests_pass": False, "readback_pass": True},
            witnesses=["witness"],
            reseed_anchor=anchor(1),
            current_git_positions=None,
        )
        self.assertEqual("LIFE_CONSUMED_ANCHOR_HOLD", out["status"])
        self.assertIn("current_git_positions_required", out["failures"])
        self.assertEqual(2, out["world"]["agents"]["A1"]["base_lives_remaining"])
