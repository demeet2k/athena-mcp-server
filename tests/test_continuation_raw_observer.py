...
from athena_mcp.continuation_raw_observer import ARTIFACT, TOOL_NAME, ContinuationRawObserver
...
class ContinuationRawObserverTests(unittest.TestCase):
    def test_collects_exact_raw_records_deterministically_without_classification(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            first = observer.read(window_start="2026-08-09T10:00:00Z", window_end="2026-08-09T11:00:00Z", expected_git_head=git.head())
            second = observer.read(window_start="2026-08-09T10:00:00+00:00", window_end="2026-08-09T11:00:00+00:00", expected_git_head=git.head())
            ...
            self.assertEqual(receipt["record"]["completion"]["terminal_gate"]["status"], "REJECTED_CONTINUE")
            ...
            self.assertRegex(receipt["record_canonical_sha256"], r"^sha256:[0-9a-f]{64}$")
            self.assertNotEqual(receipt["record_sha256"], receipt["record_canonical_sha256"])
    ...
    def test_window_is_half_open(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            _commit_trace(root)
            result = ContinuationRawObserver(git).read(window_start="2026-08-09T10:00:00Z", window_end="2026-08-09T11:00:00Z")
    ...
    def test_malformed_tracked_source_fails_coverage_closed(self):
        with tempfile.TemporaryDirectory() as td:
            root, git = _repo(Path(td))
            _write(root, "prompts/rehydration/loop-a/receipts/0001.json", {"artifact": "WRONG.ARTIFACT", "created_at": "2026-08-09T10:05:00Z"})
            _commit_trace(root)
            result = ContinuationRawObserver(git).read(window_start="2026-08-09T10:00:00Z", window_end="2026-08-09T11:00:00Z")
    ...
    def test_stale_expected_head_holds(self):
        with tempfile.TemporaryDirectory() as td:
            _, git = _repo(Path(td))
            result = ContinuationRawObserver(git).read(window_start="2026-08-09T10:00:00Z", window_end="2026-08-09T11:00:00Z", expected_git_head="0" * 40)
    ...
    def test_record_limit_is_typed_incomplete_coverage_not_silent_truncation(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            _commit_trace(root)
            result = ContinuationRawObserver(git).read(window_start="2026-08-09T10:00:00Z", window_end="2026-08-09T11:00:00Z", max_records=1)
            self.assertEqual(result["status"], "TRACE_INTEGRITY_HOLD")
    ...
    def test_read_does_not_mutate_git(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            result = ContinuationRawObserver(git).read(window_start="2026-08-09T10:00:00Z", window_end="2026-08-09T11:00:00Z", expected_git_head=before_head)
    ...
    def test_tool_is_registered_and_prompt_runtime_dispatches_it(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            result = runtime.call_tool(TOOL_NAME, {"window_start": "2026-08-09T10:00:00Z", "window_end": "2026-08-09T11:00:00Z", "expected_git_head": git.head()})
