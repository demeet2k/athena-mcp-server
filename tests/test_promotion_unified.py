...
class PromotionUnifiedTests(unittest.TestCase):
    ...
    def witness(head,kind='ci'):
        # Caller-bound synthetic packet: useful for predicate wiring, never a trusted
    ...
    def trusted(head,ci,smoke):
        return {
            'observed':True,'verifier':'TEST.TRUSTED.HOST','verification_ref':'trusted://promotion-test',
            'head_sha':head,'ci_ref':ci['ref'],'smoke_ref':smoke['ref'],
        }
    ...
    def test_mcp_schema_does_not_allow_caller_to_inject_trusted_verification(self):
        ...
        result=self.tool('athena_promotion_evaluate',{
            'git_head':head,'ci_witness':ci,'smoke_witness':smoke,
            'trusted_external_verification':self.trusted(head,ci,smoke),'persist':False,
        },expect_error=True)
    ...
    def test_host_internal_trusted_verifier_can_qualify_pure_predicate(self):
        ...
        self.assertEqual(result['status'],'QUALIFIED',result);self.assertTrue(result['promotion_allowed']);self.assertFalse(result['external_verification_required'])
        self.assertEqual(result['attestation_level'],'EXTERNALLY_VERIFIED')
        self.assertEqual(result['gates']['external_verification']['status'],'PASS')
    ...
    def test_historical_v1_qualified_receipt_remains_replayable_but_separate(self):
        head='legacyabcdef1234';ci=self.witness(head,'ci');smoke=self.witness(head,'smoke')
        inputs={'candidate_server':'Server','git_head':head,'surface_audit':self.surface(),'ci_witness':ci,'smoke_witness':smoke,'local_git_status':{'enabled':False}}
        ...
        with self.server.store.db:
            self.server.store.db.execute('INSERT INTO promotion_runs VALUES(?,?,?,?,?,?,?,?,?)',(run_id,'Server',head,'QUALIFIED',json.dumps(inputs,sort_keys=True),json.dumps(cert,sort_keys=True),dd,'EID.LEGACY.PROMOTION',0.0))
    ...
    def test_promotion_resource_and_benchmark_state_current_trust_boundary(self):
        ...
        self.assertEqual(bench['unified_manifest_version'],'ATHENA.RUNTIME.UNIFIED.7')
