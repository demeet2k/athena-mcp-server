...
class _Board:
    ...
    def read(self, **kwargs): return {"status": "OK", "active": list(self.active), "shared_frontier_verified": True}
...
def _brain(base: Path, kinds=None):
    ...
    git = GitBackend(root); prompt = PromptRuntime(git); pipeline = RollingQuestPipelineRuntime(git, prompt)
    breadth = NextQuestBreadthRuntime(pipeline); board = _Board(); economy = NextScoutEconomyRuntime(pipeline, breadth, board)
    start = pipeline.start(goal="g", quests=["Q1", "Q2", "Q3"], expected_git_head=git.head())
    planned = breadth.plan(pipeline_id=start["pipeline_id"], expected_pipeline_state_digest=start["state_digest"], expected_git_head=git.head(), kinds=kinds or ["DEPENDENCY_MAP", "RISK_SCAN", "SOURCE_REVIEW"])
...
class NextScoutEconomyTests(unittest.TestCase):
    ...
    def _allocate(self, **updates):
        args = dict(pipeline_id=self.start["pipeline_id"], expected_pipeline_state_digest=self.start["state_digest"], shared_remote_mode="DISABLED")
        args.update(updates); return self.economy.economy(**args)

    def _plan(self, kind, task):
        return next(p for p in self.planned["plans"] if p["kind"] == kind and p["quest"]["task"] == task)
    ...
    def test_resource_profiles_are_explicit_planning_estimates(self):
        ...
        self.assertEqual((result["authority"], result["claim_effect"]), ("ROUTING_ONLY", "NONE"))

    def test_reserve_vector_can_make_only_low_cost_risk_scan_feasible(self):
        result = self._allocate(max_scouts=1, reserve_slots=0, token_budget=7000, reserve_tokens=4000,
            minute_budget=20, reserve_minutes=5, tool_call_budget=5, reserve_tool_calls=1,
            coordination_budget=4, reserve_coordination=1, git_risk_budget=3, reserve_git_risk=1)
        self.assertEqual(result["status"], "SELECTED")
        self.assertEqual(result["selected"][0]["kind"], "RISK_SCAN")
        self.assertEqual(result["selected"][0]["quest"]["task"], "Q2")

    def test_active_scout_consumes_slot_and_resource_budget_first(self):
        dep = self._plan("DEPENDENCY_MAP", "Q2")
    ...
    def test_active_scouts_over_slot_budget_hold(self):
        a = self._plan("DEPENDENCY_MAP", "Q2"); b = self._plan("RISK_SCAN", "Q3")
        self.board.active = [
            {"agent_id": "a", "work_key": f"next-prep:{self.start['pipeline_id']}:{a['plan_id']}"},
            {"agent_id": "b", "work_key": f"next-prep:{self.start['pipeline_id']}:{b['plan_id']}"},
        ]
        result = self._allocate(max_scouts=2, reserve_slots=1)
        self.assertEqual(result["status"], "ACTIVE_SCOUTS_EXCEED_SLOT_BUDGET_HOLD")
        self.assertEqual(result["active_count"], 2)
        self.assertEqual(result["usable_slots"], 1)

    def test_active_scouts_over_resource_budget_hold(self):
        dep = self._plan("DEPENDENCY_MAP", "Q2")
        self.board.active = [{"agent_id": "a", "work_key": f"next-prep:{self.start['pipeline_id']}:{dep['plan_id']}"}]
        result = self._allocate(max_scouts=2, reserve_slots=0, token_budget=3000, reserve_tokens=0)
        self.assertEqual(result["status"], "ACTIVE_SCOUTS_EXCEED_RESOURCE_BUDGET_HOLD")
        self.assertIn("tokens", result["oversubscribed_resources"])

    def test_budget_can_legitimately_select_no_new_work(self):
        result = self._allocate(max_scouts=1, reserve_slots=0, token_budget=1000, reserve_tokens=0,
            minute_budget=5, reserve_minutes=0, tool_call_budget=1, reserve_tool_calls=0,
            coordination_budget=1, reserve_coordination=0, git_risk_budget=1, reserve_git_risk=0)
        self.assertEqual(result["status"], "BUDGET_HOLD_NO_FEASIBLE_NEW_WORK")
        self.assertEqual(result["selected_plan_ids"], [])

    def test_economy_is_read_only_and_deterministic(self):
        before = self.git.head(); a = self._allocate(); middle = self.git.head(); b = self._allocate(); after = self.git.head()
        self.assertEqual((before, middle, after), (before, before, before))
        self.assertEqual(a["allocation_digest"], b["allocation_digest"])

    def test_version(self): self.assertEqual(VERSION, "ATHENA.NEXT.SCOUT.ECONOMY.5")
