...
class CampaignV3LifeDispatchBridgeTests(unittest.TestCase):
    ...
    def test_translates_to_canonical_dispatch_shape_without_execution_authority(self):
        out = _translate()
        ...
        self.assertEqual(AGENT_ID, dispatch["agent_id"])
        self.assertEqual("FAIL_CLEAR", dispatch["result_class"])
        self.assertEqual("FAIL", dispatch["hard_gate_status"])
    ...
    def test_condition_observation_order_does_not_change_canonical_output(self):
        first = _translate()
    ...
    def test_condition_identity_ref_and_agent_binding_are_required(self):
        ...
        self.assertEqual("HOLD_DISPATCH_TRANSLATION", no_ref["status"])
        self.assertIn("CONDITION_IDENTITY_REF_REQUIRED", no_ref["failures"])
        wrong_agent = _translate(agent_id="ANOTHER-AGENT")
        self.assertEqual("HOLD_DISPATCH_TRANSLATION", wrong_agent["status"])
        self.assertIn("AGENT_ID_CAMPAIGN_COORDINATE_MISMATCH", wrong_agent["failures"])

    def test_compiler_anchor_digest_metadata_is_stripped_before_dispatch(self):
        packet = _packet()
        self.assertIn("anchor_digest", packet["RESEED_ANCHOR"])
        out = _translate(campaign_packet=packet)
        self.assertEqual("TRANSLATED", out["status"])
        self.assertNotIn("anchor_digest", out["dispatch_packet"]["reseed_anchor"])
    ...
    def test_reward_is_forwarded_only_when_campaign_eligibility_is_true(self):
        ...
        self.assertEqual("HOLD_DISPATCH_TRANSLATION", held["status"])
    ...
    def test_source_pin_is_exact_and_effect_standing_remains_unknown(self):
        out = _translate()
