...
class CollectiveV11UnifiedTests(unittest.TestCase):
    ...
    def test_v11_tools_resource_and_surface_are_advertised(self):
        ...
        for n in ('athena_gp_hyperfit','athena_gp_decision_evsi','athena_latent_admg_project','athena_causal_tmle_stacked','athena_sensitivity_rr_surface','athena_bapomdp_solve','athena_evidence_dependence_interval'):
    ...
    def test_gp_hyperfit_and_evsi_are_design_reads_unless_explicit_cas_apply(self):
        ...
        self.assertFalse(fit['applied']);after_fit=self.tool('athena_gp_state',{'context_key':'G11'});self.assertEqual(before,after_fit)
        evsi=self.tool('athena_gp_decision_evsi',{'context_key':'G11','actions':[{'id':'a','base_utility':.1,'utility_weights':{'x':1.0}},{'id':'b','base_utility':.3,'utility_weights':{'x':-.5}}],'experiments':[{'id':'e','features':{'x':.5},'noise_variance':.03}],'samples':40,'seed':2})
        self.assertEqual(evsi['decision'],'GP_MONTE_CARLO_EVSI_DESIGN_ONLY');self.assertEqual(before['observation_count'],self.tool('athena_gp_state',{'context_key':'G11'})['observation_count'])
    ...
    def test_latent_projection_and_bapomdp_do_not_mutate_canonical_state(self):
        ...
        proj=self.tool('athena_latent_admg_project',{'edges':[{'src':'L','dst':'X'},{'src':'L','dst':'Y'},{'src':'X','dst':'Y'}],'latent_nodes':['L'],'observed_nodes':['X','Y']})
        self.assertEqual(proj['status'],'SUPPLIED_DAG_ADMG_PROJECTION');self.assertEqual(before_edges,len(self.server.store.rows('SELECT * FROM edges')))
        models=[{'id':'M1','prior':.5,'states':['S'],'actions':[{'id':'A','reward_by_state':{'S':1.0},'transition':{'S':{'S':1.0}},'observation':{'S':{'o':1.0}}}]},{'id':'M2','prior':.5,'states':['S'],'actions':[{'id':'A','reward_by_state':{'S':.8},'transition':{'S':{'S':1.0}},'observation':{'S':{'o':1.0}}}]}]
        plan=self.tool('athena_bapomdp_solve',{'models':models,'initial_state_belief':{'S':1.0},'horizon':2,'max_nodes':10000})
        self.assertEqual(plan['decision'],'BAYES_ADAPTIVE_FINITE_MODEL_POMDP_PLAN_ONLY');self.assertEqual(plan['certificate'],'EXACT_FOR_SUPPLIED_FINITE_MODEL_SET_AND_HORIZON')
    ...
    def test_v11_dependence_interval_and_causal_read_do_not_change_y1(self):
        ...
        interval=self.tool('athena_evidence_dependence_interval',{'scope':'V11','features':{'same_dataset':1.0}});self.assertEqual(interval['status'],'LAPLACE_LOGISTIC_DEPENDENCE_INTERVAL')
        ...
        blocked=self.tool('athena_causal_tmle_stacked',{'samples':rows,'treatment':'T','outcome':'Y','adjustment':['X'],'assumptions':{'latent_confounding_possible':True}});self.assertEqual(blocked['status'],'UNIDENTIFIED_LATENT_CONFOUNDING_RISK')
