...
def _material_and_hold(runtime, root, loop, *, filename: str, residual: str, actor: str = "agent-a") -> dict:
...
class RehydrationEpochRolloverTests(unittest.TestCase):
    ...
    def test_hold_rolls_to_published_child_without_mutating_finished_parent(self):
        td = tempfile.TemporaryDirectory(); self.addCleanup(td.cleanup)
        base = Path(td.name)
        root, origin, runtime = _seed_shared_brain(base)
        started = _start_epoch(runtime)
        held = _material_and_hold(runtime, root, started, filename="epoch0.txt", residual="Implement epoch one material work")
        ...
        parent_state_blob_before = _run(root, "rev-parse", f"{held['checkpoint_head']}:{held['state_path']}").stdout.strip()
        ...
        parent_state_blob_after = _run(root, "rev-parse", f"{shared_head}:{held['state_path']}").stdout.strip()
        ...
        cold = _clone(origin, base / "cold-observer", "cold-observer")
        ...
        row = next(x for x in cold.index()["loops"] if x["loop_id"] == rolled["child_loop_id"])
        self.assertEqual(row["epoch"]["cumulative_steps_before"], 1)

    def test_stale_cold_agent_reuses_existing_child_instead_of_forking_parent(self):
        td = tempfile.TemporaryDirectory(); self.addCleanup(td.cleanup)
        base = Path(td.name)
        root, origin, runtime_a = _seed_shared_brain(base)
        started = _start_epoch(runtime_a)
        held = _material_and_hold(runtime_a, root, started, filename="parent.txt", residual="unique child task")

        # Clone B while the shared tip is still the held parent. B retains exact
        # parent identities, then A rolls/publishes a child.
        root_b = base / "agent-b"
        runtime_b = _clone(origin, root_b, "agent-b")
        parent_b = runtime_b.resume(held["loop_id"])
        self.assertEqual(parent_b["checkpoint_head"], held["checkpoint_head"])
        self.assertEqual(parent_b["state_digest"], held["state_digest"])
        baton_b = parent_b["successor_baton"]

        child_a = _roll(runtime_a, held, max_epochs=4, max_total_steps=4)
        self.assertEqual(child_a["status"], "EPOCH_STARTED")

        # B is stale. Rollover must sync/FF first, discover A's durable sidecar,
        # and return the same child instead of constructing a sibling.
        reused = runtime_b.call_tool(TOOL_NAME, {
            "parent_loop_id": held["loop_id"],
            "expected_checkpoint_head": parent_b["checkpoint_head"],
            "expected_state_digest": parent_b["state_digest"],
            "expected_successor_baton_digest": baton_b["baton_digest"],
            "actor": "agent-b",
            "shared_remote_mode": "REQUIRED",
            "max_epochs": 4,
            "max_total_steps": 4,
        })
        self.assertEqual(reused["status"], "EPOCH_ALREADY_ROLLED", reused)
        self.assertTrue(reused["durable_return"])
        self.assertTrue(reused["reused_existing_child"])
        self.assertEqual(reused["child_loop_id"], child_a["child_loop_id"])
        self.assertEqual(_run(root_b, "rev-parse", "HEAD").stdout.strip(), child_a["published_head"])
        sidecars = list((root_b / "prompts/rehydration").glob("*/epoch.json"))
        matching = [json.loads(p.read_text()) for p in sidecars if json.loads(p.read_text())["parent_loop_id"] == held["loop_id"]]
    ...
    def test_cumulative_budget_is_monotone_across_two_rollovers(self):
        ...
        held2 = _material_and_hold(runtime, root, child2, filename="e2.txt", residual="forbidden epoch three")
    ...
    def test_frozen_budget_cannot_increase_and_total_budget_exhausts(self):
        td = tempfile.TemporaryDirectory(); self.addCleanup(td.cleanup)
        base = Path(td.name)
        root, _, runtime = _seed_shared_brain(base)
        started = _start_epoch(runtime)
    ...
    def test_non_max_parent_and_missing_successor_fail_closed(self):
        td = tempfile.TemporaryDirectory(); self.addCleanup(td.cleanup)
        base = Path(td.name)
        root, _, runtime = _seed_shared_brain(base)
        ...
        current = started
        for index in (1, 2):
            _write(root, f"work{index}.txt", f"{index}\n")
            _run(root, "add", f"work{index}.txt")
            _run(root, "commit", "-m", f"work{index}")
            _run(root, "push", "origin", "main")
            current = runtime.advance(
                loop_id=started["loop_id"],
                expected_checkpoint_head=current["checkpoint_head"],
                expected_state_digest=current["state_digest"],
                expected_prompt_digest=current["prompt_digest"],
                completion={
                    "status": "SUCCEEDED", "observed": True, "terminal": False, "summary": f"step {index}",
                    "passes": _passes(), "tests": [{"name": f"step{index}", "status": "PASS"}],
                    "evidence_refs": [f"git://work{index}"], "residuals": [], "next_task": f"explicit task {index + 1}",
                },
                shared_remote_mode="REQUIRED",
            )
        self.assertEqual(current["status"], "HOLD_MAX_STEPS")
        self.assertIsNone(current.get("successor_baton"))
        missing = runtime.call_tool(TOOL_NAME, {
            "parent_loop_id": current["loop_id"],
            "expected_checkpoint_head": current["checkpoint_head"],
            "expected_state_digest": current["state_digest"],
            "expected_successor_baton_digest": "0" * 64,
            "shared_remote_mode": "REQUIRED",
