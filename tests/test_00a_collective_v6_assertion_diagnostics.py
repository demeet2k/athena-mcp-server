...
class CollectiveRuntimeV6AssertionDiagnostics(unittest.TestCase):
    def test_v6_assertions_named(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.db') as f:
            ...
            def p_claim():
                c=srv.call_tool('athena_claim_register',{'claim_key':'C','statement':'s'});[srv.call_tool('athena_claim_witness',{'claim_id':c['claim_id'],'kind':'REPLICATION','result':'SUPPORTS','independence_key':k}) for k in ('r1','r2')];assert srv.call_tool('athena_claim_state',{'claim_id':c['claim_id']})['status']=='REPLICATED_SUPPORT';srv.call_tool('athena_claim_witness',{'claim_id':c['claim_id'],'kind':'FALSIFIER','result':'FALSIFIES','independence_key':'red'});assert srv.call_tool('athena_claim_state',{'claim_id':c['claim_id']})['status']=='CONTESTED'
