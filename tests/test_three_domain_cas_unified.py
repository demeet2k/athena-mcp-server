...
class ThreeDomainCASUnifiedTests(unittest.TestCase):
    ...
    def semantic_head(self):
        return self.server.core.navigate(self.oid)['current_version']['vid']
    def topology_state(self):
        return self.server.collective_memory.topology_get('TOPO.CAS')
    ...
    def test_stale_topology_version_does_not_mutate_semantic_or_git(self):
        ...
        # A stale expected_version must fail before any replacement can commit.
        with self.assertRaises(Exception):
            self.server.collective_memory.topology_apply('TOPO.CAS',0,'REPLACE',{'modules':[{'id':'M2','role':'stale'}],'edges':[]},'TEST')
