...
class DocumentationConsistencyTests(unittest.TestCase):
    def test_release_base_and_effective_architecture_coordinates_agree(self):
        ...
        # Existing V13 documents remain the scientific/base carrier.
        ...
        # Effective current architecture is an additive coordination overlay.
        current=text('CURRENT_ARCHITECTURE.md')
        for phrase in ('ATHENA.RUNTIME.UNIFIED.10','UNIFIED.10 = UNIFIED.9 + COORDINATION_ARCHITECTURE_V1','MESSAGE_BOARD_V1','COHESION_MESH_V1','PARTY_REWARD_PROVENANCE_V3','ATHENA.ORGAN.INVENTORY.1','ATHENA.ARCHITECTURE.DRIFT.1','unclassified_surface'):
            self.assertIn(phrase,current)
        coord=text('spec/ATHENA_COORDINATION_ARCHITECTURE_V1.md')
        for phrase in ('Y1 != MB','FUZZY_SIMILARITY != DUPLICATE_PROOF','CAUSAL_EFFECT = UNKNOWN','PARTY_RESULT != RESULT_TRUTH','ORGAN_INVENTORY.1','ARCHITECTURE_DRIFT.1','OBSERVE_EXPANSION_FRONTIER'):
    ...
    def test_executable_witnesses_cover_scientific_and_coordination_planes(self):
        ...
        for phrase in ('Documentation and repository-brain consistency','Coordination organ inventory and architecture drift','test_architecture_drift.py','test_message_board_registration.py','test_cohesion_evidence_guard.py','test_party_reward_provenance.py','V13 robust continuous-bayes causal-control and authority boundaries','Promotion trust and exact-head predicate','promotion-qualification'):
            self.assertIn(phrase,ci)
        drift=text('athena_mcp/architecture_drift.py')
        for phrase in ('ATHENA.ORGAN.INVENTORY.1','ATHENA.ARCHITECTURE.DRIFT.1','MESSAGE_BOARD_V1','COHESION_EVIDENCE_GUARD_V1','PARTY_REWARD_PROVENANCE_V3','unclassified_surface'):
            self.assertIn(phrase,drift)
        effective=text('athena_mcp/coordination_manifest.py')
        for phrase in ('ATHENA.RUNTIME.UNIFIED.10','MESSAGE_BOARD_V1','ORGAN_INVENTORY.1','ARCHITECTURE_DRIFT_AUDIT.1','ORGAN_INVENTORY_EXPANSION'):
