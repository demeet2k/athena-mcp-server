...
class DocumentationConsistencyTests(unittest.TestCase):
    def test_release_manifest_and_current_brain_coordinates_agree(self):
        ...
        self.assertEqual(UNIFIED_MANIFEST_VERSION, 'ATHENA.RUNTIME.UNIFIED.6')

        readme = text('README.md')
        self.assertIn('# ATHENA Canonical MCP v3.0', readme)
        self.assertIn('Collective V1–V11', readme)
        self.assertIn('ATHENA.RUNTIME.UNIFIED.6', readme)
        ...
        self.assertIn('ATHENA.RUNTIME.UNIFIED.6', architecture)
        self.assertIn('V11 Adaptive World Model + Latent Causal Geometry', architecture)
        ...
        self.assertIn('ATHENA.RUNTIME.UNIFIED.6', migration)
    ...
    def test_executable_witnesses_name_current_release_and_v11(self):
        smoke = text('smoke.py')
        self.assertIn("init['serverInfo']['version']=='3.0.0'", smoke)
        self.assertIn('V11 ADAPTIVE', smoke)
        self.assertIn("manifest['artifact']=='ATHENA.RUNTIME.UNIFIED.6'", smoke)
