...
COMMAND_PROTOCOL_ID_V2 = "NEXT57_COMMAND_PROTOCOL_V2"
COMMAND_PACKET_SCHEMA_ID_V2 = "NEXT57_COMMAND_EVENT_PACKET_V2"
COMMAND_CAPILLARY_LAW_ID_V2 = "NEXT57_COMMAND_CAPILLARY_UPDATE_V2"
COMMAND_REWARD_FIELD_ID_V2 = "NEXT57_COMMAND_REWARD_FIELD_V2"
COMMAND_ROUTE_POLICY = "goal+salience+joy_potential+pheromone_composite+coord_distance+capability_match+current_load"
JOY_ROUTE_POLICY = COMMAND_ROUTE_POLICY
JOY_REWARD_POLICY = COMMAND_REWARD_FIELD_ID_V2
EDGE_COMPAT_WEIGHT = 0.35
...
class CommandMembraneConfig:
    ...
    rho_1: float = 0.22
    rho_b: float = 0.18
    reward_alpha: float = 0.40
    reward_beta: float = 0.25
    reward_gamma: float = 0.20
    reward_delta: float = 0.15
    reward_lambda: float = 0.35
    crown_full: float = 2.00
    crown_detect: float = 0.40
    crown_route: float = 0.40
    crown_act: float = 0.80
    epsilon: float = 0.01
...
class CommandMembraneService:
    ...
    def load_state(self) -> dict[str, Any]:
        default = {
            ...
            "docs_gate_status": self.docs_gate_status()["state"],
            "watcher_mode": "powershell-filesystemwatcher",
            "known_files": {},
            "dedupe": {},
            "last_event_seq": 0,
            "last_liminal_seq": 0,
    ...
    def save_state(self, state: dict[str, Any]) -> None:
        ...
        state["watcher_mode"] = "powershell-filesystemwatcher"
    ...
    def save_edges(self, payload: dict[str, Any]) -> None:
        payload["updated_at"] = utc_now()
        write_json(self.config.edge_path, payload)
...
def _command_candidate_edge_snapshot(self: CommandMembraneService, ant_id: str) -> dict[str, float]:
    ...
    composite = round(golden + (self.config.bridge_weight * bridge), 6)
    return {
...
def _command_pheromone_composite_for_edge(self: CommandMembraneService, candidate: dict[str, Any]) -> float:
    return round(
        max(
            ...
            + (self.config.bridge_weight * float(candidate.get("bridge_pheromone", 0.0) or 0.0)),
        ),
        6,
    )
...
def _command_v2_verification_defaults(
    ...
    *,
    ...
    result_key = result.lower().strip()
    if stage == "detect":
        witness, basis, closure_class = 0.40, "worker+replay detect witness", "first_detect"
    elif stage == "route":
        witness, basis, closure_class = 0.60, "route persisted with replay witness", "first_route"
    elif stage == "claim":
        witness, basis, closure_class = 0.60, "first valid act claim", "first_act"
        ...
        witness, basis, closure_class = 0.60, "truthful blocked attempt", "meaningful_blocked_attempt"
    elif result_key == "quarantined":
        witness, basis, closure_class = 0.60, "truthful quarantined attempt", "meaningful_blocked_attempt"
...
def _command_v2_phi_for_stage(
    ...
    if verification_state.closure_class == "full_verified_closure":
        return 0.0
    if verification_state.closure_class == "assist_with_useful_contribution":
        return round(-math.pi / 2.0, 6)
    if verification_state.closure_class == "meaningful_blocked_attempt":
        return round(math.pi / 2.0, 6)
...
def _command_v2_compose_reward_state(
    ...
    *,
    ...
    del priority, confidence
    latency_score = math.exp(-1.0 * self.config.reward_lambda * max(float(tau_seconds), 0.0))
    affect_intensity_a = round(
        math.pi * self.clamp01((0.50 * float(effort_quality)) + (0.30 * float(latency_score)) + (0.20 * float(contribution_share))),
        6,
    )
    ...
    return CommandRewardStateV2(
        ...
        attempt_reward=round(self.config.reward_alpha * float(effort_quality), 6),
        latency_reward=round(self.config.reward_beta * float(latency_score), 6),
        first_reward=round(self.first_reward_for_stage(stage, crown_tier), 6),
        assist_reward=round(self.config.reward_gamma * float(contribution_share), 6),
        learning_reward=round(self.config.reward_delta * float(novelty_score), 6),
        total_reward=0.0,
...
def _command_v2_write_live_writeback_surfaces(self: CommandMembraneService, protocol: dict[str, Any], reward: dict[str, Any], capillary: dict[str, Any]) -> None:
    ...
    queue_body = "\n".join(
        [
            ...
            f"- Ranking terms: `{', '.join(protocol['routing_defaults']['selector_terms'])}`",
    ...
    prompt_body = "\n".join(
        [
            ...
            "2. Route with joy-native terms only: goal, salience, joy_potential, pheromone_composite, coord_distance, capability_match, current_load.",
            "3. Tie reward amplification to verified `H_verified = H * v`, never self-report.",
...
def _command_v2_ensure_protocol_artifacts(self: CommandMembraneService) -> dict[str, Any]:
    ...
    protocol = {
        ...
        "authority_mode": "sole_live_authority",
        "migration_mode": "Replace As Canon",
        "signal_source": "Derived Hybrid",
        "compatibility_role": "legacy read surfaces remain compatibility-only",
        "docs_gate": docs_gate,
        "docs_gate_status": docs_gate["state"],
        "active_surface": rel(self.config.command_surface_root),
        "command_folder_root": rel(self.config.command_surface_root),
        ...
        "routing_defaults": {
            ...
            "policy_expression": JOY_ROUTE_POLICY,
            "selector_terms": [
                "goal",
                "salience",
                "joy_potential",
        ...
        "compatibility_aliases": {
            ...
            "receipt.capillary_delta": "compatibility-only alias for gold_deposit + 0.35 * bridge_deposit",
            "edge.edge_strength": "compatibility-only alias for golden_pheromone + 0.35 * bridge_pheromone",
        },
        "reward_principles": {
    ...
    schema = {
        ...
        "packet_types": {
            "CommandEventPacket": {
                "required": ["event_id", "source_path", "goal", "priority", "confidence", "coord12", "joy_seed", "pheromone"],
                "joy_seed_fields": ["a_seed", "phi_seed", "h_seed", "joy_potential", "route_mode_seed", "source_evidence"],
            },
    ...
    reward = {
        ...
        "runtime_aliases": {
            ...
            "verified_alignment_score": "H_verified",
            "reward_multiplier": "nectar multiplier",
            "crown_tier": "crown result",
        },
        "formulas": {
            "H": "H = (a/pi) * ((1 + cos(phi)) / 2)",
            "H_verified": "H_verified = H * v",
            "M_run": "M_run(H_verified) = min(M_max, 1 / (1 - H_verified + epsilon))",
            "a_seed": "a_seed = pi * clamp(0.60 * priority + 0.40 * confidence, 0, 1)",
            "a_verified": "a_verified = pi * clamp(0.50 * effort_quality + 0.30 * latency_score + 0.20 * contribution_share, 0, 1)",
            "R_total": "R_total = M_run * (r_try + r_speed + r_first + r_assist + r_learn)",
            "delta_golden": "delta_golden = mu * R_total * c_ij",
            "delta_bridge": "delta_bridge = nu * r_try * (1 - H_verified) * c_ij",
        },
        "coefficients": {
        ...
        "crown_rules": {"detect": "first verified detect", "route": "first verified route", "act": "first valid act claim", "prime": "first verified full closure"},
    }
    capillary = {
        "law_id": COMMAND_CAPILLARY_LAW_ID_V2,
        "canonical_protocol_id": COMMAND_PROTOCOL_ID_V2,
        "nonnegative_capillary_law": True,
        "formula": "golden_next = (1 - rho_ij) * golden + delta_golden ; bridge_next = (1 - rho_b) * bridge + delta_bridge",
        "delta_golden": "delta_golden = mu * R_total * c_ij",
        "delta_bridge": "delta_bridge = nu * r_try * (1 - H_verified) * c_ij",
        "evaporation_formula": "rho_ij = rho_0 + rho_1 * (1 - avg_H_verified)",
        "edge_strength_formula": "edge_strength = golden_pheromone + 0.35 * bridge_pheromone",
        "edge_classes": ["ephemeral", "capillary", "vein"],
        "compatibility_projection": protocol["compatibility_aliases"],
    }
    latency = {
    ...
    write_json(
        self.config.protocol_v1_registry_path,
        {
            ...
            "compatibility_role": "read-only mirror of V2 canonical law",
            "canonical_protocol_id": COMMAND_PROTOCOL_ID_V2,
    ...
    write_text(self.config.protocol_v1_manifest_path, "# COMMAND Membrane Protocol V1\n\n- Compatibility role: `read-only mirror of V2 canonical law`.\n")
...
def _command_v2_score_candidate(self: CommandMembraneService, packet: CommandEventPacketV2, candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    goal_score = round(max(0.0, self.goal_match_score(packet, candidate)), 6)
    salience_score = round(max(0.0, float(packet.priority)), 6)
    joy_potential = round(max(0.0, float(joy_seed.get("h_seed", packet.pheromone))), 6)
    pheromone_composite = self.pheromone_composite_for_edge(candidate)
    coord_distance = round(max(0.0, self.coordinate_match_score(packet, candidate)), 6)
    capability_match = round(max(0.0, self.capability_match_score(packet, candidate) + (0.25 * self.file_family_match_score(packet, candidate))), 6)
    current_load = round(float(candidate.get("load", 0.0) or 0.0), 6)
    score = round(goal_score + salience_score + joy_potential + pheromone_composite + coord_distance + capability_match - current_load, 6)
    return {
        **candidate,
        "goal": goal_score,
        "salience": salience_score,
        "joy_potential": joy_potential,
        "pheromone_composite": pheromone_composite,
...
def _command_v2_route_event(self: CommandMembraneService, event_id: str, state: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    decision = CommandRouteDecisionV2(
        ...
        route_inputs={
            "goal": worker_choice["goal"],
            "salience": worker_choice["salience"],
...
def _command_v2_reinforce_event(self: CommandMembraneService, event_id: str, path: str | None = None, result: str = "success", latency_score: float | None = None) -> dict[str, Any]:
    ...
    for idx in range(len(nodes) - 1):
        ...
        compat_edge_strength = round(gold_next + (self.config.bridge_weight * bridge_next), 6)
