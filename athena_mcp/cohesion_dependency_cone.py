...
def _add_caller_edges(graph: dict, caller_edges: Optional[Iterable[Mapping[str, Any]]]) -> list[dict]:
    ...
    for index, raw in enumerate(caller_edges or []):
        ...
        if not evidence:
            unresolved.append({"code": "CALLER_EDGE_MISSING_EVIDENCE", "edge_index": index, "src": src, "dst": dst})
        _add_edge(graph["adjacency"], graph["edge_keys"], src, "CALLER_SUPPLIED_EDGE", dst, standing="CALLER_SUPPLIED", evidence_refs=evidence, metadata={"declared_relation": relation, "edge_index": index})
    return unresolved
...
def _change_seed(board, graph: dict, change: Mapping[str, Any]) -> tuple[str, dict, list[dict], bool]:
    ...
    if kind not in CHANGE_KINDS:
        raise ValueError(f"change.kind must be one of {sorted(CHANGE_KINDS)}")
    normalized: dict = {"kind": kind}; unresolved: list[dict] = []; git_change = False
    root = _node("change", _digest(dict(change))[:24])

    def seed(prefix: str, value: Any, relation: str, *, metadata=None, standing="OBSERVED_CHANGE"):
        raw = _text(value)
        if raw:
            _add_edge(graph["adjacency"], graph["edge_keys"], root, relation, _node(prefix, raw), standing=standing, evidence_refs=[f"change:{kind}"], metadata=metadata)

    if kind == "GIT_RANGE":
        git_change = True; git, extra = _git_range(board, change); normalized.update(git); unresolved.extend(extra)
        if git.get("status") != "OK":
            return root, normalized, unresolved, git_change
        for path in git.get("changed_paths") or []:
            seed("target", path, "CHANGE_GIT_TARGET", metadata={"path": path}); seed("dep", path, "CHANGE_GIT_DEPENDENCY", metadata={"path": path})
    elif kind == "GIT_PATHS":
        git_change = True; paths = _targets(change.get("changed_paths"))
        if not paths:
            raise ValueError("GIT_PATHS requires non-empty changed_paths")
        normalized["changed_paths"] = paths
        for path in paths:
            seed("target", path, "CHANGE_GIT_TARGET", metadata={"path": path}, standing="CALLER_SUPPLIED_CHANGE"); seed("dep", path, "CHANGE_GIT_DEPENDENCY", metadata={"path": path}, standing="CALLER_SUPPLIED_CHANGE")
    elif kind == "CLAIM":
        claim_id = _text(change.get("claim_id")); agent_id = _text(change.get("agent_id")); work_key = _text(change.get("work_key")); targets = _targets(change.get("targets"))
        if not any((claim_id, agent_id, work_key, targets)):
            raise ValueError("CLAIM requires claim_id, agent_id, work_key, or targets")
        normalized.update({"claim_id": claim_id or None, "agent_id": agent_id or None, "work_key": work_key or None, "targets": targets, "status_change": _text(change.get("status_change")) or None})
        seed("claim", claim_id, "CHANGE_CLAIM"); seed("agent", agent_id, "CHANGE_AGENT"); seed("work", work_key, "CHANGE_WORK_KEY")
        for path in targets:
            seed("target", path, "CHANGE_TARGET", metadata={"path": path})
    elif kind == "MESSAGE":
        event_id = _text(change.get("event_id")); recipients = _refs(change.get("recipients")); sender = _text(change.get("sender")); work_key = _text(change.get("work_key")); targets = _targets(change.get("targets")); requires_ack = bool(change.get("requires_ack"))
        if not event_id and not recipients:
            raise ValueError("MESSAGE requires event_id or recipients")
        normalized.update({"event_id": event_id or None, "sender": sender or None, "recipients": recipients, "work_key": work_key or None, "targets": targets, "requires_ack": requires_ack})
        if event_id:
            if event_id in graph["messages"]:
                seed("message", event_id, "CHANGE_MESSAGE")
            else:
                unresolved.append({"code": "MESSAGE_EVENT_NOT_OBSERVED", "event_id": event_id})
        if recipients and (not event_id or event_id not in graph["messages"]):
            for recipient in recipients:
                _add_edge(graph["adjacency"], graph["edge_keys"], root, "MESSAGE_RECIPIENT", _node("agent", recipient), standing="CALLER_SUPPLIED_CHANGE", evidence_refs=[f"change:{kind}"], metadata={"ack_required": requires_ack, "message_id": event_id or None})
        if work_key:
            seed("work", work_key, "CHANGE_WORK_KEY", standing="CALLER_SUPPLIED_CHANGE")
        for path in targets:
            seed("target", path, "CHANGE_TARGET", metadata={"path": path}, standing="CALLER_SUPPLIED_CHANGE")
    elif kind == "WORK_KEY":
        work_key = _text(change.get("work_key"))
        if not work_key:
            raise ValueError("WORK_KEY requires work_key")
        normalized["work_key"] = work_key; seed("work", work_key, "CHANGE_WORK_KEY")
    elif kind == "TARGET":
        targets = _targets(change.get("targets"))
        if not targets:
            raise ValueError("TARGET requires non-empty targets")
        ...
        if not refs:
            raise ValueError("DEPENDENCY requires dependency_refs")
        normalized["dependency_refs"] = refs
        for ref in refs:
            seed("dep", ref, "CHANGE_DEPENDENCY", metadata={"dependency_ref": ref})
    elif kind == "COHESION_ENTRY":
        entry_id = _text(change.get("entry_id")); goal_ref = _text(change.get("goal_ref")); work_key = _text(change.get("work_key")); targets = _targets(change.get("targets")); dependencies = _refs(change.get("dependency_refs"))
        if not any((entry_id, goal_ref, work_key, targets, dependencies)):
            raise ValueError("COHESION_ENTRY requires entry_id or explicit entry coordinates")
        normalized.update({"entry_id": entry_id or None, "goal_ref": goal_ref or None, "work_key": work_key or None, "targets": targets, "dependency_refs": dependencies})
        if entry_id:
            if entry_id in graph["entries"]:
                seed("request", entry_id, "CHANGE_COHESION_ENTRY")
            elif entry_id in graph["campaigns"]:
                seed("campaign", entry_id, "CHANGE_COHESION_ENTRY")
            else:
                unresolved.append({"code": "COHESION_ENTRY_NOT_OBSERVED", "entry_id": entry_id})
        seed("goal", goal_ref, "CHANGE_GOAL_REF", standing="CALLER_SUPPLIED_CHANGE"); seed("work", work_key, "CHANGE_WORK_KEY", standing="CALLER_SUPPLIED_CHANGE")
        for path in targets:
            seed("target", path, "CHANGE_TARGET", metadata={"path": path}, standing="CALLER_SUPPLIED_CHANGE")
        for ref in dependencies:
            seed("dep", ref, "CHANGE_DEPENDENCY", metadata={"dependency_ref": ref}, standing="CALLER_SUPPLIED_CHANGE")
    elif kind == "DECISION":
        decision_ref = _text(change.get("decision_ref")); refs = _refs(change.get("refs"))
        if not decision_ref:
            raise ValueError("DECISION requires decision_ref")
        normalized.update({"decision_ref": decision_ref, "refs": refs}); decision = _node("decision", decision_ref)
        _add_edge(graph["adjacency"], graph["edge_keys"], root, "CHANGE_DECISION", decision, standing="CALLER_SUPPLIED_CHANGE", evidence_refs=[f"decision:{decision_ref}"])
        unresolved.append({"code": "DECISION_RUNTIME_UNAVAILABLE", "decision_ref": decision_ref, "standing": "CALLER_SUPPLIED_ONLY"})
        for ref in refs:
            _add_edge(graph["adjacency"], graph["edge_keys"], decision, "CALLER_SUPPLIED_EDGE", _parse_node_ref(ref), standing="CALLER_SUPPLIED", evidence_refs=[f"decision:{decision_ref}"], metadata={"declared_relation": "DECISION_AFFECTS"})
    return root, normalized, unresolved, git_change


def _enumerate_paths(adjacency: Mapping[str, list[dict]], root: str, max_depth: int, max_paths: int = 4000):
    stack: list[tuple[str, list[dict], set[str]]] = [(root, [], {root})]; agent_paths: Dict[str, list[list[dict]]] = defaultdict(list); visited_nodes: set[str] = {root}; truncated = False; cutoff_nodes: set[str] = set(); count = 0
    while stack:
        node, path, seen = stack.pop(); outgoing = [edge for edge in adjacency.get(node, []) if edge.get("propagate")]
        if len(path) >= max_depth:
            if outgoing:
                truncated = True; cutoff_nodes.add(node)
            continue
        for edge in outgoing:
            dst = str(edge["dst"])
            if dst in seen:
                continue
            next_path = path + [edge]; visited_nodes.add(dst); count += 1
            if count > max_paths:
                truncated = True; cutoff_nodes.add(node); return agent_paths, visited_nodes, truncated, sorted(cutoff_nodes)
            if dst.startswith("agent:"):
                agent_paths[dst.split(":", 1)[1]].append(next_path)
            stack.append((dst, next_path, seen | {dst}))
    return agent_paths, visited_nodes, truncated, sorted(cutoff_nodes)
...
def _actions(paths: list[list[dict]], *, git_change: bool) -> list[str]:
    ...
    if "MESSAGE_RECIPIENT" in relations:
        ...
        if any(bool(meta.get("ack_required")) for meta in metadata):
            actions.add("ACK_MESSAGE")
    return sorted(actions)


def _path_public(path: list[dict]) -> dict:
    semantic_depth = len(_semantic_edges(path))
    return {"nodes": [path[0]["src"]] + [edge["dst"] for edge in path] if path else [], "relations": [edge["relation"] for edge in path], "reasons": sorted({_reason(edge) for edge in path}), "standing": sorted({str(edge.get("standing")) for edge in path}), "evidence_refs": sorted({ref for edge in path for ref in (edge.get("evidence_refs") or [])}), "semantic_depth": semantic_depth, "direct": semantic_depth <= 1}
...
def _lane_rows(graph: dict, agent_paths: Mapping[str, list[list[dict]]], *, git_change: bool):
    ...
    for agent_id in sorted(agent_paths):
        ...
        if not presence:
            continue
        unique = {}
        for path in [_path_public(path) for path in agent_paths[agent_id]]:
            unique[json.dumps(path, sort_keys=True, separators=(",", ":"))] = path
...
def _party_impacts(graph: dict, affected_agents: set[str]) -> list[dict]:
    ...
    for agent_id in sorted(affected_agents):
        for party_id in sorted(graph["party_memberships"].get(agent_id, set())):
            row = parties.setdefault(party_id, {"party_id": party_id, "affected_member_ids": [], "affected_member_goal_refs": {}, "other_members_auto_invalidated": False, "law": "PARTY_MEMBERSHIP != UNIVERSAL_DEPENDENCY"})
            row["affected_member_ids"].append(agent_id); row["affected_member_goal_refs"][agent_id] = list(graph["party_goal_refs"].get((party_id, agent_id), []))
...
def _visited_entries(graph: dict, visited_nodes: set[str]) -> list[dict]:
    ...
    for request_id, payload in sorted(graph["entries"].items()):
        if _node("request", request_id) not in visited_nodes:
            continue
...
def _graph_digest(adjacency: Mapping[str, list[dict]]) -> str:
    ...
    for src in sorted(adjacency):
        for edge in adjacency[src]:
            edges.append({"src": edge["src"], "relation": edge["relation"], "dst": edge["dst"], "standing": edge["standing"], "evidence_refs": edge.get("evidence_refs") or [], "propagate": bool(edge.get("propagate")), "metadata": edge.get("metadata") or {}})
    edges.sort(key=lambda row: (row["src"], row["relation"], row["dst"], row["standing"], json.dumps(row["metadata"], sort_keys=True, separators=(",", ":"))))
...
def augment_dependency_cone_resource(resource: Mapping[str, Any]) -> dict:
    ...
    if "athena_cohesion_dependency_cone" not in tools:
        tools.append("athena_cohesion_dependency_cone")
    value["tools"] = tools; residual = []
    for item in value.get("residual") or []:
        residual.append("remaining C3 steering tools 13-15" if str(item) in {"remaining C3 steering tools 12-15", "C3 steering tools 12-15"} else item)
...
def dependency_cone(cohesion_runtime: Any, party_runtime: Any, *, change: Mapping[str, Any], caller_edges: Optional[Iterable[Mapping[str, Any]]] = None, max_depth: int = 4, remote: str = "origin", shared_remote_mode: str = "REQUIRED") -> dict:
    ...
    if normalized_change.get("status") == "GIT_REF_HOLD":
        classification = "GIT_REF_HOLD"; agent_paths = {}; visited_nodes = {root}; truncated = False; cutoff_nodes = []
    else:
        agent_paths, visited_nodes, truncated, cutoff_nodes = _enumerate_paths(graph["adjacency"], root, max_depth); classification = "TARGETED_CONE" if agent_paths else "NO_OBSERVED_AFFECTED_LANES"
    if truncated:
    ...
    if git_change and not affected:
        ...
        if classification == "NO_OBSERVED_AFFECTED_LANES":
            classification = "NO_OBSERVED_LANE_HIT_WITH_UNKNOWN_SEMANTIC_RESIDUE"
    propagation_paths = []
    for row in affected:
        for path in row["propagation_paths"]:
            propagation_paths.append({"agent_id": row["agent_id"], **path})
    ...
    value["decision_digest"] = _digest({"classification": classification, "change": normalized_change, "directly_affected": direct, "transitively_affected": transitive, "affected_entries": entries, "parties": parties, "required_actions": required_actions, "unaffected_observed_lanes": unaffected, "unresolved": unresolved, "propagation_paths": propagation_paths, "max_depth": max_depth, "truncated": bool(truncated), "graph_digest": graph_digest, "mata": value["mata"]}); return value
