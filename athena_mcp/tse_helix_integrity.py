...
class TseHelixIntegrityRuntime(TseHelixRuntime):
    """TSE Helix with strict handoff-envelope and shared-adoption integrity.
    ...
    It also extends the existing `athena_tse_helix_advance` state machine with
    operation APPLY. APPLY never performs a merge; it delegates to the
    Self-Tightening Knot observer, which accepts only an already-current shared
    Git ancestry adoption.

    No path here creates claim, merge, execution, life, or causal authority.
    """
