...
def rollover(
    ...
    *,
    ...
    """Create one bounded descendant epoch from a verified HOLD_MAX_STEPS parent."""
    ...
    if mode == "DISABLED":
        ...
        # Epoch creation changes mission lineage. BEST_EFFORT is not allowed to
        # speculate on an unverified shared frontier; only explicit DISABLED mode
        # may construct a local-only epoch.
    ...
    # Construct the child completely locally. The shared remote will only see the
    # ref after both the start commit and epoch sidecar commit exist.
...
EPOCH_TOOL = {
    "name": TOOL_NAME,
    "description": (
        "Roll a verified HOLD_MAX_STEPS rehydration loop into one bounded descendant epoch without resetting cumulative "
        "mission budgets. Requires an exact selected successor baton; the old epoch remains immutable and the complete child "
        "start+lineage is published as one shared ref transition."
    ),
    "inputSchema": {
        "type": "object",
        "required": [
