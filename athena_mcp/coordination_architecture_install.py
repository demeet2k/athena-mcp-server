...
from .architecture_drift_protocol import ARCHITECTURE_DRIFT_RESOURCES,ARCHITECTURE_DRIFT_RESOURCE_URIS
...
def install_coordination_architecture(namespace:dict[str,Any])->None:
    ...
    # OMEGA descriptor overlay; no remote coordination sync during observation.
    old_project=sp.project_omega
    if 'coordination' not in sp.OMEGA_COMPONENTS:sp.OMEGA_COMPONENTS=tuple(list(sp.OMEGA_COMPONENTS)+['coordination'])
    def project(server):
