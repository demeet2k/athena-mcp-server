...
from .project_atlas_surface import ProjectAtlasSurface
...
AOR_DEVELOPMENT_TOOLS=(
    ...
    list(BIONANOMACHINE_TOOLS)+list(PROJECT_ATLAS_TOOLS)
)
AOR_DEVELOPMENT_RESOURCES=(
    ...
    list(BIONANOMACHINE_RESOURCES)+[PROJECT_ATLAS_RESOURCE]
)
...
class AorDevelopmentSurface:
    ...
    Constructor order is intentional:
      ...
      bounded BNMK biological-mechanism operator library -> read-only Project Atlas.
    ...
    def __init__(self,server):
        ...
        self.project_atlas=ProjectAtlasSurface(server)
    ...
    def call_tool(self,name:str,args:Dict[str,Any]):
        ...
        handled,value=self.project_atlas.call_tool(name,args)
    ...
    def read_resource(self,uri:str):
        if uri==PROJECT_ATLAS_RESOURCE['uri']:return self.project_atlas.read_resource(uri)
    ...
    def benchmark(self):
        result={};result.update(self.equivalence.benchmark());result.update(self.extraction.benchmark());result.update(self.retrieval.benchmark());result.update(self.hug.benchmark());result.update(self.gap.benchmark());result.update(self.field.benchmark());result.update(self.transport.benchmark());result.update(self.integrity.benchmark());result.update(self.cycle.benchmark());result.update(self.qhug_pareto_kernel.benchmark());result.update(self.bionanomachine.benchmark());result.update(self.project_atlas.benchmark());return result
