...
AOR_DEVELOPMENT_TOOLS=(
    ...
    list(QHUG_PARETO_KERNEL_TOOLS)+list(RECOVERY_TOOLS)
)
AOR_DEVELOPMENT_RESOURCES=(
    ...
    [CYCLE_RESOURCE,QHUG_PARETO_KERNEL_RESOURCE,RECOVERY_RESOURCE]
)
...
class AorDevelopmentSurface:
    ...
    Constructor order is intentional:
      ...
      read-only Drive recovery index.
    ...
    def benchmark(self):
        ...
        result.update(self.transport.benchmark())
        result.update(self.integrity.benchmark())
        result.update(self.cycle.benchmark())
        result.update(self.qhug_pareto_kernel.benchmark())
        result.update(self.drive_recovery.benchmark())
        return result
