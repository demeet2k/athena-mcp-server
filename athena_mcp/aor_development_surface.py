...
from .party_surface import PartySurface,PARTY_RESOURCE,PARTY_TOOLS
...
AOR_DEVELOPMENT_TOOLS=(
    ...
    list(PARTY_TOOLS)+list(BIONANOMACHINE_TOOLS)
)
AOR_DEVELOPMENT_RESOURCES=(
    ...
    list(AOR_COLLECTIVE_TRANSPORT_RESOURCES)+list(INTEGRITY_RESOURCES)+[CYCLE_RESOURCE,QHUG_PARETO_KERNEL_RESOURCE,PARTY_RESOURCE]+
...
class AorDevelopmentSurface:
    ...
    Constructor order is intentional:
      ...
      party coordination/credit boundary -> bounded BNMK biological-mechanism
      operator library.
    ...
    def __init__(self,server):
        ...
        self.party=PartySurface(server)
    ...
    def call_tool(self,name:str,args:Dict[str,Any]):
        ...
        handled,value=self.party.call_tool(name,args)
    ...
    def read_resource(self,uri:str):
        ...
        if uri==PARTY_RESOURCE['uri']:return self.party.read_resource(uri)
    ...
    def benchmark(self):
        result={};result.update(self.equivalence.benchmark());result.update(self.extraction.benchmark());result.update(self.retrieval.benchmark());result.update(self.hug.benchmark());result.update(self.gap.benchmark());result.update(self.field.benchmark());result.update(self.transport.benchmark());result.update(self.integrity.benchmark());result.update(self.cycle.benchmark());result.update(self.qhug_pareto_kernel.benchmark());result.update(self.party.benchmark());result.update(self.bionanomachine.benchmark());return result