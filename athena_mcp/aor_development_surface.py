...
from .drive_recovery_surface import DriveRecoverySurface
from .drive_recovery_protocol import RECOVERY_RESOURCE,RECOVERY_TOOLS
...
AOR_DEVELOPMENT_TOOLS=(
    ...
    list(BIONANOMACHINE_TOOLS)+list(RECOVERY_TOOLS)
)
AOR_DEVELOPMENT_RESOURCES=(
    ...
    list(BIONANOMACHINE_RESOURCES)+[RECOVERY_RESOURCE]
)
...
class AorDevelopmentSurface:
    ...
    Constructor order is intentional:
      ...
      bounded BNMK biological-mechanism operator library -> read-only Drive recovery index.
    ...
    def __init__(self,server):
        ...
        self.drive_recovery=DriveRecoverySurface()
    ...
    def call_tool(self,name:str,args:Dict[str,Any]):
        ...
        if handled:return True,value
        handled,value=self.drive_recovery.call_tool(name,args)
        if handled:return True,value
    ...
    def read_resource(self,uri:str):
        ...
        if uri==RECOVERY_RESOURCE['uri']:return self.drive_recovery.read_resource(uri)
    ...
    def benchmark(self):
        result={};result.update(self.equivalence.benchmark());result.update(self.extraction.benchmark());result.update(self.retrieval.benchmark());result.update(self.hug.benchmark());result.update(self.gap.benchmark());result.update(self.field.benchmark());result.update(self.transport.benchmark());result.update(self.integrity.benchmark());result.update(self.cycle.benchmark());result.update(self.qhug_pareto_kernel.benchmark());result.update(self.bionanomachine.benchmark());result.update(self.drive_recovery.benchmark());return result
