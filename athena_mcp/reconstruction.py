...
class ReconstructionLedger:
    """Freeze one OMEGA projection plus the exact declared source contract."""
    ...
    def compile(self,task_ref:str,source_refs:Iterable[str],expected_refs:Optional[Iterable[str]]=None,actor:str='agent',persist:bool=True):
        ...
        omega=project_omega(self.server);defects=[]
        ...
        rd=digest(carrier,64);output={**carrier,'reconstruction_digest':rd,'status':'COMPLETE_WITH_DEFECTS' if defects else 'COMPLETE'}
        if not persist:return {**output,'persisted':False}
        parent=self.s.head('global');pe=parent['eid'] if parent else None
        payload={'operation':'RECONSTRUCT','task_ref':task_ref,'source_refs':source_refs,'expected_refs':expected,'defect_count':len(defects),'omega_id':omega['omega_id'],'reconstruction_digest':rd}
        ...
        with self.s.db:
            self.s.db.execute('''INSERT INTO reconstruction_runs(
                run_id,task_ref,actor,source_refs_json,expected_refs_json,omega_json,defects_json,reconstruction_digest,eid,created_at
            ) VALUES(?,?,?,?,?,?,?,?,?,?)''',(
                run_id,task_ref,actor,json.dumps(source_refs,sort_keys=True),json.dumps(expected,sort_keys=True),
                json.dumps(omega,sort_keys=True,ensure_ascii=False),json.dumps(defects,sort_keys=True),rd,eid,time.time()))
    ...
    def get(self,run_id:str):
        ...
        return {
            'run_id':row['run_id'],'task_ref':row['task_ref'],'actor':row['actor'],
            'source_refs':json.loads(row['source_refs_json']),'expected_refs':json.loads(row['expected_refs_json']),
            'omega':json.loads(row['omega_json']),'defects':json.loads(row['defects_json']),
            'reconstruction_digest':row['reconstruction_digest'],'eid':row['eid'],'created_at':row['created_at'],
        }

    def verify(self,run_id:str):
        stored=self.get(run_id)
        carrier={'version':RECON_VERSION,'task_ref':stored['task_ref'],'source_refs':stored['source_refs'],'expected_refs':stored['expected_refs'],'omega':stored['omega'],'defects':stored['defects'],'boundary':BOUNDARY}
    ...
    def benchmark(self):
        return {'reconstruction_runs':self.s.one('SELECT COUNT(*) n FROM reconstruction_runs')['n']}
