...
CREATE TABLE IF NOT EXISTS authority_claims(
 claim_id TEXT PRIMARY KEY,source_ref TEXT NOT NULL,y TEXT NOT NULL,status TEXT NOT NULL,
 evidence_json TEXT NOT NULL,test_json TEXT NOT NULL,canonical_ref TEXT,last_eid TEXT NOT NULL,updated_at REAL NOT NULL);
CREATE TABLE IF NOT EXISTS authority_events(
 authority_event_id TEXT PRIMARY KEY,claim_id TEXT NOT NULL,operation TEXT NOT NULL,from_y TEXT,to_y TEXT,payload_json TEXT NOT NULL,eid TEXT NOT NULL,created_at REAL NOT NULL);
...
def _evidence(items:Optional[Iterable[Mapping[str,Any]]])->list[Dict[str,Any]]:
    result=[]
    for raw in items or []:
        item=dict(raw);kind=str(item.get('kind') or 'support');ref=_verified_ref(item,'evidence')
...
def _verified_test(test:Mapping[str,Any])->Dict[str,Any]:
    test=dict(test or {});missing=[n for n in ('procedure','observation','result','witness') if not test.get(n)]
    ...
    elif not str(witness).strip():raise ValueError('test witness required')
    return test
...
class AuthorityLedger:
    def __init__(self,core):
        ...
        self.core.register('TOOL','AUTHORITY','PROMOTE','CLAIM_STATE','Y_TYPED_WITNESS_GATES',{'claim':'stable','target':'?+!#','witnesses':'typed'},{'y':'?+!#','status':'typed','history':'events'},actor='GENESIS.AOR.Y1',status='CANONICAL')
    def _event(self,claim_id,operation,from_y,to_y,payload,actor):
        parent=self.s.head('global');pe=parent['eid'] if parent else None;ep={'claim_id':claim_id,'operation':operation,'from_y':from_y,'to_y':to_y,**dict(payload)};eid=event_id(f'AUTHORITY_{operation}',actor,pe,ep);ed=digest(ep,32);aeid='AUTH.'+digest({'eid':eid,'claim':claim_id,'op':operation},24)
    ...
    def state(self,claim_id:str):
        row=self.s.one('SELECT * FROM authority_claims WHERE claim_id=?',(claim_id,));return self._decode(row) if row else None
    def list(self,y:Optional[str]=None,status:Optional[str]=None,limit:int=100):
        ...
        return [self._decode(r) for r in self.s.rows(f'SELECT * FROM authority_claims{where} ORDER BY updated_at DESC LIMIT ?',tuple(args))]
    def promote(self,claim_id,target_y,evidence=None,test=None,canonical_authority=None,actor='agent'):
        ...
        prior_evidence=json.loads(row['evidence_json']);prior_test=json.loads(row['test_json']);canonical_ref=row['canonical_ref'];payload={}
        ...
        elif current=='+' and target_y=='!':prior_test=_verified_test(test or {});payload['test_witness']=prior_test.get('witness')
        elif current=='!' and target_y=='#':canonical_ref=_canonical_authority(canonical_authority or {});payload['canonical_ref']=canonical_ref
    ...
    def challenge(self,claim_id,witness,reason,actor='agent'):
        ...
        current=row['y'];new_y='#' if current=='#' else '?';status='CANONICAL_CHALLENGED' if current=='#' else 'CHALLENGED';aeid,eid=self._event(claim_id,'CHALLENGE',current,new_y,{'witness_ref':ref,'reason':reason,'status':status},actor)
    ...
    def resolve_canonical_challenge(self,claim_id,decision,authority,actor='agent'):
        row=self.s.one('SELECT * FROM authority_claims WHERE claim_id=?',(claim_id,))
        if not row:raise KeyError('unknown claim')
        ...
        new_y='#' if decision=='UPHOLD' else '!';aeid,eid=self._event(claim_id,'RESOLVE_CANONICAL_CHALLENGE','#',new_y,{'decision':decision,'authority_ref':ref},actor)
        with self.s.db:self.s.db.execute("UPDATE authority_claims SET y=?,status='ACTIVE',canonical_ref=?,last_eid=?,updated_at=? WHERE claim_id=?",(new_y,ref,eid,time.time(),claim_id))
