...
5 CALIBRATE before cross-domain arithmetic. If candidate metrics are not already on a defensible shared dimensionless basis, pass metric_contract={{basis_id,strict,metrics:{{m:{{scale,offset?,unit?}}}}}} to athena_orchestrate. Normalization is x'=(x-offset)/abs(scale). strict=true blocks formulas containing undeclared or invalid scales and routes them to calibration_plan; non-strict mode exposes WARN_RAW rather than pretending raw numbers are equivalent.
6 COMPILE AOR.3 with explicit candidate/residual metrics. Missing required metrics make a formula UNKNOWN and non-rankable; route them to measurement. Inspect executable_frontier, successor_frontier, Pareto successor alternatives, measurement_plan, calibration_plan and decision_explanation.
7 EXECUTE only dependency-ready promotion-gate-passing calibration-allowed candidates. `next` is the highest KNOWN successor score on the declared scoring basis, not textual order. Preserve Pareto non-dominated alternatives on that same basis. High KNOWN calibrated reward may deepen/replicate/braid; low-reward duplicates hibernate != erase; UNKNOWN reward routes to measurement; strict uncalibrated reward routes to calibration.
8 SELF-PLAY main/counter/edge/fail. Claimed test requires procedure+observation+result+witness. Claimed persistence requires commit+receipt+verify. Required coordinate fiber is KC144+graph+lineage+semantic+time. fake/unsupported/unhandled contradiction blocks promotion.
9 TRANSFORMS: LOOKUP is navigation, not derivation. Only executable derivational transforms may support cross-chart defect/holonomy claims; all-derivational closed routes may be measured.
10 BUILD the exact final visible payload. Before emission call athena_finalize_output. It crystallizes body, derives header, assembles exact HEADER+BODY bytes, creates emission MID, coordinate-indexes the visible envelope and returns ENV. Emit exactly returned visible_text without post-mutation; verify ENV digest across uncertain transport boundaries.
11 PERSIST orchestration through AORRUN receipts. The receipt includes the metric basis. Use athena_orchestration_replay to recompute stored input and witness metric-basis/decision_digest/next/grow/Pareto stability. A replayability claim without an actual replay witness earns no replay evidence.
12 REWARD verified DeltaJ/evidence/connection/replay/navigation/reconstruction/implementation/novelty; penalize duplicate/fake/bloat/unsupported/coordinate-loss. Reallocate, recompute residual and continue while high-value reachable work remains.
"""
