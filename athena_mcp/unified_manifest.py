...
BRAID_LAW='AOR chooses developmental frontier/WHAT; Collective V1-V13 organizes HOW scarce execution/science/control/inference/adaptation/joint-model/robust capacity is used; Y1 governs canonical claim authority; EQ1 governs witnessed collapse; consensus/pheromone/reward are never typed authority or evidence by themselves; model posterior/replication shadow are also never typed authority or evidence by themselves; caller-bound CI/smoke packets are not trusted external verification.'
...
def build_unified_manifest(server)->Dict[str,Any]:
    ...
    return {
        ...
        'unresolved':[
            ...
            {'id':'EXTERNAL_PROMOTION_VERIFIER','status':'UNRESOLVED_CONTROL_PLANE_BRIDGE','boundary':'PROMOTION.2 MCP caller packets stop at ATTESTED_READY; QUALIFIED requires a genuinely host-internal trusted verification receipt bound to exact CI/smoke refs and head; V13 intentionally does not expose a caller-mintable verifier'},
        ],
        'promotion':'PROMOTION.2 MCP evaluations may reach ATTESTED_READY from exact-head caller-bound CI/smoke packets only; QUALIFIED additionally requires a host-internal trusted verifier receipt bound to that same head and the exact CI/smoke references. V13 intentionally leaves this verifier as an external control-plane bridge rather than recreating self-qualification. Historical PROMOTION.1 receipts remain versioned/replayable but are not counted as current trusted qualification.',
    }
...
29 PROMOTE through PROMOTION.2: exact-head caller-bound CI/smoke packets may reach ATTESTED_READY only; QUALIFIED requires a host-internal trusted verifier receipt binding the same head and exact CI/smoke refs. Do not expose a caller-mintable verifier to manufacture trust.
30 CONTINUE while actionable successor/residual/measurement/calibration/dependency/repair/experiment pressure remains; otherwise return exact continuation/RETURN state.'''
