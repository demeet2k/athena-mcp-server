...
UNIFIED_MANIFEST_VERSION='ATHENA.RUNTIME.UNIFIED.4'

LAYERS=[
 'CCR','JSPACE','SCALE','KC144','POLYCOORDINATE_ATLAS','CRYSTAL_OUTPUT_ABI',
 'COLLECTIVE_RUNTIME_V1','COLLECTIVE_GROWTH_V1','COLLECTIVE_MEMORY_V2','COLLECTIVE_LEARNING_V3','COLLECTIVE_ECOLOGY_V4','COLLECTIVE_SCIENCE_V5','COLLECTIVE_DISCOVERY_V6','COLLECTIVE_DUAL_CONTROL_V7','COLLECTIVE_BELIEF_V8','COLLECTIVE_INFERENCE_V9',
...
INVARIANTS=[
 ...
 'athena_claim_* = Y1 canonical authority; athena_discovery_claim_* = V6-V9 science-shadow replication/falsification metadata; namespaces must never alias',
]

CYCLE='HYDRATE -> RECONRUN/OMEGA -> MEMORY -> EXTRACT -> RETRIEVE -> HUG -> GAP -> FIELD -> MEASURE -> AUTHORITY/AOR -> COLLECTIVE(V1-V9) -> EXECUTE -> VERIFY -> LEARN -> SUCCESSOR -> COMPLETE'
BRAID_LAW='AOR chooses developmental frontier/WHAT; Collective V1-V9 organizes HOW scarce execution/science/control/inference capacity is used; Y1 governs canonical claim authority; EQ1 governs witnessed collapse; consensus/pheromone/reward are never typed authority or evidence by themselves; model posterior/replication shadow are also never typed authority or evidence by themselves.'
...
def build_unified_manifest(server)->Dict[str,Any]:
    ...
    return {
        'artifact':UNIFIED_MANIFEST_VERSION,'artifact_compat':['ATHENA.RUNTIME.UNIFIED.1','ATHENA.RUNTIME.UNIFIED.2','ATHENA.RUNTIME.UNIFIED.3'],'role':'live machine-readable runtime architecture projection','runtime_class':type(server).__name__,
        'layers':list(LAYERS),'navigation':'KC144 <-> SCALE <-> JSPACE <-> AOR <-> Collective(V1-V9) <-> Git/MCP <-> Source/RETURN','cycle':CYCLE,'invariants':list(INVARIANTS),'braid_law':BRAID_LAW,
        ...
        'claim_namespace_law':'athena_claim_* is canonical Y1 authority; athena_discovery_claim_* is V6-V9 science-shadow state; no RPC-name aliasing or implicit promotion/demotion is permitted',
        ...
        'unresolved':[
            ...
            {'id':'MODEL_TO_AUTHORITY_BRIDGE','status':'EXPLICIT_WITNESS_REQUIRED','boundary':'V3-V9 predictions, beliefs, estimates, experiments, discovery claims, replication diagnostics and control plans cannot enter Y1/AOR evidence lanes without explicit observed/witnessed transport and authority gating'},
            {'id':'GENERAL_BELIEF_CONTROL','status':'UNRESOLVED_OPTIONAL_RESEARCH','boundary':'V8/V9 belief-control is bounded finite/discrete or finite-dimensional Gaussian-linear machinery; general Bayes-adaptive POMDP/nonparametric inference is not claimed'},
            {'id':'FORMAL_CAUSAL_DISCOVERY','status':'UNRESOLVED_OPTIONAL_RESEARCH','boundary':'V8/V9 bootstrap/partial graphs are heuristic uncertainty-preserving hypothesis surfaces, not validated FCI/PAG/CPDAG causal discovery'},
        ],
