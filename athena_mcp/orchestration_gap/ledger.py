...
CREATE TABLE IF NOT EXISTS gap_runs(
 run_id TEXT PRIMARY KEY,task_ref TEXT NOT NULL,actor TEXT NOT NULL,input_json TEXT NOT NULL,output_json TEXT NOT NULL,decision_digest TEXT NOT NULL,eid TEXT NOT NULL,created_at REAL NOT NULL);
CREATE INDEX IF NOT EXISTS idx_gap_runs_created ON gap_runs(created_at);
'''

def _carrier(output):
    return {'closure_kind':output.get('closure_kind'),'policy':output.get('policy'),'closure_nodes':output.get('closure_nodes'),'admissible_edge_ids':output.get('admissible_edge_ids'),'rejected_edges':output.get('rejected_edges'),'covered_target_ids':output.get('covered_target_ids'),'gap_target_ids':output.get('gap_target_ids'),'ranked_gap_ids':output.get('ranked_gap_ids'),'grow':(output.get('grow') or {}).get('id'),'measurement_plan':output.get('measurement_plan')}
def decision_digest(output):return hashlib.sha256(json.dumps(_carrier(output),sort_keys=True,ensure_ascii=False,separators=(',',':')).encode()).hexdigest()
...
class GapLedger:
    ...
    def compile(self,task_ref,sources,edges,targets,policy,actor='agent',persist=True):
        ...
        inputs={'sources':{str(k):list(v or []) for k,v in dict(sources or {}).items()},'edges':[dict(x) for x in edges or []],'targets':[dict(x) for x in targets or []],'policy':dict(policy or {})};output=compile_gap(**inputs);output['decision_digest']=decision_digest(output)
        if not persist:return {**output,'persisted':False}
        parent=self.s.head('global');pe=parent['eid'] if parent else None;ep={'operation':'GAP_COMPILE','task_ref':task_ref,'actor':actor,'closure_count':len(output['closure_nodes']),'gap_ids':output['gap_target_ids'],'grow':(output.get('grow') or {}).get('id'),'decision_digest':output['decision_digest']};eid=event_id('GAP_COMPILE',actor,pe,ep);ed=digest(ep,32);run_id='GAPRUN.'+digest({'eid':eid,'decision':output['decision_digest']},24)
        ...
        self.s.put_event(eid,'GAP_COMPILE',actor,pe,ep,ed);self.s.set_head('global',None,None,eid,ed);return {**output,'persisted':True,'run_id':run_id,'eid':eid}
    def get(self,run_id):
