...
def _edge_id(edge,index):
    return str(edge.get("id") or edge.get("edge_id") or f"edge:{index:05d}")


def _normalize_sources(sources: Mapping[str,Iterable[str]]):
    groups={};origins={}
    for group,values in dict(sources or {}).items():
        ids=sorted({str(v) for v in (values or []) if str(v)})
...
def _admissible(edge,policy):
    ...
    allowed=set(policy.get("traversable_relations") or [])
    defects=[]
...
def compile_gap(sources:Mapping[str,Iterable[str]],edges:Iterable[Mapping[str,Any]],targets:Iterable[Mapping[str,Any]],policy:Mapping[str,Any]):
    policy=dict(policy or {});relations=list(dict.fromkeys(str(x) for x in policy.get("traversable_relations") or []))
    invalid=[r for r in relations if r not in AOR_EDGE_TYPES]
    ...
    groups,origins=_normalize_sources(sources);edge_rows=[];adj={};seen_edge_ids=set()
    for index,raw in enumerate(edges or []):
        edge=dict(raw);eid=_edge_id(edge,index)
        if eid in seen_edge_ids:raise ValueError(f"duplicate edge id {eid}")
        seen_edge_ids.add(eid);src=str(edge.get("src") or "");dst=str(edge.get("dst") or "")
    ...
    for index,raw in enumerate(targets or []):
        ...
        covered=node in reached;score=_residual_score(target) if not covered else {"status":"N/A_COVERED","value":None,"components":{},"defects":[]}
        row={**target,"id":tid,"node":node,"covered":covered,"closure_path":paths.get(node),"residual_score":score};target_rows.append(row)
        if not covered:
            gap.append(row)
            if score["status"]!="KNOWN":measurement.append({"target":tid,"node":node,"defects":score["defects"]})
    ranked=sorted((row for row in gap if row["residual_score"]["status"]=="KNOWN"),key=lambda row:(-float(row["residual_score"]["value"]),row["id"]))
    grow=ranked[0] if ranked else None
    rejected_edges=[{"id":e["id"],"src":e["src"],"dst":e["dst"],"relation":e.get("relation"),"defects":e["defects"]} for e in edge_rows if not e["admissible"]]
    return {
        "closure_kind":"WITNESSED_DIRECTED_REACHABILITY_NOT_LOGICAL_PROOF",
        "policy":policy,
        "source_groups":groups,
        "closure_nodes":sorted(reached),
        "closure_paths":paths,
        "admissible_edge_ids":[e["id"] for e in edge_rows if e["admissible"]],
        "rejected_edges":rejected_edges,
        "targets":target_rows,
        "covered_target_ids":[r["id"] for r in target_rows if r["covered"]],
        "gap_target_ids":[r["id"] for r in gap],
        "gap":gap,
        "ranked_gap_ids":[r["id"] for r in ranked],
        "grow":grow,
        "measurement_plan":measurement,
        "law":"gap = explicit target nodes - witnessed directed reachability closure; grow = max severity*leverage*information_gain/cost among uncovered KNOWN residuals",
        "epistemic_boundary":"reachability is navigation closure only; logical/causal entailment requires separately registered operator semantics",
    }
