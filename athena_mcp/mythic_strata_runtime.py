...
class MythicStrataRuntime:
    def transport(
        ...
        explicit_bridge:Dict[str,Any]|None=None,
    )->Dict[str,Any]:
        ...
        if risk_class in HAZARDS or operation=="HAZARDOUS_EXECUTION":
            return _hold("HOLD_HAZARDOUS_EXECUTION",source,target,operation,
                "Historical/symbolic description cannot authorize hazardous execution.",{"risk_class":risk_class})

        if operation=="AUTHORITY_GRANT":
            return _hold("HOLD_AUTHORITY_MINT",source,target,operation,
                "Runtime/source access cannot mint religious, initiatory, restricted, or practice authority.")

        if operation=="RESTRICTED_INFERENCE" and (
            source.get("authorization_scope")=="PUBLIC" or
            target.get("authorization_scope") in {"INITIATORY","RESTRICTED","MIXED"}
        ):
            return _hold("HOLD_RESTRICTED_INFERENCE",source,target,operation,
                "Public/descriptive material cannot be expanded into restricted or initiatory content.")

        if operation=="HISTORICAL_PROMOTION":
            if source.get("standing")!="PRIMARY_EVIDENCE" or target.get("standing")=="PRIMARY_EVIDENCE":
                return _hold("HOLD_HISTORICAL_PROMOTION",source,target,operation,
                    "Modern, living, traditional, or secondary standing cannot self-promote into primary historical evidence.")

        if operation=="CORPUS_EXHAUSTIVENESS" and source.get("corpus_mutability") in {"OPEN","LIVING","LAYERED","UNKNOWN"}:
            return _hold("HOLD_CORPUS_EXHAUSTIVENESS",source,target,operation,
                "Finite address/code structure does not establish an exhaustive meaning corpus when corpus mutability is not CLOSED.")

        if operation=="CATEGORY_FLATTENING" and (
            source.get("category_scope")=="NATIVE" or str(target_model_class).upper().startswith("GENERIC")
        ):
            return _hold("HOLD_CATEGORY_FLATTENING",source,target,operation,
                "A native religion/tradition cannot be declared interchangeable with a generic magic/category model.")

        if operation=="IDENTITY_EQUIVALENCE" and (
            source.get("category_scope")=="SCHOLARLY_UMBRELLA" or source.get("category_scope")!=target.get("category_scope")
        ):
            return _hold("HOLD_UMBRELLA_OR_CATEGORY_IDENTITY",source,target,operation,
                "A scholarly umbrella or cross-category mapping cannot become native/cultural identity equivalence.")

        if operation=="AUTHORITY_SCOPE_ESCAPE":
            return _hold("HOLD_AUTHORITY_SCOPE_ESCAPE",source,target,operation,
                "Role-, lineage-, grade-, or order-scoped authority cannot become universal authority through transport.")

        if operation=="EPISTEMIC_PROOF":
            return _hold("HOLD_EPISTEMIC_COLLAPSE",source,target,operation,
                "Observation/material evidence does not by itself prove a distinct theory, cosmology, or symbolic interpretation layer.")

        if source.get("standing")=="MODERN_RECONSTRUCTION" and target.get("standing")=="PRIMARY_EVIDENCE":
            return _hold("HOLD_TEMPORAL_BACK_PROJECTION",source,target,operation,
                "Modern reconstruction cannot be transported into a primary historical layer as historical meaning.")

        if cross_layer and operation in EQUIVALENCE_OPS and explicit_bridge is None:
            return _hold("HOLD_EXPLICIT_BRIDGE_REQUIRED",source,target,operation,
                "Cross-layer shared symbol/term does not establish semantic, operator, or identity equivalence.")

        if not cross_layer:
            return {
                "version":STRATA_VERSION,
                "status":"WITHIN_LAYER_ALLOWED",
                "allowed":True,
                "operation":operation,
                "source":deepcopy(source),
                "target":deepcopy(target),
        ...
        if explicit_bridge is None:
            return _hold("HOLD_EXPLICIT_BRIDGE_REQUIRED",source,target,operation,
                "Cross-layer transport requires an explicit source-bearing bridge with invariants and transform loss.")

        bridge=deepcopy(explicit_bridge)
        required=(
            bool(str(bridge.get("source_ref") or "").strip())
            and bool(bridge.get("invariants"))
            and bool(bridge.get("transform_loss"))
            and bool(str(bridge.get("evidence_standing") or "").strip())
        )
        if not required:
            return _hold("HOLD_INCOMPLETE_BRIDGE",source,target,operation,
                "Explicit bridge is missing source_ref, evidence_standing, invariants, or transform_loss.")

        if _STANDING_RANK.get(target.get("standing"),0)>_STANDING_RANK.get(bridge.get("evidence_standing"),0):
            return _hold("HOLD_STANDING_ESCALATION",source,target,operation,
                "Target standing exceeds the bridge evidence standing.",{"bridge":bridge})

        return {
            "version":STRATA_VERSION,
            "status":"BRIDGE_ALLOWED_WITH_LOSS",
            "allowed":True,
            "operation":operation,
            "source":deepcopy(source),
            "target":deepcopy(target),
            "bridge":bridge,
            "identity_equivalence":False,
            "transform_loss":list(bridge["transform_loss"]),
            "invariants":list(bridge["invariants"]),
            "execution_authority":"NONE",
            "authority":bridge.get("authority","SYMBOLIC_ONLY"),
            "mck_v2_promotion":False,
            "laws":list(LAWS),
        }
    ...
    def benchmark(self)->Dict[str,Any]:
        ...
        cases=[
            (L("tarot.game","PRIMARY_EVIDENCE"),L("tarot.occult"),"SEMANTIC_EQUIVALENCE","NONE","","HOLD_EXPLICIT_BRIDGE_REQUIRED"),
            ...
            (L("dao.waidan"),L("dao.neidan"),"OPERATOR_EQUIVALENCE","NONE","","HOLD_EXPLICIT_BRIDGE_REQUIRED"),
        ...
        controls=[]
        a=L("control.same")
        controls.append(self.transport(a,a,"SEMANTIC_TRANSPORT"))
        bridge={
            "source_ref":"source://bridge",
            "evidence_standing":"SECONDARY_SCHOLARSHIP",
            "invariants":["preserve layer identity"],
            "transform_loss":["decoder meaning is not identical across layers"],
        ...
        return {
            "version":STRATA_VERSION,
            "benchmark_kind":"DETERMINISTIC_SYNTHETIC_STRATA_REGRESSION_NOT_CULTURAL_VALIDATION",
            "regression_cases":len(cases),
            "regression_passed":sum(1 for x in outcomes if x["pass"]),
            "illegal_transports_admitted":sum(1 for x in outcomes if x["actual"] in {"WITHIN_LAYER_ALLOWED","BRIDGE_ALLOWED_WITH_LOSS"}),
            "unscoped_reference_admits":len(cases),
            "controls":len(controls),
            "controls_allowed":sum(1 for x in controls if x["allowed"]),
            ...
            "outcomes":outcomes,
            "laws":[
                "UNSCOPED_REFERENCE_MODEL != OBSERVED_V1_FAILURE_RATE",
