...
Frontier V1, the rehydration-loop V1 harness, and successor-baton V1 are explicit
...
from .successor_baton import (
    SUCCESSOR_BATON_TOOLS,
    SUCCESSOR_BATON_TOOL_NAMES,
    SuccessorBatonRuntime,
)
...
for _tool in FRONTIER_TOOLS + REHYDRATION_TOOLS + SUCCESSOR_BATON_TOOLS:
...
if not getattr(PromptRuntime, "_athena_successor_baton_v1_registered", False):
    _prompt_call_without_successor = PromptRuntime.call_tool

    def _prompt_call_with_successor(self, name, arguments):
        if name in SUCCESSOR_BATON_TOOL_NAMES:
            runtime = getattr(self, "_successor_baton_runtime_v1", None)
            if runtime is None:
                loop_runtime = getattr(self, "_rehydration_loop_runtime_v1", None)
                runtime = SuccessorBatonRuntime(self.git, self, loop_runtime)
                self._successor_baton_runtime_v1 = runtime
            return runtime.call_tool(name, arguments)
        return _prompt_call_without_successor(self, name, arguments)

    PromptRuntime.call_tool = _prompt_call_with_successor
    PromptRuntime._athena_successor_baton_v1_registered = True
