...
def _pulse_integrity(pulse: Mapping[str, Any]) -> bool:
    ...
    if not digest:
        return False
    basis = {key: value for key, value in pulse.items() if key != "pulse_digest"}
...
def _normalize_clear_conditions(clear_conditions: Sequence[Any]) -> list[str]:
    """Freeze ordered public clear-condition definitions, never future results."""
...
def _normalize_reward_candidate(candidate: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    eligible = bool(requested and candidate_id and evidence_refs)
    reasons: list[str] = []
    if requested and not candidate_id:
        reasons.append("CANDIDATE_ID_REQUIRED")
    if requested and not evidence_refs:
...
def compile_campaign_v3_life_quest_packet(
    *,
    ...
    """Compile one life-aware Campaign V3 quest packet without executing it."""
    ...
    for value, failure in (
        ...
        (agent_coordinate_name, "AGENT_COORDINATE_NAME_REQUIRED"),
        (quest_id, "QUEST_ID_REQUIRED"),
        (quest_version, "QUEST_VERSION_REQUIRED"),
    ):
        if not value:
    ...
    if not isinstance(reseed_anchor, Mapping):
        ...
        if reseed_anchor.get("platform_counter_reset_claimed") is not False:
            anchor_errors.append("platform_counter_reset_claimed_must_be_false")
    if anchor_errors:
    ...
    if isinstance(reseed_anchor, Mapping) and not anchor_errors:
        try:
            ...
            target_versions = {}
        if target_versions.get("campaign_v3.pulse_digest") != str(pulse.get("pulse_digest") or ""):
    ...
    anchor_digest = _anchor_digest(reseed_anchor)
    packet = {
        "artifact": ARTIFACT,
        "status": "COMPILED",
        "campaign": {
    ...
    packet["packet_digest"] = _sha({key: value for key, value in packet.items() if key != "packet_digest"})
    return packet
...
def validate_campaign_v3_life_quest_packet(packet: Mapping[str, Any]) -> list[str]:
    ...
    if packet.get("LIFE_POLICY") != _life_policy():
    ...
    if not isinstance(anchor, Mapping):
        ...
        if packet.get("RESEED_ANCHOR_DIGEST") != _anchor_digest(anchor):
            errors.append("reseed_anchor_digest")
        try:
            anchor_errors = list(validate_campaign_v3_reseed_anchor(anchor))
        except Exception as exc:
            anchor_errors = [f"validator_exception:{type(exc).__name__}"]
        if anchor_errors:
        ...
        if isinstance(campaign, Mapping) and isinstance(pulse_binding, Mapping):
            try:
                targets = _target_version_map(anchor)
            except ValueError as exc:
            ...
            if str(anchor.get("agent_coordinate_name") or "") != str(campaign.get("agent_coordinate_name") or ""):
    ...
    expected_digest = _sha({key: value for key, value in packet.items() if key != "packet_digest"})
