...
_RESOURCE_URIS = {item["uri"] for item in HUB_RESOURCES + REGISTRY_RESOURCES + POLYATLAS_RESOURCES}
...
def _resource_value(server: Any, uri: str) -> Any:
    ...
    if uri == "athena://kc144/hub/readiness":
        value = server._remove_discharged_field_blocker(hub.readiness())
        value["polyatlas"] = polyatlas_status()
        return value
...
AGENT={agent}
TASK={task}
1 HYDRATE athena://kc144/hub, athena://kc144/registry/status, and athena://kc144/polyatlas/status before acting. Preserve GID!=object!=coordinate!=claim!=evidence!=authority and adjacency!=bridge.
...
5 ROUTE only over typed executable graph layers. Use the C01..C16 polyatlas and exact odd-resolution transport when changing charts; do not round or equate projections with semantic identity. Source-bounded graph records may be queried, but unavailable executable edges must not be fabricated.
...
10 VALIDATE with athena_kc144_hub_validate, athena_kc144_registry_verify, athena_kc144_polyatlas_validate and the existing surface/composition/smoke suites. Do not self-promote.
...
def handle(server: Any, message: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if method == "resources/list":
        ...
        resources.extend(item for item in HUB_RESOURCES + REGISTRY_RESOURCES + POLYATLAS_RESOURCES if item["uri"] not in seen)
    ...
    if method == "resources/read" and params.get("uri") == "athena://manifest":
        ...
        if result and "result" in result:
            ...
            for layer in ("KC144_TOPOLOGICAL_COMMAND_HUB", "KC144_AUTHORITATIVE_REGISTRY_PACK", "KC144_16_COORDINATE_POLYATLAS", "FIELD1_CANDIDATE_ASSEMBLER"):
