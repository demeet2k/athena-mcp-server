...
from .project_atlas_query_index import build_query_index, lookup_identifier, seed_list_candidates, typed_mcp_identifier
...
class ProjectAtlasSurface:
    ...
    def _hold(self, status: str, **extra):
        return {"status":status,"surface_version":PROJECT_ATLAS_SURFACE_VERSION,"authority":"NONE","laws":list(PROJECT_ATLAS_LAWS),**extra}
    ...
    def _invalidate_cache(self):
        self._cache_key=None;self._cache_head=None;self._cache_runtime_head=None;self._cache_surface_signature=None;self._cache_snapshot_id=None;self._cache=None

    @staticmethod
    def _record_entries(configured_atlas: dict, runtime_atlas: dict | None, runtime_is_configured: bool, mcp_surface: dict):
        rows=[("configured_git",r) for r in configured_atlas.get("records",[])]
        if runtime_atlas is not None and not runtime_is_configured:rows.extend(("runtime_git",r) for r in runtime_atlas.get("records",[]))
        rows.extend(("mcp",r) for r in mcp_surface.get("records",[]))
        return rows

    @staticmethod
    def _snapshot_identity(configured_atlas: dict, runtime_atlas: dict | None, runtime_provenance: dict, mcp_surface: dict, federation: dict, surface_signature: str, query_index: dict):
        configured_repo=configured_atlas["repository"];runtime_repo=runtime_atlas["repository"] if runtime_atlas is not None else None
        basis={
            "schema":"ATHENA.KC144.FEDERATED_RUNTIME_PROJECT_ATLAS.V2",
            "configured":{"repo":configured_repo["repo_key"],"head":configured_repo["head"],"tree":configured_repo["tree"],"atlas_digest":configured_atlas["atlas_digest"]},
            "runtime_provenance":{k:runtime_provenance.get(k) for k in ("status","mode","repo_key","head","attestation_level")},
            "runtime_git":None if runtime_repo is None else {"repo":runtime_repo["repo_key"],"head":runtime_repo["head"],"tree":runtime_repo["tree"],"atlas_digest":runtime_atlas["atlas_digest"]},
            ...
            "query_index_digest":query_index.get("digest"),
    ...
    def _compile_snapshot(self, configured_head: str, runtime_before: dict, surface_signature: str) -> dict:
        ...
        runtime_atlas=None;runtime_is_configured=False
        if runtime_before.get("status")=="RESOLVED":
            runtime_head=runtime_before["head"];runtime_repo=runtime_before["repo_key"];runtime_root=runtime_before.get("root")
            if runtime_root:
                root=Path(runtime_root).resolve()
                if root==Path(self.server.git.root).resolve() and runtime_head==configured_head:runtime_atlas=configured_atlas;runtime_is_configured=True
                else:runtime_atlas=compile_git_atlas(root,ref=runtime_head,include_trees=True)
                runtime_repo=runtime_atlas["repository"]["repo_key"];runtime_head=runtime_atlas["repository"]["head"]
            from .protocol import PROMPTS, SERVER_INFO, TOOLS
            mcp_surface=mcp_surface_atlas(repo_key=runtime_repo,head=runtime_head,server_name=SERVER_INFO.get("name","athena-canonical-mcp"),tools=TOOLS,prompts=PROMPTS)
        else:
            mcp_surface={"schema":"ATHENA.KC144.MCP_SURFACE_ATLAS.v1","status":"HOLD_RUNTIME_PROVENANCE","reason":runtime_before.get("reason"),"repo_key":runtime_before.get("repo_key"),"head":runtime_before.get("head"),"server":None,"count":0,"records":[],"surface_digest":None}
        ...
        query_index=build_query_index(self._record_entries(configured_atlas,runtime_atlas,runtime_is_configured,mcp_surface))
        snapshot_id,snapshot_basis=self._snapshot_identity(configured_atlas,runtime_atlas,runtime_before,mcp_surface,federation,surface_signature,query_index)
        ...
        return {
            "schema":"ATHENA.KC144.FEDERATED_RUNTIME_PROJECT_ATLAS.V2","status":standing,"snapshot_id":snapshot_id,"snapshot_basis":snapshot_basis,
            "configured_git":configured_atlas,"runtime_git":runtime_atlas,"runtime_git_is_configured":runtime_is_configured,
            "runtime_provenance":dict(runtime_before),"runtime_tree_available":runtime_atlas is not None,"mcp_surface":mcp_surface,
            "federation":federation,"surface_signature":surface_signature,"query_index":query_index,
        }

    def _snapshot(self, expected_head=None, expected_runtime_head=None):
        git=getattr(self.server,"git",None)
        if git is None or not git.enabled:return None,self._hold("HOLD_GIT_UNAVAILABLE",reason="Project Atlas query surface requires a configured ATHENA_GIT_ROOT checkout.",configured_head=None,runtime_head=None),None
        expected=str(expected_head).lower().strip() if expected_head else None;expected_runtime=str(expected_runtime_head).lower().strip() if expected_runtime_head else None
        for _ in range(2):
            ...
            if expected is not None and configured_head!=expected:
                return None,self._hold("HOLD_STALE_CONFIGURED_HEAD",expected_head=expected,current_head=configured_head,current_runtime_head=runtime_before.get("head"),configured_head=configured_head,runtime_head=runtime_before.get("head"),reason="Expected configured Git head does not match ATHENA_GIT_ROOT."),configured_before
            if expected_runtime is not None:
                if runtime_before.get("status")!="RESOLVED":return None,self._hold("HOLD_RUNTIME_PROVENANCE",expected_runtime_head=expected_runtime,configured_head=configured_head,runtime_head=runtime_before.get("head"),runtime_provenance=runtime_before,reason="Cannot validate expected runtime head without exact runtime-source provenance."),configured_before
                if runtime_before.get("head")!=expected_runtime:return None,self._hold("HOLD_STALE_RUNTIME_HEAD",expected_runtime_head=expected_runtime,current_runtime_head=runtime_before.get("head"),configured_head=configured_head,runtime_head=runtime_before.get("head"),runtime_provenance=runtime_before),configured_before
            ...
            if stable:
                self._cache_key=cache_key;self._cache_head=configured_head;self._cache_runtime_head=runtime_before.get("head");self._cache_surface_signature=surface_signature;self._cache_snapshot_id=snapshot["snapshot_id"];self._cache=snapshot
                observation={"configured_git":{"branch":configured_after.get("branch"),"head":configured_head,"dirty":bool(configured_after.get("dirty"))},"runtime_source":dict(runtime_after),"mcp_surface_signature":surface_signature}
    ...
    def _summary_record(source: str, rec: dict) -> dict:
        ...
        out={"source":source,"poid":rec["poid"],"address":rec["address"],"native":{"repo":native.get("repo"),"ref":native.get("ref"),"head":native.get("head"),"tree":native.get("tree"),"path":native.get("path"),"object_sha":native.get("object_sha"),"git_type":native.get("git_type"),"mode":native.get("mode")},"project_kc144":{"gid":project["gid"],"sid":project["sid"],"row":project["row"],"col":project["col"],"row_semantic":project.get("row_semantic"),"col_semantic":project.get("col_semantic")},"kc144_reference":{"gid":reference["gid"],"sid":reference["sid"],"row":reference["row"],"col":reference["col"]},"return":rec["return"]}
        if rec.get("mcp") is not None:out["mcp"]=dict(rec["mcp"])
        if rec.get("navigation") is not None:out["navigation"]={"parent_path":rec["navigation"].get("parent_path"),"station_ordinal":rec["navigation"].get("station_ordinal"),"station_population":rec["navigation"].get("station_population")}
        return out

    @staticmethod
    def _all_records(snapshot: dict):
        return list(snapshot["query_index"]["entries"])
    ...
    def _strong_identifier(identifier: str, source: str, rec: dict) -> bool:
        ...
        typed=typed_mcp_identifier(ident);mcp=rec.get("mcp")
        return bool(source=="mcp" and typed and mcp and typed==(mcp.get("kind"),mcp.get("name")))

    def _resolve_in_snapshot(self, snapshot: dict, identifier: str) -> dict:
        ident=str(identifier);matches,lookup_mode=lookup_identifier(snapshot["query_index"],ident)
        ...
        if not matches:
            if runtime_status!="RESOLVED":return self._hold("HOLD_RUNTIME_PROVENANCE",identifier=ident,lookup_mode=lookup_mode,snapshot_id=snapshot_id,configured_head=configured_head,runtime_head=runtime_head,runtime_provenance=snapshot.get("runtime_provenance"),reason="Identifier was not found in configured Git, but runtime/MCP provenance is incomplete.")
            if not tree_available and lookup_mode=="OPEN_WORLD":return self._hold("HOLD_RUNTIME_TREE_UNAVAILABLE",identifier=ident,lookup_mode=lookup_mode,snapshot_id=snapshot_id,configured_head=configured_head,runtime_head=runtime_head,runtime_provenance=snapshot.get("runtime_provenance"),reason="Runtime head is known but its Git tree is not enumerated; UNKNOWN_RUNTIME_TREE != EMPTY_RUNTIME_TREE.")
            return self._hold("HOLD_NOT_FOUND",identifier=ident,lookup_mode=lookup_mode,snapshot_id=snapshot_id,configured_head=configured_head,runtime_head=runtime_head,candidates=[])
        if len(matches)>1:return self._hold("HOLD_AMBIGUOUS",identifier=ident,lookup_mode=lookup_mode,snapshot_id=snapshot_id,configured_head=configured_head,runtime_head=runtime_head,candidate_count=len(matches),candidates=[self._summary_record(source,rec) for source,rec in matches[:20]])
        source,rec=matches[0]
        if runtime_status=="RESOLVED" and not tree_available and not self._strong_identifier(ident,source,rec):return self._hold("HOLD_RUNTIME_TREE_UNAVAILABLE",identifier=ident,lookup_mode=lookup_mode,snapshot_id=snapshot_id,configured_head=configured_head,runtime_head=runtime_head,candidate=self._summary_record(source,rec),runtime_provenance=snapshot.get("runtime_provenance"),reason="One visible candidate exists, but an unenumerated runtime Git tree could contain an equal unscoped identifier; use a POID/address/native RETURN or typed MCP locator.")
        return {"status":"RESOLVED","surface_version":PROJECT_ATLAS_SURFACE_VERSION,"authority":"NONE","snapshot_id":snapshot_id,"lookup_mode":lookup_mode,"configured_head":configured_head,"runtime_head":runtime_head,"runtime_provenance":snapshot.get("runtime_provenance"),"identifier":ident,"record":self._summary_record(source,rec),"full_record":rec,"laws":list(PROJECT_ATLAS_LAWS)}
    ...
    def list_records(self, args: dict):
        ...
        if source_filter=="runtime_git" and snapshot.get("runtime_git") is None:return self._hold("HOLD_RUNTIME_TREE_UNAVAILABLE",snapshot_id=snapshot_id,configured_head=snapshot["configured_git"]["repository"]["head"],runtime_head=rp.get("head"),runtime_provenance=rp,reason="Runtime repository/head is known but the runtime Git tree is not available for enumeration.")
        ...
        poid_prefix=args.get("poid_prefix");seed,seed_mode=seed_list_candidates(snapshot["query_index"],args,bool(snapshot.get("runtime_git_is_configured")));rows=[]
        for source,rec in seed:
    ...
    def route(self, args: dict):
        ...
        route=route_records(src["full_record"],dst["full_record"],wrap=bool(args.get("wrap",False)));s_native=src["full_record"]["native"];d_native=dst["full_record"]["native"];same_repo=s_native.get("repo")==d_native.get("repo");same_head=s_native.get("head")==d_native.get("head")
        ...
        if route["cross_frontier"]:route["federation_transition"]={"federation_digest":snapshot.get("federation",{}).get("federation_digest"),"src":{"repo":s_native.get("repo"),"head":s_native.get("head")},"dst":{"repo":d_native.get("repo"),"head":d_native.get("head")},"law":"CROSS_REPO_ROUTE_REQUIRES_EXACT_REPO_HEAD"}
        return {"status":"ROUTED","surface_version":PROJECT_ATLAS_SURFACE_VERSION,"authority":"NONE","snapshot_id":snapshot_id,"configured_head":snapshot["configured_git"]["repository"]["head"],"runtime_head":rp.get("head"),"runtime_provenance":rp,"query_index_digest":snapshot["query_index"]["digest"],"src":src["record"],"dst":dst["record"],"route":route,"laws":list(PROJECT_ATLAS_LAWS)}
