...
def _decision_carrier(output: Mapping[str,Any]):
    return {
        "selected_ids":output.get("selected_ids",[]),
        "solver":output.get("solver"),
        "optimality":output.get("optimality"),
...
def _decision_digest(output: Mapping[str,Any]):
    raw=json.dumps(_decision_carrier(output),sort_keys=True,ensure_ascii=False,separators=(",",":"))
...
class RetrievalLedger:
    def __init__(self,core):
        ...
        self.core.register("TOOL","RETRIEVAL","COMPILE","DECISION_CONTEXT","RAG1_COVERAGE_EQ_SAFE",{"query":"typed","candidates":"supplied provenance records","eq_snapshot":"optional"},{"selected":"source refs","coverage":"typed","decision_digest":"sha256"},actor="GENESIS.RAG.1",status="CANONICAL")
    ...
    def compile(self,query_ref:str,query:Mapping[str,Any],candidates:Iterable[Mapping[str,Any]],eq_snapshot:Optional[Mapping[str,Any]]=None,actor:str="agent",task:str="",persist:bool=True):
        ...
        inputs={"query":dict(query or {}),"candidates":[dict(x) for x in candidates],"eq_snapshot":dict(eq_snapshot or {}) if eq_snapshot else None}
        output=compile_selection(**inputs);output["law"]=retrieval_law();output["decision_digest"]=_decision_digest(output)
        if not persist:return {**output,"persisted":False}
        parent=self.s.head("global");pe=parent["eid"] if parent else None
        ep={"operation":"RAG_COMPILE","query_ref":query_ref,"actor":actor,"task":task,"selected_ids":output["selected_ids"],"solver":output["solver"],"optimality":output["optimality"],"decision_digest":output["decision_digest"],"coverage":output["coverage"]}
        eid=event_id("RAG_COMPILE",actor,pe,ep);ed=digest(ep,32);run_id="RAGRUN."+digest({"eid":eid,"decision":output["decision_digest"]},24)
        ...
        self.s.put_event(eid,"RAG_COMPILE",actor,pe,ep,ed);self.s.set_head("global",None,None,eid,ed)
