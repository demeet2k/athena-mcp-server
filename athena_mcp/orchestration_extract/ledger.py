...
CREATE TABLE IF NOT EXISTS extraction_runs(
 run_id TEXT PRIMARY KEY,seed_ref TEXT NOT NULL,seed_json TEXT NOT NULL,max_depth INTEGER NOT NULL,max_tasks_per_generation INTEGER NOT NULL,actor TEXT NOT NULL,start_eid TEXT NOT NULL,created_at REAL NOT NULL);
CREATE TABLE IF NOT EXISTS extraction_tasks(
 task_id TEXT PRIMARY KEY,run_id TEXT NOT NULL,parent_task_id TEXT,seed_ref TEXT NOT NULL,seed_json TEXT NOT NULL,transform TEXT NOT NULL,depth INTEGER NOT NULL,ordinal INTEGER NOT NULL,status TEXT NOT NULL,result_refs_json TEXT NOT NULL,last_eid TEXT NOT NULL,created_at REAL NOT NULL,updated_at REAL NOT NULL);
CREATE TABLE IF NOT EXISTS extraction_results(
 result_id TEXT PRIMARY KEY,task_id TEXT NOT NULL,run_id TEXT NOT NULL,output_index INTEGER NOT NULL,payload_json TEXT NOT NULL,witness_json TEXT NOT NULL,eid TEXT NOT NULL,created_at REAL NOT NULL);
...
class ExtractionLedger:
    def __init__(self,core):
        ...
        self.core.register('TOOL','DEVELOPMENT','PLAN','SEED_TRANSFORM_FRONTIER','AOR3_WITNESSED_EXTRACTION',{'seed':'payload','transforms':'typed bank','depth':'bounded'},{'tasks':'planned','results':'witnessed','recursive_frontier':'bounded'},actor='GENESIS.AOR.SX1',status='CANONICAL')
    def _event(self,operation,payload,actor):
        parent=self.s.head('global');pe=parent['eid'] if parent else None;eid=event_id(f'EXTRACTION_{operation}',actor,pe,payload);ed=digest(payload,32);self.s.put_event(eid,f'EXTRACTION_{operation}',actor,pe,payload,ed);self.s.set_head('global',None,None,eid,ed);return eid
    def plan(self,seed_ref:str,seed:Any,transforms:Optional[Iterable[str]]=None,max_depth:int=1,max_tasks_per_generation:int=16,actor:str='agent'):
        ...
        chosen=list(transforms or TRANSFORM_ORDER);unknown=[n for n in chosen if n not in TRANSFORM_SPECS]
        if unknown:raise ValueError(f'unknown transforms {unknown}')
        chosen=list(dict.fromkeys(chosen))[:max_tasks_per_generation];payload={'operation':'PLAN','seed_ref':seed_ref,'transforms':chosen,'max_depth':max_depth,'max_tasks_per_generation':max_tasks_per_generation};start_eid=self._event('PLAN',payload,actor);run_id='EXTRUN.'+digest({'seed_ref':seed_ref,'seed':seed,'eid':start_eid},24)
        ...
        tasks=self._create_tasks(run_id,None,seed_ref,seed,chosen,0,actor);manifest=transform_manifest();return {'run_id':run_id,'seed_ref':seed_ref,'max_depth':max_depth,'max_tasks_per_generation':max_tasks_per_generation,'tasks':tasks,'transform_manifest':{n:manifest[n] for n in chosen}}
    def _create_tasks(self,run_id,parent_task_id,seed_ref,seed,transforms,depth,actor):
        ...
        for ordinal,name in enumerate(transforms):
            task_id='EXTTASK.'+digest({'run':run_id,'parent':parent_task_id,'seed_ref':seed_ref,'transform':name,'depth':depth,'ordinal':ordinal},24);payload={'operation':'TASK_CREATE','task_id':task_id,'run_id':run_id,'parent_task_id':parent_task_id,'seed_ref':seed_ref,'transform':name,'depth':depth,'ordinal':ordinal};eid=self._event('TASK_CREATE',payload,actor)
    ...
    def complete(self,task_id,outputs,witness,actor='agent'):
        row=self.s.one('SELECT * FROM extraction_tasks WHERE task_id=?',(task_id,))
        ...
        with self.s.db:
            for index,output in enumerate(outputs):
                result_id='EXTRES.'+digest({'task':task_id,'index':index,'payload':output,'eid':eid},24);result_refs.append(result_id);self.s.db.execute('INSERT INTO extraction_results VALUES(?,?,?,?,?,?,?,?)',(result_id,task_id,row['run_id'],index,json.dumps(output,sort_keys=True,ensure_ascii=False),json.dumps(dict(witness),sort_keys=True),eid,time.time()))
    ...
    def fail(self,task_id,reason,witness,actor='agent'):
        row=self.s.one('SELECT * FROM extraction_tasks WHERE task_id=?',(task_id,))
        if not row:raise KeyError('unknown extraction task')
    ...
    def expand_result(self,result_id,transforms=None,actor='agent'):
        ...
        chosen=list(transforms or TRANSFORM_ORDER);unknown=[n for n in chosen if n not in TRANSFORM_SPECS]
        if unknown:raise ValueError(f'unknown transforms {unknown}')
        chosen=list(dict.fromkeys(chosen))[:int(run['max_tasks_per_generation'])];tasks=self._create_tasks(result['run_id'],result['task_id'],result_id,result['payload'],chosen,depth,actor);return {'run_id':result['run_id'],'result_id':result_id,'status':'EXPANDED','depth':depth,'tasks':tasks}
    def frontier(self,run_id):return self.s.rows("SELECT task_id,run_id,parent_task_id,seed_ref,transform,depth,ordinal,status,last_eid,created_at,updated_at FROM extraction_tasks WHERE run_id=? AND status='PLANNED' ORDER BY depth,ordinal,task_id",(run_id,))
    def run(self,run_id):
    ...
    def benchmark(self):
        q=lambda table:self.s.one(f'SELECT COUNT(*) n FROM {table}')["n"];statuses={r['status']:r['n'] for r in self.s.rows('SELECT status,COUNT(*) n FROM extraction_tasks GROUP BY status')};return {'extraction_runs':q('extraction_runs'),'extraction_tasks':q('extraction_tasks'),'extraction_results':q('extraction_results'),'extraction_status':statuses}
