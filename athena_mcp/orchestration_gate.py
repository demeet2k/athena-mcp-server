...
def test_gate(candidate:Mapping[str,Any])->Dict[str,Any]:
    test=candidate.get('test') or {};required=('procedure','observation','result','witness');missing=[n for n in required if not _present(test.get(n))];claimed=bool(test.get('claimed',False))
...
def persistence_gate(candidate:Mapping[str,Any])->Dict[str,Any]:
    tx=candidate.get('transaction') or {};claimed=bool(tx.get('persisted',False));required=('commit','receipt','verify');missing=[n for n in required if not _present(tx.get(n))]
    return {'claimed':claimed,'status':'PASS' if not missing else ('FAIL' if claimed else 'INCOMPLETE'),'missing':missing,'promotion_allowed':not claimed or not missing}

def coordinate_gate(candidate:Mapping[str,Any])->Dict[str,Any]:
    coords=candidate.get('coordinates') or {};required=('kc144','graph','lineage','semantic','time');missing=[n for n in required if not _present(coords.get(n))];require=bool(candidate.get('require_coordinates',False))
...
def promotion_gate(candidate:Mapping[str,Any])->Dict[str,Any]:
    gates={'test':test_gate(candidate),'persistence':persistence_gate(candidate),'coordinates':coordinate_gate(candidate),'evidence':evidence_gate(candidate),'lifecycle':lifecycle_gate(candidate)};blocked=[n for n,r in gates.items() if not r['promotion_allowed']]
    return {'status':'PASS' if not blocked else 'BLOCKED','blocked_by':blocked,'gates':gates}
