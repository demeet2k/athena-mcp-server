...
def _carrier(output:Mapping[str,Any]):
    return {
        'version':output.get('version'),'seed_ref':output.get('seed_ref'),'ecosystem':output.get('ecosystem'),
        'candidate_ids':output.get('candidate_ids'),'candidates':output.get('candidates'),
        'unmeasured_candidate_ids':output.get('unmeasured_candidate_ids'),'conflict_candidate_ids':output.get('conflict_candidate_ids'),
        'field_edges':output.get('field_edges'),'module_presence':output.get('module_presence'),
    }
...
class FieldLedger:
    def __init__(self,core):
        ...
        self.core.register('TOOL','DEVELOPMENT','ASSEMBLE','CANDIDATE_FIELD','FIELD1_PROVENANCE_ACTIONS',
    ...
    def compile(self,seed_ref,module_outputs,explicit_candidates=None,ecosystem=None,actor='agent',persist=True):
        inputs={
            'seed_ref':str(seed_ref),'module_outputs':dict(module_outputs or {}),
            'explicit_candidates':[dict(x) for x in (explicit_candidates or [])],
            'ecosystem':dict(ecosystem or {}),
        }
        output=build_field(**inputs);fd=field_digest(output);output['field_digest']=fd
        if not persist:return {**output,'persisted':False}
        parent=self.s.head('global');pe=parent['eid'] if parent else None
        payload={'operation':'FIELD_COMPILE','seed_ref':seed_ref,'actor':actor,'candidate_ids':output['candidate_ids'],'unmeasured':output['unmeasured_candidate_ids'],'conflicts':output['conflict_candidate_ids'],'field_digest':fd}
        eid=event_id('FIELD_COMPILE',actor,pe,payload);ed=digest(payload,32);run_id='FIELDRUN.'+digest({'eid':eid,'field_digest':fd},24)
        with self.s.db:self.s.db.execute('INSERT INTO field_runs VALUES(?,?,?,?,?,?,?,?)',(
        ...
        self.s.put_event(eid,'FIELD_COMPILE',actor,pe,payload,ed);self.s.set_head('global',None,None,eid,ed)
    ...
    def replay(self,run_id):
        ...
        return {'run_id':run_id,'status':'REPLAY_MATCH' if match else 'REPLAY_DIVERGED','match':match,
            'stored_field_digest':stored['field_digest'],'recomputed_field_digest':now,
            'stored_candidate_ids':stored['output'].get('candidate_ids',[]),'recomputed_candidate_ids':recomputed.get('candidate_ids',[]),
            'stored_edges':stored['output'].get('field_edges',[]),'recomputed_edges':recomputed.get('field_edges',[]),
            'stored_conflicts':stored['output'].get('conflict_candidate_ids',[]),'recomputed_conflicts':recomputed.get('conflict_candidate_ids',[])}
