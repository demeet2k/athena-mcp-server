...
ROUTING_METADATA_FIELDS={"claim_id","min_authority","resolved","branch_id","requires","require_coordinates","coordinates"}
...
def _proposal(kind,operation,target_ref,payload=None,source_refs=None,dependencies=None,explicit_fields=None):
    kind=str(kind or "").upper();operation=str(operation or "").strip();target_ref=str(target_ref or "").strip()
    ...
    if explicit_fields:out.update(dict(explicit_fields))
    return out
    ...
    for raw in proposals:
        ...
        if both_explicit:
            ...
            if metric_conflict:
                ...
                # Once measurements disagree, do not retain a cherry-picked
                # subset as though the packet were still safely rankable.
        ...
        # An UNMEASURED duplicate never overwrites explicit measurements.
...
def _from_branches(branches):
    ...
    for branch in branches or []:
        ...
        p=_proposal("REVIEW","review_branch_reactivation",ref,{"basis_id":branch.get("basis_id"),"ewma_reward":branch.get("ewma_reward",branch.get("ewma")),"last_trigger_ref":branch.get("last_trigger_ref")},[ref,branch.get("last_eid")]);p["field_origin"]=["BRANCH_EVOLUTION"];out.append(p)
    return out
...
def _from_aor(aor):
    ...
    for item in aor.get("measurement_plan") or []:
        ...
        p=_proposal("MEASURE","measure_aor_candidate",target,{"defects":item.get("defects") or item.get("missing_metrics") or item.get("missing") or []},[run_ref]);p["field_origin"]=["AOR.3"];out.append(p)
    for item in aor.get("calibration_plan") or aor.get("calibration_frontier") or []:
        target=item.get("candidate") or item.get("id")
        p=_proposal("CALIBRATE","calibrate_aor_candidate",target,{"metrics":item.get("metrics") or item.get("defects") or []},[run_ref]);p["field_origin"]=["AOR.3"];out.append(p)
    return out
...
def _from_explicit(explicit):
    ...
    for item0 in explicit or []:
        item=dict(item0);fields={k:v for k,v in item.items() if k not in reserved}
        ...
        p["field_origin"]=sorted({str(x) for x in (item.get("field_origin") or ["EXPLICIT"]) if str(x)});out.append(p)
    return out
...
def build_field(seed_ref:str,module_outputs:Mapping[str,Any],explicit_candidates:Iterable[Mapping[str,Any]]=(),ecosystem:Mapping[str,Any]|None=None):
    ...
    for candidate in candidates:
        for src in candidate["source_refs"]:edges.append({"src":src,"relation":"proposes","dst":candidate["id"]})
        for dep in candidate["dependencies"]:edges.append({"src":candidate["id"],"relation":"depends","dst":dep})
    ...
    return {
        "version":"FIELD.1","seed_ref":seed_ref,"ecosystem":dict(ecosystem or {}),
        "candidates":candidates,"candidate_ids":[c["id"] for c in candidates],
        "unmeasured_candidate_ids":unmeasured,"conflict_candidate_ids":conflicts,
        "exact_signature_merges":exact_merges,"field_edges":edges,
