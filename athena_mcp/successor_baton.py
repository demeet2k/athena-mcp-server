...
class SuccessorBatonRuntime:
    """Content-addressed delta projection over a verified rehydration receipt chain.

    Batons do not create Git commits. They are deterministic projections of the
    already-durable rehydration state/receipt/prompt chain, preventing a baton-only
    write from being mistaken for substantive work by the parent loop.
    """
    ...
    def _prompt_metadata(self, path: Path) -> dict:
        text = path.read_text(encoding="utf-8")
        match = _PROMPT_META.search(text)
        if not match:
    ...
    def _latest_transition(self, loop_id: str) -> dict:
        ...
        previous_prompt_path = self.loop._loop_prompt_path(paths, f"{paths['prompts']}/{step - 1:04d}.md", f"{step - 1:04d}.md")
        current_prompt_path = self.loop._loop_prompt_path(paths, receipt["prompt_path"], f"{step:04d}.md")
        previous_meta = self._prompt_metadata(previous_prompt_path)
        current_meta = self._prompt_metadata(current_prompt_path)
        ...
        exact_tip = bool(checkpoint_head and current_git_head == checkpoint_head)
        ...
        for raw in receipt.get("material_work_paths") or []:
            path = str(raw)
            cones.setdefault(_cone_for_path(path), []).append(path)
        for key in cones:
            cones[key] = sorted(set(cones[key]))
        if prompt_changed:
            cones.setdefault("prompt_policy", [])
        if frontier_changed:
            cones.setdefault("frontier", [])
        if receipt.get("completion", {}).get("evidence_refs"):
            cones.setdefault("evidence", [])
        if receipt.get("completion", {}).get("residuals"):
            cones.setdefault("residual", [])

        if not content_delta_covered:
            mode = "FULL_REHYDRATE_REQUIRED"
            required = ["full_rehydration"]
        elif not exact_tip:
            mode = "HEAD_MOVED_REHYDRATE_REQUIRED"
            required = ["shared_git", "loop_checkpoint", "full_rehydration"]
        elif prompt_changed and frontier_changed:
            mode = "PROMPT_AND_FRONTIER_CONE"
        ...
        core = {
            ...
            "transition": {
                ...
                "completion_summary": (receipt.get("completion") or {}).get("summary"),
                "tests": (receipt.get("completion") or {}).get("tests") or [],
                "evidence_refs": (receipt.get("completion") or {}).get("evidence_refs") or [],
                "residuals": (receipt.get("completion") or {}).get("residuals") or [],
                "next_task": (receipt.get("completion") or {}).get("next_task") or state.get("task"),
                "handoff_to": (receipt.get("completion") or {}).get("handoff_to"),
            ...
            "affected_cone": {
                "components": sorted(cones),
                "paths": cones,
            },
            "hydration": {
                "mode": mode,
                "required": required,
                "content_delta_covered": content_delta_covered,
                "exact_loop_tip": exact_tip,
                "frontier_interpretation_refresh_required_before_scheduler_action": state.get("source", {}).get("use_frontier", True),
            },
            "coverage": coverage,
            "goal": state.get("goal"),
            "task": state.get("task"),
        ...
        baton_digest = _sha(core)
        return {
            "status": "BATON_READY" if mode not in {"FULL_REHYDRATE_REQUIRED", "HEAD_MOVED_REHYDRATE_REQUIRED"} else mode,
            "baton": {**core, "baton_digest": baton_digest},
            "baton_digest": baton_digest,
            "verification": verification,
            "observation": {
                "current_git_head": current_git_head,
                "checkpoint_head": checkpoint_head,
    ...
    def derive(self, loop_id: str) -> dict:
        return self._latest_transition(loop_id)
    ...
    def _compact_successor_prompt(self, baton: dict) -> str:
        transition = baton["transition"]
        delta = baton["coordinate_delta"]
        cone = baton["affected_cone"]
        return f"""# ATHENA Successor Delta Baton — {baton['loop_id']} / step {baton['step_index']}

Baton digest: `{baton['baton_digest']}`
...
## Affected cone
{json.dumps(cone, indent=2, sort_keys=True)}

## Hydration law
Hydrate only the required cone declared by the verified baton. If Git head, prompt digest, frontier digest, or baton digest no longer matches, stop and rehydrate rather than extrapolating stale state.

`BATON != AUTHORITY`; execute only with current tool/repository/user authority.
"""
    ...
    def consume(
        ...
        *,
        ...
        initial = self.derive(loop_id)
        if initial.get("baton_digest") != expected_baton_digest:
            return {
                "status": "STALE_BATON_HOLD",
                "expected_baton_digest": expected_baton_digest,
                "current_baton_digest": initial.get("baton_digest"),
                "detail": initial,
+                "durable_return": False,
        ...
        refreshed = self.derive(loop_id)
        if refreshed.get("baton_digest") != expected_baton_digest:
            return {
                "status": "STALE_BATON_AFTER_SYNC_HOLD",
                "expected_baton_digest": expected_baton_digest,
                "current_baton_digest": refreshed.get("baton_digest"),
                "remote_sync": remote_sync,
                "detail": refreshed,
                "durable_return": False,
            }
        if refreshed.get("status") != "BATON_READY":
            fallback = self.loop.resume(loop_id, include_prompt=True)
            return {
                "status": refreshed.get("status"),
                "baton_digest": expected_baton_digest,
                "baton": refreshed.get("baton"),
                "fallback": fallback,
                "remote_sync": remote_sync,
                "durable_return": False,
            }

        baton = refreshed["baton"]
        state, _ = self.loop._read_state(loop_id)
        context = self.loop._context(
            task=str((baton.get("transition") or {}).get("next_task") or state.get("task") or ""),
        ...
        live_prompt_digest = (context.get("prompt") or {}).get("prompt_stack_digest")
        live_frontier_digest = (context.get("frontier") or {}).get("frontier_digest")
        persisted_prompt_digest = baton["successor"].get("prompt_stack_digest")
        persisted_frontier_digest = baton["successor"].get("frontier_digest")
        live_drift = {
            "prompt_stack": live_prompt_digest != persisted_prompt_digest,
            "frontier": live_frontier_digest != persisted_frontier_digest,
        }
        if any(live_drift.values()):
            return {
                "status": "LIVE_COORDINATE_DRIFT_HOLD",
                "baton_digest": expected_baton_digest,
                "live_drift": live_drift,
                "persisted": {"prompt_stack_digest": persisted_prompt_digest, "frontier_digest": persisted_frontier_digest},
                "live": {"prompt_stack_digest": live_prompt_digest, "frontier_digest": live_frontier_digest},
                "remote_sync": remote_sync,
                "durable_return": False,
            }

        delta_prompt = self._compact_successor_prompt(baton)
        full_prompt_path = state.get("current_prompt_path")
        full_prompt = self.loop._loop_prompt_path(self.loop._paths(loop_id), full_prompt_path).read_text(encoding="utf-8")
        ...
        result = {
            ...
            "remote_sync": remote_sync,
            "live_coordinates": {"prompt_stack_digest": live_prompt_digest, "frontier_digest": live_frontier_digest},
            "durable_return": bool(remote_sync.get("shared_frontier_verified")) if mode != "DISABLED" else False,
            "laws": baton["laws"],
        }
        if include_full_prompt_on_change and baton["coordinate_delta"].get("prompt_stack_changed"):
            result["prompt_cone"] = self.prompt_runtime.compile(
                task=str((baton.get("transition") or {}).get("next_task") or state.get("task") or ""),
                profile=state.get("profile"),
                include_text=True,
            )
        if include_frontier_on_change and baton["coordinate_delta"].get("frontier_changed"):
...
SUCCESSOR_BATON_TOOLS = [
    {
        ...
        "description": "Derive a deterministic content-addressed successor baton from the latest verified rehydration transition. The baton is a projection of durable receipts and creates no Git progress.",
        "inputSchema": {
            "type": "object",
            "required": ["loop_id"],
    ...
    },
    {
        ...
        "description": "Verify and consume an exact successor baton, refresh the shared frontier when requested, reject drift, and hydrate only the affected dependency cone; falls back to full rehydration when delta coverage is insufficient.",
        "inputSchema": {
            "type": "object",
...
SUCCESSOR_BATON_TOOL_NAMES = {x["name"] for x in SUCCESSOR_BATON_TOOLS}
