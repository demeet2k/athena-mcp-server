...
from .architecture_drift import ARCHITECTURE_DRIFT_VERSION
from .coordination_inventory import COORDINATION_INVENTORY_VERSION,inventory_manifest
...
COORDINATION_LAYERS=[
    ...
    'AGENT_BOOT_COHESION_TREATMENT_V1','PARTY_COORDINATION_V1','PARTY_CHANNEL_V2','PARTY_REWARD_PROVENANCE_V3_2',
...
def build_effective_manifest(server)->Dict[str,Any]:
    ...
    organs['coordination']={
        'inventory_version':COORDINATION_INVENTORY_VERSION,
        'base_inventory_version':inventory.get('base_inventory_version'),
        'architecture_drift_version':ARCHITECTURE_DRIFT_VERSION,
        'party_reward_current':inventory.get('party_reward_current'),
        'organs':[{
        ...
        'law':'coordination maturity is explicit; Message Board coordination claims never alias Y1 semantic authority; Cohesion and Party reward remain advisory/provenance planes; Party reward current membrane is V3.2 finite-envelope hardened',
    }
    base['organs']=organs
    ...
    unresolved.append({
        ...
        'boundary':'effective inventory covers Message Board, Cohesion, bootstrap treatment and Party V1-V3.2; Freshness Train and additional post-V13 extension families such as Frontier/Rehydration/Campaign/QHUG/Mythic/Bionano must be classified explicitly before architecture maturity is claimed',
    })
    base['unresolved']=unresolved
    base['architecture_drift']={
        'version':ARCHITECTURE_DRIFT_VERSION,'inventory_version':COORDINATION_INVENTORY_VERSION,
...
def effective_maxdev_law()->str:
    return maxdev_law()+'''\n31 COORDINATION AUTHORITY: Message Board owns presence/coordination claims and messages; Y1 owns canonical semantic claims. Never alias these planes.\n32 COHESION: matchmaking, duplicate treatment and evidence coverage are advisory/read-only membranes; options/comparisons do not execute assignments, claims, truth or XP.\n33 PARTY: party messages/results use Message Board transport; V3.1 requires exact inner/outer RESULT-VERIFY envelope coherence; V3.2 requires finite non-boolean base XP before inherited reward processing. Party reward provenance cannot create result truth or global XP authority.\n34 ARCHITECTURE DRIFT: before promotion, require every declared mature organ to exist in the live runtime surface, SURFACE contract, effective manifest and OMEGA coordinate.\n35 INVENTORY EXPANSION: expose unclassified live tools/resources as recursive maturity-review pressure; do not auto-canonicalize them by file/tool existence. Freshness Train is currently an unclassified expansion candidate until its authority/mutation semantics are separately admitted.'''
