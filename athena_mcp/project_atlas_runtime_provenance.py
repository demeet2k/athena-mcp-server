...
def _root_frontier(root: Path, source: str) -> dict:
    try:
        ...
        try:
            remote=backend._git("remote","get-url","origin")
...
def runtime_frontier() -> dict:
    """Resolve the runtime source frontier without borrowing the brain Git head.

    Priority:
      1. ATHENA_RUNTIME_GIT_ROOT exact checkout.
      2. package source checkout (athena_mcp/ directly under .git root).
      3. explicit ATHENA_RUNTIME_REPOSITORY + ATHENA_RUNTIME_GIT_HEAD attestation.
    ...
    if configured_root:
        ...
        if not (root/".git").exists():
            return {
                "status":"HOLD_RUNTIME_PROVENANCE",
                "source":"ATHENA_RUNTIME_GIT_ROOT",
                "mode":"GIT_CHECKOUT",
                "root":str(root),
                "reason":"ATHENA_RUNTIME_GIT_ROOT is not an exact Git checkout",
            }
        return _root_frontier(root,"ATHENA_RUNTIME_GIT_ROOT")

    source_root=_source_checkout_root()
    if source_root is not None:
        return _root_frontier(source_root,"PACKAGE_SOURCE_CHECKOUT")

    repo=str(os.getenv("ATHENA_RUNTIME_REPOSITORY") or "").strip()
    head=str(os.getenv("ATHENA_RUNTIME_GIT_HEAD") or "").strip()
    if repo or head:
        if not repo or not HEX40.fullmatch(head):
            return {
                "status":"HOLD_RUNTIME_PROVENANCE",
                "source":"ENV_ATTESTATION",
                "mode":"HEAD_ATTESTATION",
                "repo_key":_normalize_repo(repo) if repo else None,
                "head":head or None,
...
def same_runtime_frontier(left: dict, right: dict) -> bool:
    keys=("status","mode","repo_key","head","root")
