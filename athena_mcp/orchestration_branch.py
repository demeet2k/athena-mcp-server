...
CREATE TABLE IF NOT EXISTS orchestration_branches(
 branch_id TEXT NOT NULL,basis_id TEXT NOT NULL,status TEXT NOT NULL,ewma_reward REAL,
 observations INTEGER NOT NULL,last_reward REAL,last_witness_ref TEXT,last_eid TEXT,
 policy_json TEXT NOT NULL,metadata_json TEXT NOT NULL,updated_at REAL NOT NULL,
 PRIMARY KEY(branch_id,basis_id));
CREATE TABLE IF NOT EXISTS branch_observations(
 observation_id TEXT PRIMARY KEY,branch_id TEXT NOT NULL,basis_id TEXT NOT NULL,reward REAL,
 witness_json TEXT NOT NULL,triggers_json TEXT NOT NULL,transition TEXT NOT NULL,eid TEXT NOT NULL,created_at REAL NOT NULL);
...
'''
DEFAULT_POLICY={'alpha':0.30,'min_observations':3,'hibernate_below':0.0,'resurrect_above':0.5}
...
def _policy(value:Optional[Mapping[str,Any]])->Dict[str,Any]:
    p={**DEFAULT_POLICY,**dict(value or {})};alpha=finite_number(p.get('alpha'));hb=finite_number(p.get('hibernate_below'));ra=finite_number(p.get('resurrect_above'))
...
def _verified_triggers(triggers:Optional[Iterable[Mapping[str,Any]]])->list[Dict[str,str]]:
    result=[]
    for item0 in triggers or []:
        item=dict(item0);kind=str(item.get('type') or '');ref=str(item.get('ref') or '').strip()
...
class BranchLedger:
    def __init__(self,core):
        ...
        self.core.register('TOOL','DEVELOPMENT','EVOLVE','BRANCH_LIFECYCLE','WITNESSED_EWMA_HIBERNATION',{'branch_id':'stable','basis_id':'metric basis','reward':'calibrated','witness':'verified','triggers':'verified refs'},{'status':'ACTIVE|HIBERNATED|REVIEW','ewma_reward':'number','transition':'typed'},actor='GENESIS.AOR.3',status='CANONICAL')
    ...
    def state(self,branch_id:str,basis_id:Optional[str]=None):
        if basis_id is not None:
            row=self.s.one('SELECT * FROM orchestration_branches WHERE branch_id=? AND basis_id=?',(branch_id,basis_id));return self._decode(row) if row else None
        return [self._decode(r) for r in self.s.rows('SELECT * FROM orchestration_branches WHERE branch_id=? ORDER BY updated_at DESC',(branch_id,))]
    def list(self,status:Optional[str]=None,limit:int=100):
        ...
        else:rows=self.s.rows('SELECT * FROM orchestration_branches ORDER BY updated_at DESC LIMIT ?',(limit,))
    ...
    def observe(self,branch_id:str,basis_id:str,reward:Any,witness:Mapping[str,Any],policy:Optional[Mapping[str,Any]]=None,triggers:Optional[Iterable[Mapping[str,Any]]]=None,metadata:Optional[Mapping[str,Any]]=None,actor:str='agent'):
        ...
        witness_ref=_verified_witness(witness);trigger_rows=_verified_triggers(triggers);p=_policy(policy);previous=self.s.one('SELECT * FROM orchestration_branches WHERE branch_id=? AND basis_id=?',(branch_id,basis_id));prior_status=previous['status'] if previous else 'ACTIVE';prior_ewma=previous['ewma_reward'] if previous else None;prior_n=int(previous['observations']) if previous else 0;ewma=reward_value if prior_ewma is None else p['alpha']*reward_value+(1-p['alpha'])*float(prior_ewma);observations=prior_n+1;status=prior_status;transition='OBSERVE'
        if prior_status=='HIBERNATED' and trigger_rows:status='REVIEW';transition='HIBERNATED->REVIEW'
        if status in {'HIBERNATED','REVIEW'} and observations>=p['min_observations'] and ewma>=p['resurrect_above']:status='ACTIVE';transition=f'{prior_status}->ACTIVE'
        elif status=='ACTIVE' and observations>=p['min_observations'] and ewma<=p['hibernate_below']:status='HIBERNATED';transition='ACTIVE->HIBERNATED'
        parent=self.s.head('global');pe=parent['eid'] if parent else None;event_payload={'operation':'BRANCH_OBSERVE','branch_id':branch_id,'basis_id':basis_id,'reward':reward_value,'ewma_reward':ewma,'observations':observations,'prior_status':prior_status,'status':status,'transition':transition,'witness_ref':witness_ref,'triggers':trigger_rows,'policy':p};eid=event_id('BRANCH_OBSERVE',actor,pe,event_payload);ed=digest(event_payload,32);observation_id='BROBS.'+digest({'eid':eid,'branch':branch_id,'basis':basis_id},24);merged={**(json.loads(previous['metadata_json']) if previous else {}),**dict(metadata or {})}
        with self.s.db:
            self.s.db.execute('INSERT OR REPLACE INTO orchestration_branches VALUES(?,?,?,?,?,?,?,?,?,?,?)',(branch_id,basis_id,status,ewma,observations,reward_value,witness_ref,eid,json.dumps(p,sort_keys=True),json.dumps(merged,sort_keys=True),time.time()))
    ...
    def review(self,branch_id:str,basis_id:str,trigger:Mapping[str,Any],actor:str='agent'):
        ...
        parent=self.s.head('global');pe=parent['eid'] if parent else None;payload={'operation':'BRANCH_REVIEW','branch_id':branch_id,'basis_id':basis_id,'prior_status':prior,'status':'REVIEW','trigger':trigger_rows[0]};eid=event_id('BRANCH_REVIEW',actor,pe,payload);ed=digest(payload,32)
        ...
        self.s.put_event(eid,'BRANCH_REVIEW',actor,pe,payload,ed);self.s.set_head('global',None,None,eid,ed);return {'branch_id':branch_id,'basis_id':basis_id,'status':'REVIEW','transition':'HIBERNATED->REVIEW','eid':eid,'trigger':trigger_rows[0]}
    def enrich_candidates(self,candidates:Iterable[Mapping[str,Any]],basis_id:str):
        ...
        for candidate in candidates:
            ...
            if branch_id:
                ...
                if state:item['lifecycle']={'branch_id':branch_id,'basis_id':basis_id,'status':state['status'],'ewma_reward':state['ewma_reward'],'observations':state['observations'],'last_eid':state['last_eid']}
