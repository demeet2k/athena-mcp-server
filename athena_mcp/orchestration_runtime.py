...
class OrchestrationRuntime:
    ...
    def compile(self, seed: Any, candidates=None, residuals=None, budget: Optional[Mapping[str, Any]]=None, metric_contract: Optional[Mapping[str, Any]]=None, actor: str="agent", task: str="", session_id: Optional[str]=None, persist: bool=True):
        ...
        with self.s.db:
            self.s.db.execute("INSERT INTO orchestration_runs VALUES(?,?,?,?,?,?,?,?,?)",(run_id,actor,task,session_id,json.dumps(inputs,sort_keys=True,ensure_ascii=False),json.dumps(output,sort_keys=True,ensure_ascii=False),output["decision_digest"],eid,time.time()))
    ...
    def recent(self,limit:int=20):
        ...
        return self.s.rows("SELECT run_id,actor,task,session_id,decision_digest,eid,created_at FROM orchestration_runs ORDER BY created_at DESC LIMIT ?",(limit,))
