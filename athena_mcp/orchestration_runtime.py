...
class OrchestrationRuntime:
    ...
    def compile(self,seed:Any,candidates=None,residuals=None,budget:Optional[Mapping[str,Any]]=None,metric_contract:Optional[Mapping[str,Any]]=None,actor:str="agent",task:str="",session_id:Optional[str]=None,persist:bool=True):
        metric_contract=dict(metric_contract or {}); basis_id=str(metric_contract.get("basis_id") or "RAW.UNDECLARED")
        candidate_rows=list(candidates or [])
        if self.branches is not None: candidate_rows=self.branches.enrich_candidates(candidate_rows,basis_id)
        inputs={"seed":seed,"candidates":candidate_rows,"residuals":list(residuals or []),"budget":dict(budget or {}),"metric_contract":metric_contract}
