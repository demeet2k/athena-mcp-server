...
def install_frontier_claim_provider_guard(runtime_cls=FrontierClaimRuntime) -> None:
    ...
    PG-002: after a claim provider effect is observed, revalidate the exact
    scheduler contract on the fresh source head before preparing CLAIM_ACQUIRED.
    """
    ...
    def claim_event_prepare_guarded(runtime, packet: dict, prepared: dict) -> dict:
        ...
        if contract.get("status") != "PASS" or not expected or current != expected:
            return {
                ...
                "reason": "scheduler interpretation contract changed after claim creation",
