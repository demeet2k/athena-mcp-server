...
VERSION = "ATHENA.NEXT.SCOUT.ECONOMY.5"
TOOL_NAME = "athena_next_scout_economy"

# Planning estimates only. They are deterministic routing priors, not observed
# runtime consumption, evidence, truth, or billing measurements.
RESOURCE_PROFILE = {
    "DEPENDENCY_MAP": {"tokens": 3200, "minutes": 10, "tool_calls": 2, "coordination": 1, "git_risk": 1,
                       "reconstruction_reduction": 6, "blocker_removal": 6, "information_gain": 4},
    "TEST_DESIGN": {"tokens": 4000, "minutes": 12, "tool_calls": 3, "coordination": 2, "git_risk": 1,
                    "reconstruction_reduction": 5, "blocker_removal": 5, "information_gain": 5},
    "RISK_SCAN": {"tokens": 2800, "minutes": 9, "tool_calls": 2, "coordination": 1, "git_risk": 1,
                  "reconstruction_reduction": 4, "blocker_removal": 5, "information_gain": 5},
    "RETRIEVAL_PLAN": {"tokens": 5000, "minutes": 15, "tool_calls": 4, "coordination": 2, "git_risk": 1,
                       "reconstruction_reduction": 5, "blocker_removal": 3, "information_gain": 6},
    "INTERFACE_MAP": {"tokens": 3500, "minutes": 10, "tool_calls": 2, "coordination": 2, "git_risk": 2,
                      "reconstruction_reduction": 5, "blocker_removal": 4, "information_gain": 4},
    "SOURCE_REVIEW": {"tokens": 6000, "minutes": 18, "tool_calls": 5, "coordination": 2, "git_risk": 1,
                      "reconstruction_reduction": 4, "blocker_removal": 2, "information_gain": 6},
}

RESOURCE_KEYS = ("tokens", "minutes", "tool_calls", "coordination", "git_risk")
VALUE_KEYS = ("reconstruction_reduction", "blocker_removal", "information_gain")
...
def _sum_profiles(rows: list[dict]) -> dict[str, int]:
    out = {key: 0 for key in (*RESOURCE_KEYS, *VALUE_KEYS)}
    for row in rows:
        profile = RESOURCE_PROFILE.get(str(row.get("kind")))
        if not profile:
            continue
        for key in out:
            out[key] += int(profile[key])
    return out


def _fits(cost: dict[str, int], available: dict[str, int]) -> bool:
...
class NextScoutEconomyRuntime:
    ...
    def economy(self, *, pipeline_id: str, expected_pipeline_state_digest: str,
        ...
        available_total = {k: budgets[k] - reserves[k] for k in RESOURCE_KEYS}

        state = self.pipeline.state(pipeline_id)
        if state.get("state_digest") != expected_pipeline_state_digest:
            raise ValueError("STALE_PIPELINE_STATE_FOR_SCOUT_ECONOMY")
        ...
        if str(shared_remote_mode).upper() == "REQUIRED" and not board.get("shared_frontier_verified"):
            return {"artifact": VERSION, "status": "SCOUT_ECONOMY_SHARED_FRONTIER_HOLD",
        ...
        for row in board.get("active") or []:
            ...
            if plan_id in plans and plans[plan_id].get("kind") in RESOURCE_PROFILE:
        ...
        for plan_id, raw in sorted(plans.items()):
            ...
            elif plan.get("kind") not in PREP_KINDS or plan.get("kind") not in RESOURCE_PROFILE:
                reason = "UNKNOWN_RESOURCE_PROFILE"
        ...
        if optimal_sets:
            if len(optimal_sets) == 1:
                ...
                status = "AMBIGUOUS_ECONOMIC_ALLOCATION"
...
def install_next_scout_economy_extension(prompt_runtime_cls, tool_list: list[dict], tool_names: set[str]) -> None:
    ...
    def call_with_economy(self, name, arguments):
        if name in NEXT_SCOUT_ECONOMY_TOOL_NAMES:
            ...
            if runtime is None:
                ...
                runtime = NextScoutEconomyRuntime(pipeline, breadth)
...
__all__ = ["VERSION", "TOOL_NAME", "RESOURCE_PROFILE", "NextScoutEconomyRuntime",
           "NEXT_SCOUT_ECONOMY_TOOLS", "NEXT_SCOUT_ECONOMY_TOOL_NAMES", "install_next_scout_economy_extension"]
