...
def install_release_v15(namespace: dict[str, Any]) -> None:
    ...
    if not getattr(um,'_athena_collective_v15_installed',False):
        ...
        def build_unified_manifest_v15(server):
            ...
            navigation=navigation.replace(
                'COLLECTIVE_SYNTHESIS_V14 <-> PROMOTION_TRUST_V2',
                'COLLECTIVE_SYNTHESIS_V14 <-> COLLECTIVE_CALIBRATED_V15 <-> PROMOTION_TRUST_V2',
            )
            navigation=navigation.replace(
                'COLLECTIVE_SYNTHESIS_V14 <-> Git/MCP',
                'COLLECTIVE_SYNTHESIS_V14 <-> COLLECTIVE_CALIBRATED_V15 <-> Git/MCP',
            )
            navigation=navigation.replace(
                'Collective(V1-V14) <-> COLLECTIVE_SYNTHESIS_V14 <-> Git/MCP',
                'Collective(V1-V15) <-> COLLECTIVE_SYNTHESIS_V14 <-> COLLECTIVE_CALIBRATED_V15 <-> Git/MCP',
            )
            ...
            for unresolved in payload.get('unresolved') or []:
                ...
                if uid=='GENERAL_BELIEF_CONTROL':
                    unresolved['v15_boundary']='exact finite-dimensional linear-Gaussian belief update plus linear-Gaussian action control; non-Gaussian/general continuous belief-MDP remains unresolved'
                elif uid=='FORMAL_CAUSAL_DISCOVERY':
                    unresolved['v15_boundary']='out-of-fold isotonic reliability calibration from externally labelled structural examples; calibrated causal graph posterior/full FCI-RFCI remains unresolved'
                elif uid=='LONGITUDINAL_CAUSAL_POLICY':
                    unresolved['v15_boundary']='cross-fitted bounded two-timepoint sequential logistic TMLE and sequential AIPW dynamic-policy value; arbitrary-horizon/general longitudinal theory remains unresolved'
                elif uid=='STOCHASTIC_RESOURCE_CONTROL':
                    unresolved['v15_boundary']='finite-horizon rectangular total-variation robust dynamic program; non-rectangular/general multistage stochastic-DRO control remains unresolved'
    ...
    if 'collective_v14' in sc.REQUIRED_TOOLS:
        sc.REQUIRED_TOOLS['collective_v14']=set(sc.REQUIRED_TOOLS['collective_v14'])-v15_names
    ...
    if not getattr(sc,'_athena_collective_v15_installed',False):
        ...
        def contract_manifest_v15():
            ...
            payload['law']=str(payload.get('law','')).replace('Collective V1-V14','Collective V1-V15')
            return payload
