...
NUMERIC_METRICS = (
    "readiness", "gain", "independence", "bridge", "cost",
    "delta_j", "information_gain", "option_value", "evidence", "connection",
    "replay", "navigation", "reconstruction", "implementation", "novelty",
    "duplicate", "fake", "bloat", "unsupported", "unhandled_contradiction",
    "coordinate_loss", "severity", "leverage",
)
...
FORMULA_METRICS = {
    ...
    "reward": (
        ...
        "reconstruction", "implementation", "novelty", "duplicate", "fake",
        "bloat", "unsupported", "unhandled_contradiction", "coordinate_loss",
    ),
...
def _contract_meta(contract: Optional[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        "basis_id": contract.get("basis_id") or "RAW.UNDECLARED",
...
def normalize_item(item: Mapping[str, Any], contract: Optional[Mapping[str, Any]]) -> tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    for name in NUMERIC_METRICS:
        ...
        if value is None:
            status[name] = {"status": "UNKNOWN", "basis": None}
            continue
        spec = meta["metrics"].get(name)
        if spec is None:
            status[name] = {"status": "RAW_UNCALIBRATED", "basis": meta["basis_id"]}
            continue
        ...
        if scale is None or abs(scale) <= 1e-12 or offset is None:
            status[name] = {"status": "INVALID_CONTRACT", "basis": meta["basis_id"], "spec": dict(spec) if isinstance(spec, Mapping) else spec}
            continue
...
def formula_calibration(report: Mapping[str, Any], formula: str) -> Dict[str, Any]:
    ...
    if invalid:
        ...
        status = "CALIBRATED" if calibrated else "NO_KNOWN_OPERANDS"
    return {
        "status": status,
        "strict": strict,
        ...
        "unknown": unknown,
        "ranking_allowed": status != "BLOCKED",
    }
...
def calibration_requests(candidate_id: str, report: Mapping[str, Any]) -> list[Dict[str, Any]]:
    ...
    for formula in FORMULA_METRICS:
        gate = formula_calibration(report, formula)
        missing = sorted(set(gate["raw_uncalibrated"] + gate["invalid_contract"]))
        if missing:
            requests.append({
                "candidate": candidate_id,
                "formula": formula,
                "metrics": missing,
                "strict_block": not gate["ranking_allowed"],
                "basis_id": report.get("basis_id"),
            })
    return requests
...
def contract_summary(contract: Optional[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "strict": meta["strict"],
        "declared_metrics": sorted(meta["metrics"]),
        "normalization": "x'=(x-offset)/abs(scale)",
        "undeclared_policy": "BLOCK in strict mode; WARN_RAW otherwise",
    }
