...
from .drive_recovery_wave3 import (
    AUTOWEAVE_ISSUE,
    WAVE3_ATHENA_HEAD,
    WAVE3_MCP_HEAD,
    WAVE3_ORGANS,
...
VERSION = "DRIVE.ORGAN-RECOVERY.3"
LAWS = list(WAVE1_LAWS) + [
    ...
    "FORWARD_CONTINUATION != LATERAL_SEAM_DISCOVERY",
    "EXPLICIT_TYPED_TRANSPORT != AUTOMATIC_BRIDGE_DISCOVERY",
    "SIMILARITY != BRIDGE",
    "BRIDGE_CANDIDATE != VALIDATED_TRANSPORT",
    "HOLONOMY != ERROR_BY_DEFINITION",
    "PROPAGATION != AUTHORITY_ELEVATION",
    "PROPAGATION != EXECUTION_PERMISSION",
    "MORE_RETRIEVED_DOCS != MORE_NOVEL_ORGANS",
    "LOCAL_CONVERGENCE SHOULD REDIRECT SEARCH BUDGET",
]
ORGANS = [deepcopy(row) for row in WAVE1_ORGANS] + [deepcopy(row) for row in WAVE2_ORGANS] + [deepcopy(row) for row in WAVE3_ORGANS]
_ORGAN_BY_ID = {row["organ_id"]: row for row in ORGANS}
if len(_ORGAN_BY_ID) != len(ORGANS):
    raise RuntimeError("duplicate recovered organ id across Drive recovery waves")
...
_FRONTIER_STATUSES = {
    ...
    "THEORY_OPERATOR_SOURCE_SCOPED_ISSUE",
}
...
def residual_frontier(limit: int = 10, include_theory: bool = False) -> dict[str, Any]:
    ...
    if include_theory:
        allowed.update({"THEORY_SOURCE_NOT_RUNTIME_REQUIREMENT", "THEORY_PREDECESSOR_PARTIAL_CURRENT"})
    ...
    return {
        ...
        "source_heads": {
            ...
            "wave3_athena": WAVE3_ATHENA_HEAD,
            "wave3_mcp": WAVE3_MCP_HEAD,
        },
...
def holoaddress_for(organ_id: str) -> dict[str, Any]:
    ...
    version_pin = {"status": "UNPINNED", "revision_id": None} if revision is None else {"status": "PINNED", "revision_id": str(revision)}
    return {
        "version": VERSION,
        "holoaddress": {
            "RootID": "ATHENA.GOOGLE-DOCS.ORGAN-RECOVERY.V3",
...
class DriveRecoveryRegistryRuntime:
    def describe(self) -> dict[str, Any]:
        ...
        return {
            ...
            "autoweave_residual_issue": AUTOWEAVE_ISSUE,
