...
TSE_HELIX_ADVANCE_TOOL={
    ...
    'description':(
        'Advance one TSE Helix transition. PUBLISH/MATCH/HANDOFF/CLAIM_STATE/RETURN_CHECK execute the real population source then append '
        'a source-bound observation. APPLY observes an externally completed shared Git ancestry adoption and may emit RETURN_APPLIED, but '
        'never merges, rebases, cherry-picks, pushes, or creates merge authority. If source mutation is already durable and telemetry fails, '
        'the source action is not rolled back or reclassified.'
    ),
    'inputSchema':{
        'type':'object',
        ...
        'properties':{
            ...
            'child_return':{
                'type':['object','null'],
                'description':(
                    'For RETURN_CHECK: the child Return payload. For APPLY: an apply packet '
                    ...
                    'platform_counter_reset_claimed:false}}. parent_event_id is the exact SOURCE_BOUND CHILD_VERIFIED_RETURN event.'
                ),
            },
...
TSE_HELIX_CONSUMPTION_TOOL={
    ...
    'description':(
        'Read the current shared Message Board and emit HANDOFF_CONSUMED only when the exact routed TSE envelope is still bound and the '
        'exact matched agent has an actual ACK for that handoff message. ACK remains distinct from claim.'
    ),
    'inputSchema':{
        'type':'object',
...
TSE_HELIX_RECONCILE_TOOL={
    ...
    'description':(
        ...
        'shared-adoption observation path through athena_tse_helix_advance and is not reconciled by replaying a merge.'
    ),
    'inputSchema':{
        'type':'object',
        'required':['mission_id','operation','route','parent_event_id','actor_id','witnesses','cost'],
