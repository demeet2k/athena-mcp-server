...
CREATE TABLE IF NOT EXISTS hug_implementations(
 impl_id TEXT PRIMARY KEY,name TEXT NOT NULL,version TEXT NOT NULL,algorithm_ref TEXT NOT NULL,implementation_digest TEXT NOT NULL,
 parameter_semantics_json TEXT NOT NULL,input_schema_json TEXT NOT NULL,output_schema_json TEXT NOT NULL,status TEXT NOT NULL,
 test_packet_json TEXT NOT NULL,canonical_ref TEXT,last_eid TEXT NOT NULL,created_at REAL NOT NULL,updated_at REAL NOT NULL);
...
CREATE TABLE IF NOT EXISTS hug_invocations(
 invocation_id TEXT PRIMARY KEY,impl_id TEXT NOT NULL,impl_snapshot_json TEXT NOT NULL,arguments_json TEXT NOT NULL,context_json TEXT NOT NULL,
 input_digest TEXT NOT NULL,status TEXT NOT NULL,output_json TEXT,receipt_json TEXT NOT NULL,failure_json TEXT NOT NULL,eid TEXT NOT NULL,created_at REAL NOT NULL,updated_at REAL NOT NULL);
...
def _verified_ref(packet:Mapping[str,Any],label:str):
    ...
    return _nonempty(packet.get('ref'),f'{label}.ref')
...
def _parameter_semantics(value:Mapping[str,Any]):
    ...
    for key in HUG_PARAMS:
        ...
        packet=dict(packet);packet['meaning']=_nonempty(packet.get('meaning'),f'HUG parameter {key}.meaning');out[key]=packet
    return out

def _snapshot(row):
    return {'impl_id':row['impl_id'],'name':row['name'],'version':row['version'],'algorithm_ref':row['algorithm_ref'],'implementation_digest':row['implementation_digest'],'parameter_semantics':json.loads(row['parameter_semantics_json']),'input_schema':json.loads(row['input_schema_json']),'output_schema':json.loads(row['output_schema_json']),'status':row['status'],'canonical_ref':row['canonical_ref'],'last_eid':row['last_eid']}
...
class HugRegistry:
    ...
    def _event(self,op,payload,actor):
        parent=self.s.head('global');pe=parent['eid'] if parent else None;eid=event_id(f'HUG_{op}',actor,pe,payload);ed=digest(payload,32);self.s.put_event(eid,f'HUG_{op}',actor,pe,payload,ed);self.s.set_head('global',None,None,eid,ed);return eid
    def register(self,name,version,algorithm_ref,implementation_digest,parameter_semantics,input_schema,output_schema,actor='agent'):
        ...
        identity={'name':name,'version':version,'algorithm_ref':algorithm_ref,'implementation_digest':implementation_digest,'parameter_semantics':sem,'input_schema':dict(input_schema),'output_schema':dict(output_schema)};impl_id='HUGIMPL.'+digest(identity,24)
    ...
    def list(self,status:Optional[str]=None,limit:int=100):
        ...
        args.append(limit);return [self.state(r['impl_id']) for r in self.s.rows(f'SELECT impl_id FROM hug_implementations{where} ORDER BY updated_at DESC LIMIT ?',tuple(args))]
    def promote(self,impl_id,target_status,test=None,canonical_authority=None,actor='agent'):
        row=self.s.one('SELECT * FROM hug_implementations WHERE impl_id=?',(impl_id,))
    ...
    def plan(self,impl_id,arguments,context=None,required_status='CANONICAL',actor='agent'):
        row=self.s.one('SELECT * FROM hug_implementations WHERE impl_id=?',(impl_id,))
        if not row:raise KeyError('unknown HUG implementation')
        ...
        validate(json.loads(row['input_schema_json']),args);snap=_snapshot(row);input_packet={'implementation':snap,'arguments':args,'context':dict(context or {})};raw=json.dumps(input_packet,sort_keys=True,ensure_ascii=False,separators=(',',':'));input_digest=hashlib.sha256(raw.encode()).hexdigest();invocation_id='HUGINV.'+digest({'impl':impl_id,'input_digest':input_digest,'nonce':time.time_ns()},24);payload={'operation':'PLAN','invocation_id':invocation_id,'impl_id':impl_id,'required_status':required,'input_digest':input_digest};eid=self._event('PLAN',payload,actor);now=time.time()
    ...
    def complete(self,invocation_id,output,receipt,actor='agent'):
        ...
        receipt_ref=_verified_ref(receipt,'HUG execution receipt');impl=json.loads(row['impl_snapshot_json']);validate(impl['output_schema'],output);payload={'operation':'COMPLETE','invocation_id':invocation_id,'impl_id':row['impl_id'],'receipt_ref':receipt_ref};eid=self._event('COMPLETE',payload,actor)
    ...
    def fail(self,invocation_id,reason,witness,actor='agent'):
        row=self.s.one('SELECT * FROM hug_invocations WHERE invocation_id=?',(invocation_id,))
        if not row:raise KeyError('unknown HUG invocation')
    ...
    def verify_packet(self,invocation_id):
    ...
    def benchmark(self):
        counts={r['status']:r['n'] for r in self.s.rows('SELECT status,COUNT(*) n FROM hug_implementations GROUP BY status')};inv={r['status']:r['n'] for r in self.s.rows('SELECT status,COUNT(*) n FROM hug_invocations GROUP BY status')};return {'hug_implementations':sum(counts.values()),'hug_implementation_status':counts,'hug_invocations':sum(inv.values()),'hug_invocation_status':inv}
