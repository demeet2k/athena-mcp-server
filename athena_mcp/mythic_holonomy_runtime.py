...
def _distance(start:Dict[str,Any],end:Dict[str,Any],required_prov:List[str],ledger_prov:List[str],ledger_invariants:List[str],required_invariants:List[str],loss_ledger:List[str])->Dict[str,Any]:
    ...
    changed=int(bool(role_delta))+int(bool(decoder_delta))+int(ontology_delta>0)+int(bool(authority_delta))+int(bool(standing_delta))+int(provenance_delta>0)
    unaccounted_loss=max(0,changed-len(set(loss_ledger or [])))
    return {
...
class MythicHolonomyRuntime:
    ...
    def _edgewise(self,path:List[str],case:Dict[str,Any],layers:Dict[str,Dict[str,Any]])->Dict[str,Any]:
        ...
        for a,b in zip(path,path[1:]):
            ...
            if a==b:
                receipt=self.strata.transport(_strata_layer(src),_strata_layer(dst),"SEMANTIC_TRANSPORT")
            else:
                receipt=self.strata.transport(_strata_layer(src),_strata_layer(dst),"SEMANTIC_TRANSPORT",explicit_bridge=_bridge(case,src,dst))
    ...
    def _a1(self,case:Dict[str,Any],layers:Dict[str,Dict[str,Any]])->Dict[str,Any]:
        ...
        if op=="SEMANTIC_EQUIVALENCE":
            ...
            receipt=self.strata.transport(_strata_layer(src),_strata_layer(dst),"SEMANTIC_EQUIVALENCE",explicit_bridge=_bridge(case,src,dst))
            ok=(not receipt.get("allowed")) and case.get("expected_class")=="HOLD_EQUIVALENCE"
            return {"status":receipt["status"],"allowed":receipt.get("allowed",False),"receipt":receipt,"expected_pass":ok}
        if op=="HOLONOMY_LOOP":
            forward=path[:-1] if len(path)>1 and path[-1]==path[0] else path
            edge=self._edgewise(forward,case,layers)
            return {"status":"EDGEWISE_ONLY_NO_HOLONOMY","allowed":edge["allowed"],"receipts":edge["receipts"],"holonomy_nonzero":None,"expected_pass":False}
        if op=="PATH_ORDER_COMPARE":
    ...
    def _compose(self,path:List[str],case:Dict[str,Any],layers:Dict[str,Dict[str,Any]])->Dict[str,Any]:
        ...
        for a,b in zip(path,path[1:]):
            ...
            if a==b:
                ...
                receipt=self.strata.transport(_strata_layer(src),_strata_layer(dst),"SEMANTIC_TRANSPORT",explicit_bridge=bridge)
    ...
    def _a2(self,case:Dict[str,Any],layers:Dict[str,Dict[str,Any]],lawful_edges:Dict[str,set])->Dict[str,Any]:
        ...
        if op=="SEMANTIC_EQUIVALENCE":
            src=layers[path[0]];dst=layers[path[-1]]
            receipt=self.strata.transport(_strata_layer(src),_strata_layer(dst),"SEMANTIC_EQUIVALENCE",explicit_bridge=_bridge(case,src,dst))
            return {"status":receipt["status"],"allowed":False,"receipt":receipt,"expected_pass":(not receipt.get("allowed")) and expected=="HOLD_EQUIVALENCE"}
        if op=="PATH_ORDER_COMPARE":
        ...
        if op=="SAME_LAYER_CONTROL":
            ...
            return {"status":"SAME_LAYER_CONTROL_EVALUATED","allowed":True,"composition":composed,"holonomy_vector":vector,"holonomy_nonzero":not zero,"expected_pass":zero and expected=="ZERO_HOLONOMY_CONTROL"}
        return {"status":"COMPOSED_TRANSPORT_ALLOWED","allowed":True,"composition":composed,"expected_pass":expected=="ALLOW_WITH_LOSS"}
    ...
    def _summarize(self,arm:str,results:List[Dict[str,Any]],cases:List[Dict[str,Any]])->Dict[str,Any]:
        ...
        lawful=[i for i,c in enumerate(cases) if c.get("expected_class")=="ALLOW_WITH_LOSS"]
