...
CRITICAL_REQUIRED_COLUMNS={
    ...
    'transport_runs':{'run_id','kind','input_json','output_json','transport_digest','eid'},
}
...
class StateFoundationSurface:
    ...
    def __init__(self,server,development):
        ...
        self.CRITICAL_REQUIRED_TABLES=CRITICAL_REQUIRED_TABLES;self.CRITICAL_REQUIRED_COLUMNS=CRITICAL_REQUIRED_COLUMNS
        self.schema=SchemaManager(server.core);self.reconstruction=ReconstructionLedger(server)
    ...
    def read_resource(self,uri:str):
        if uri=='athena://schema':
            return {'version':SCHEMA_LEDGER_VERSION,'target_db_schema_version':CURRENT_DB_SCHEMA_VERSION,'critical_required_tables':sorted(CRITICAL_REQUIRED_TABLES),'critical_required_columns':{k:sorted(v) for k,v in CRITICAL_REQUIRED_COLUMNS.items()},'status':self.schema.status(),'plan':self.schema.plan(),'verification':self.schema.verify(CRITICAL_REQUIRED_TABLES,CRITICAL_REQUIRED_COLUMNS),'law':'migrations are explicit and versioned; v1 inventories existing additive state, v2 repairs the RECONRUN expected-ref column contract; future schema versions block silent downgrade and required-column absence is a hard failure'}
        if uri=='athena://state/omega':return {'version':OMEGA_VERSION,'projection':project_omega(self.server),'law':'OMEGA is one current accessible runtime-state projection; unavailable external components remain UNKNOWN and the digest covers the observed packet, not unseen world state'}
        if uri=='athena://reconstruction':return {'version':RECON_VERSION,'benchmark':self.reconstruction.benchmark(),'recent':self.reconstruction.recent(50),'law':'RECONRUN freezes current OMEGA plus exact consulted and expected source refs; missing expected refs become explicit defects and unlisted sources are never implied searched'}
        raise KeyError(uri)

    def benchmark(self):return {'db_schema_version':self.schema.current_version(),'schema_target_version':CURRENT_DB_SCHEMA_VERSION,**self.reconstruction.benchmark()}
