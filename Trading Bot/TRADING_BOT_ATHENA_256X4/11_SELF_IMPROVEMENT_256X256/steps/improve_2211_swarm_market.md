# Improvement Step 2211

## Improvement Signature

- `swarm` -> `market` -> `expand` -> `relay`
...
This step repairs a local weakness where `swarm` has not yet been fully carried through `market` into `expand` and `relay`.
