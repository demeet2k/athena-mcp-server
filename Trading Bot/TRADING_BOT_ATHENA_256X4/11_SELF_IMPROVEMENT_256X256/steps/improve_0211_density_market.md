# Improvement Step 0211

## Improvement Signature

- `density` -> `market` -> `expand` -> `relay`
...
This step repairs a local weakness where `density` has not yet been fully carried through `market` into `expand` and `relay`.
