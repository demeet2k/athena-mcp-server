# Improvement Step 1010

## Improvement Signature

- `routing` -> `control` -> `expand` -> `seed`
...
This step repairs a local weakness where `routing` has not yet been fully carried through `control` into `expand` and `seed`.
