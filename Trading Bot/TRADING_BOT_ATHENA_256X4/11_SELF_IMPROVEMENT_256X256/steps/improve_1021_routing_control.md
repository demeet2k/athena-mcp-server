# Improvement Step 1021

## Improvement Signature

- `routing` -> `control` -> `verify` -> `relay`
...
This step repairs a local weakness where `routing` has not yet been fully carried through `control` into `verify` and `relay`.
