# Improvement Step 3213

## Improvement Signature

- `recursion` -> `market` -> `expand` -> `renew`
...
This step repairs a local weakness where `recursion` has not yet been fully carried through `market` into `expand` and `renew`.
