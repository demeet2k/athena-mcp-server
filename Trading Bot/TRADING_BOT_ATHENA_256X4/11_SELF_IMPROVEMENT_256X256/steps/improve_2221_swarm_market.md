# Improvement Step 2221

## Improvement Signature

- `swarm` -> `market` -> `verify` -> `relay`
- role owner: `R01 Corpus Cartographer`
- metro route: `L1 Atlas Line` via `H2 Phase Hub` -> `H5 Risk Hub`
- chapter anchor: `Ch14 Metro Map of the Bot`
- appendix anchor: `AppA Corpus Intake Schema`

## Gap Addressed

This step repairs a local weakness where `swarm` has not yet been fully carried through `market` into `verify` and `relay`.
