# Improvement Step 0020

## Improvement Signature

- `density` -> `control` -> `verify` -> `seed`
- role owner: `R07 K-Wave Sentinel`
- metro route: `L7 Replay Line` via `H5 Risk Hub` -> `H8 Audit Hub`
- chapter anchor: `Ch20 Deployment and Monitoring`
- appendix anchor: `AppG Signal Schema`

## Gap Addressed

This step repairs a local weakness where `density` has not yet been fully carried through `control` into `verify` and `seed`.
