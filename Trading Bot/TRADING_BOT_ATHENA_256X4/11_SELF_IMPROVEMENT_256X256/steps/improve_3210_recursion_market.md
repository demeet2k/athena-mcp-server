# Improvement Step 3210

## Improvement Signature

- `recursion` -> `market` -> `expand` -> `seed`
...
This step repairs a local weakness where `recursion` has not yet been fully carried through `market` into `expand` and `seed`.
