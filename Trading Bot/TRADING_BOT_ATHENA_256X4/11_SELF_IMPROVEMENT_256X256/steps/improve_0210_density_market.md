# Improvement Step 0210

## Improvement Signature

- `density` -> `market` -> `expand` -> `seed`
...
This step repairs a local weakness where `density` has not yet been fully carried through `market` into `expand` and `seed`.
