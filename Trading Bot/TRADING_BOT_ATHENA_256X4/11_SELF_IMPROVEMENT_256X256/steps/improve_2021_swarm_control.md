# Improvement Step 2021

## Improvement Signature

- `swarm` -> `control` -> `verify` -> `relay`
...
This step repairs a local weakness where `swarm` has not yet been fully carried through `control` into `verify` and `relay`.
