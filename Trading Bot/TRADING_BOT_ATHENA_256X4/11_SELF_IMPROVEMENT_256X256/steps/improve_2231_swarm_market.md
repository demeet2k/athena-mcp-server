# Improvement Step 2231

## Improvement Signature

- `swarm` -> `market` -> `integrate` -> `relay`
...
This step repairs a local weakness where `swarm` has not yet been fully carried through `market` into `integrate` and `relay`.
