# Improvement Step 1233

## Improvement Signature

- `routing` -> `market` -> `integrate` -> `renew`
...
This step repairs a local weakness where `routing` has not yet been fully carried through `market` into `integrate` and `renew`.
