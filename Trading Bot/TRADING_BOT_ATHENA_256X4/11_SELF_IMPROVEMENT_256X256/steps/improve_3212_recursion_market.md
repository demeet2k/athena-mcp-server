# Improvement Step 3212

## Improvement Signature

- `recursion` -> `market` -> `expand` -> `audit`
...
This step repairs a local weakness where `recursion` has not yet been fully carried through `market` into `expand` and `audit`.
