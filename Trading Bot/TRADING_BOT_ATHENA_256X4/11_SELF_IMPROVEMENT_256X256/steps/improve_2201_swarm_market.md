# Improvement Step 2201

## Improvement Signature

- `swarm` -> `market` -> `repair` -> `relay`
...
This step repairs a local weakness where `swarm` has not yet been fully carried through `market` into `repair` and `relay`.
