# Improvement Step 0013

## Improvement Signature

- `density` -> `control` -> `expand` -> `renew`
...
This step repairs a local weakness where `density` has not yet been fully carried through `control` into `expand` and `renew`.
