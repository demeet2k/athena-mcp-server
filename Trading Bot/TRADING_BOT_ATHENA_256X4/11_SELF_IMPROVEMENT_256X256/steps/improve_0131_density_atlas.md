# Improvement Step 0131

## Improvement Signature

- `density` -> `atlas` -> `integrate` -> `relay`
- role owner: `R05 LPPL Watcher`
- metro route: `L5 Signal Line` via `H4 Signal Hub` -> `H7 Replay Hub`
- chapter anchor: `Ch16 Portfolio Tensor`
- appendix anchor: `AppE Triangle Grammar`

## Gap Addressed

This step repairs a local weakness where `density` has not yet been fully carried through `atlas` into `integrate` and `relay`.
