# Improvement Step 2121

## Improvement Signature

- `swarm` -> `atlas` -> `verify` -> `relay`
...
This step repairs a local weakness where `swarm` has not yet been fully carried through `atlas` into `verify` and `relay`.
