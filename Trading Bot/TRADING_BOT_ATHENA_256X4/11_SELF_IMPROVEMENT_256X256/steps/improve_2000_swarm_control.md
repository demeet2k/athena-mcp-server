# Improvement Step 2000

## Improvement Signature

- `swarm` -> `control` -> `repair` -> `seed`
...
This step repairs a local weakness where `swarm` has not yet been fully carried through `control` into `repair` and `seed`.
