...
cal=call('athena_structural_reliability_calibrate',{'calibration_examples':examples,'supports':[.3,.8],'folds':3,'seed':2},12);assert cal['status']=='OUT_OF_FOLD_ISOTONIC_STRUCTURAL_RELIABILITY'
...
transport=call('athena_approx_error_transport',{'feature_order':['x'],'witnesses':[{'features':{'x':0},'absolute_error':.1},{'features':{'x':1},'absolute_error':.2}],'queries':[{'id':'mid','features':{'x':.5},'decision_margin':.8}],'lipschitz_bound':.2,'max_transport_radius':1,'margin_safety':.5},15);assert transport['status']=='DECLARED_LIPSCHITZ_APPROXIMATION_ERROR_TRANSPORT';assert transport['queries'][0]['decision_preserving_under_bound'] is True
...
tmle=call('athena_longitudinal_tmle_crossfit',{'samples':rows,'treatment1':'A1','intermediate':'L1','treatment2':'A2','outcome':'Y','baseline':['X'],'folds':2,'seed':3},18);assert tmle['cross_fitted'] is True
dr=call('athena_sequential_dr_policy_crossfit',{'samples':rows,'treatment1':'A1','intermediate':'L1','treatment2':'A2','outcome':'Y','baseline':['X'],'folds':2,'seed':4,'policies':[{'id':'none','a1':0,'a2':0},{'id':'both','a1':1,'a2':1}]},19);assert dr['cross_fitted'] is True;assert 'OBSERVED_A1_L1' in dr['history_invariant']
...
p.terminate();p.wait(timeout=5);print('SMOKE PASS: 3.4.0 / UNIFIED.11 / V15 CALIBRATION+CROSS-FIT+GAUSSIAN+ERROR-TRANSPORT+TV-DRO / DEPLOYMENT.2 / Y1+TRUST FIREWALLS')
