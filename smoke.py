...
init=rpc('initialize',{'protocolVersion':'2025-11-25','capabilities':{},'clientInfo':{'name':'smoke','version':'10'}},1);assert init['serverInfo']['version']=='3.2.0',init
...
for n in ('athena_orchestrate','athena_cycle_start','athena_self_test','athena_schema_migrate','athena_experiment_design','athena_causal_identify','athena_dual_control_plan','athena_belief_register','athena_decision_evi','athena_gaussian_belief_register','athena_decision_evpi','athena_gp_register','athena_gp_predict','athena_gp_hyperfit','athena_gp_hyperposterior','athena_gp_bma_predict','athena_gp_sparse_predict','athena_pag_candidate_discover','athena_longitudinal_gformula','athena_gp_hyperqmc','athena_gp_fitc_predict','athena_gp_joint_design','athena_fci_lite_discover','athena_longitudinal_tmle','athena_dynamic_policy_value','athena_dro_resource_select','athena_discovery_claim_register','athena_claim_register','athena_promotion_evaluate','athena_promotion_verify_github','athena_finalize_output'):
    assert n in names,n
manifest=resource('athena://manifest',3);assert manifest['artifact']=='ATHENA.RUNTIME.UNIFIED.9',manifest;assert 'PROMOTION.2' in manifest['layers'];assert 'COLLECTIVE_ROBUST_V13' in manifest['layers'];assert 'GITHUB_PROMOTION_VERIFIER.1' in manifest['layers']
...
p.terminate(); print('SMOKE PASS: V8 BELIEF + V10/V12 GP + V13 QMC/FITC/JOINT-DESIGN/FCI-LITE/LONGITUDINAL-TMLE/DYNAMIC-POLICY/ROBUST-RESOURCE + PROMOTION.2 CALLER-BOUND READINESS + GITHUB VERIFIER SURFACE + SCHEMA/SELFTEST + AOR + FAIL-CLOSED CYCLE + FINAL EMISSION')
