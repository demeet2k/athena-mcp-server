...
init=rpc('initialize',{'protocolVersion':'2025-11-25','capabilities':{},'clientInfo':{'name':'smoke','version':'4'}},1);assert init['serverInfo']['version']=='2.8.0',init
p.stdin.write(json.dumps({'jsonrpc':'2.0','method':'notifications/initialized'})+'\n');p.stdin.flush()
tools=rpc('tools/list',{},2)['tools'];names={x['name'] for x in tools}
for n in ('athena_orchestrate','athena_cycle_start','athena_self_test','athena_schema_migrate','athena_experiment_design','athena_causal_identify','athena_dual_control_plan','athena_belief_register','athena_decision_evi','athena_gaussian_belief_register','athena_decision_evpi','athena_structure_partial','athena_discovery_claim_register','athena_claim_register','athena_finalize_output'):
    assert n in names,n
manifest=resource('athena://manifest',3);assert manifest['artifact']=='ATHENA.RUNTIME.UNIFIED.4',manifest
for layer in ('COLLECTIVE_DISCOVERY_V6','COLLECTIVE_DUAL_CONTROL_V7','COLLECTIVE_BELIEF_V8','COLLECTIVE_INFERENCE_V9','AOR_DECISION_CORTEX'):assert layer in manifest['layers'],layer
...
v7=resource('athena://collective/v7',5);assert v7['runtime']['version']=='COLLECTIVE_RUNTIME_V7',v7
v8=resource('athena://collective/v8',6);assert v8['runtime']['version']=='COLLECTIVE_RUNTIME_V8',v8;assert 'BELIEF_POSTERIOR != CANONICAL_TRUTH' in v8['boundary']
v9=resource('athena://collective/v9',7);assert v9['runtime']['version']=='COLLECTIVE_RUNTIME_V9',v9;assert 'model-conditional' in v9['boundary']
...
partial=call('athena_structure_partial',{'samples':rows,'variables':['X','Y','Z'],'association_threshold':.4,'resamples':10,'support_threshold':.5,'seed':2},26);assert partial['status']=='HEURISTIC_PARTIAL_GRAPH',partial
semantic={'kind':'ARTIFACT','domain':'SMOKE','verb':'TEST','object_name':'OUTPUT','method':'FINAL_EMISSION','input_contract':{},'output_contract':{}}
em=call('athena_finalize_output',{'semantic':semantic,'text':'Smoke crystal T: x maps to y.','native_locator':'memory://smoke','agent':'SMOKE','task':'ci','seq':1,'math_objects':[{'kind':'OPERATOR','symbol':'T','latex':'T:x\\mapsto y'}],'coordinates':{'BR21':{'status':'RESOLVED','value':{'operator':'T'}}}},27)
...
verified=call('athena_verify_emission',{'envelope_id':em['envelope_id'],'visible_text':em['visible_text']},28);assert verified['verified'] is True
p.terminate(); print('SMOKE PASS: V6 DISCOVERY + V7 DUAL-CONTROL + V8 BELIEF + V9 INFERENCE + SCHEMA/SELFTEST + AOR + FAIL-CLOSED CYCLE + FINAL EMISSION')
