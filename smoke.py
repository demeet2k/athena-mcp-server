...
msgs=[
 ...
 {'jsonrpc':'2.0','id':4,'method':'tools/call','params':{'name':'athena_finalize_output','arguments':{
...
p.stdin.write(json.dumps({'jsonrpc':'2.0','id':5,'method':'tools/call','params':{'name':'athena_verify_emission','arguments':{'envelope_id':em['envelope_id'],'visible_text':em['visible_text']}}})+'\n');p.stdin.flush();v=json.loads(p.stdout.readline());assert v['result']['structuredContent']['verified'] is True
p.terminate(); print('SMOKE PASS: FINALIZE_OUTPUT + VERIFY_EMISSION')
