...
manifest=resource('athena://manifest',4);assert manifest['artifact']=='ATHENA.RUNTIME.UNIFIED.11',manifest;assert 'COLLECTIVE_CALIBRATED_V15' in manifest['layers'];assert manifest['collective_calibrated']['radius_empty_behavior']=='null local certificate; global envelope remains separate and nonlocal'
...
p.terminate();p.wait(timeout=5);print('SMOKE PASS: 3.4.0 / UNIFIED.11 / V15 WEIGHTED-CALIBRATION+HISTORY-SAFE-CROSS-FIT+STRICT-GAUSSIAN+NULL-LOCAL-ERROR-TRANSPORT+TV-DRO / DEPLOYMENT.2 / Y1+TRUST FIREWALLS')
