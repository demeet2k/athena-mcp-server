# archive-tree-extractor

## address
1020

## priority
P0

## lane
formal-framework

## description
Promote ZIP-backed formal framework trees into canonical live roots so they can be diffed, tested, and referenced like first-class source.

## triggers
- archive tree extractor
- ingest this
- bring this into the system
- capture this source

## inputs
- framework source paths
- kernel references
- archive or live tree

## outputs
- witness-bearing artifact
- source linkage
- replay note

## procedure
1. Identify the correct source surface and its current boundaries.
2. Capture the highest-yield evidence without hiding extraction limits.
3. Normalize the result into the target Athena schema.
4. Emit the resulting artifact together with lineage and next gaps.

## validation
- artifact cites real source paths or live-memory provenance
- lineage is preserved without silent rewrite

## failure modes
- If the source cannot be fully extracted, keep metadata and log the limit.
- If the source boundary is unclear, record the ambiguity before proceeding.

## references
- `C:\Users\dmitr\Documents\Athena Agent\self_actualize\archive_atlas.json`
- `C:\Users\dmitr\Documents\Athena Agent\self_actualize\archive_manifest.json`

## rationale
Archive-backed code exists but still needs live-tree promotion.
