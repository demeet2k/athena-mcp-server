...
"""
Quick benchmark + ablation demo for QP-GEMM.

Run:
    python qpgemm_bench.py

This prints summary tables for:
- exact dense GEMM
- approximate-enabled (low-rank opportunism)
- reuse + noncontiguous (cached contiguity)
"""

import numpy as np

from qpgemm_framework_v3 import QPGEMM, make_ablation_configs, summarize_ablation
...
def make_workloads(seed: int = 0):
    ...
    # Noncontiguous (transpose slice)
    A4 = rng.standard_normal((180, 240)).T  # non-contig (400,300)
    B4 = rng.standard_normal((180, 120))
    # align dims: A4 is (400,300), B4 is (300,200)
    # Use these as a workload where reuse_a matters.
