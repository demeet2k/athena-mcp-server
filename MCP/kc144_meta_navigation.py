...
def register_kc144_meta_navigation(mcp: Any) -> Any:
    """Register KC144 tools exactly once on a FastMCP-compatible server."""
    identity = id(mcp)
    ...
    def locate_kc144(query: str = "KC144 meta compiler") -> str:
        """Locate the executable KC144 compiler through aliases, tags, maps, and routes."""
        return _dump(locate(query))

    @mcp.tool()
    def use_kc144(query: str, operation: str = "compile") -> str:
        """Invoke the located KC144 compiler and emit an M12-bound return receipt."""
    ...
    def kc144_station(address: str = "GID119") -> str:
        """Resolve a GID, grid address, or station code into a KC144 station face."""
        return _dump(station(resolve(address)))

    @mcp.tool()
    def kc144_route(source: str = "H06", target: str = "M12", objective: str = "preserve") -> str:
        """Route through KC144 without treating reachability as evidence or authority."""
    ...
    def kc144_status() -> str:
        """Return navigation readiness and the unchanged production evidence boundary."""
        return _dump(status())

    @mcp.tool()
    def kc144_validate() -> str:
        """Validate coordinates, the map registry, locator, and H06-to-M12 return."""
